...
# Weighted FWL coefficient uniqueness

This file proves the coefficient-uniqueness direction of the
Frisch-Waugh-Lovell identity for the `Causalean.Stat.Weighted` substrate. Let
`c : WeightedSupport R` be a weighted support, `H : Submodule ℝ (R → ℝ)` a
nuisance subspace, and `X : Fin K → R → ℝ` a regressor vector. If a pair
`(β, α)` minimizes

    ω-WLS objective  c.ip (Y - ∑ k β k • X k - α) (Y - ∑ k β k • X k - α)

over all `(β : Fin K → ℝ, α ∈ H)`, then `β` equals the residualized
coefficient `θ̂ := Q_XX⁻¹ * ⟨X̃, Y⟩_ω`, where
`X̃ := c.tildeXVec H X`.

The theorem identifies the coefficient of any supplied joint minimizer. It
does not construct a minimizer or prove that `thetaHat` minimizes a residualized
...
This file states and proves the coefficient-uniqueness direction of the finite
weighted-support Frisch-Waugh-Lovell identity. It constructs the residualized Gram matrix
...
/-- Given [a finite record set](hyp:R), [a nonnegative integer $K$](hyp:K),
[a weighted support](hyp:c), [a family of $K$ regressors](hyp:X), and
[a nuisance subspace](hyp:H), the [residualized regressor Gram matrix](goal) has
at $(j,k)$ the weighted inner product of residualized regressors $j$ and $k$.
...
/-- Given [a finite record set](hyp:R), [a nonnegative integer $K$](hyp:K),
[a weighted support](hyp:c), [a family of $K$ regressors](hyp:X),
[a nuisance subspace](hyp:H), and [an outcome array](hyp:Y), the
[residualized right-hand-side vector](goal) has coordinate $j$ equal to the
weighted inner product of residualized regressor $j$ with the outcome.
...
[a weighted support](hyp:c), [a family of $K$ regressors](hyp:X),
[a nuisance subspace](hyp:H), and [an outcome array](hyp:Y), the
[residualized weighted least-squares coefficient vector](goal) is the inverse
Gram matrix times the residualized right-hand-side vector.
...
/-- Given [a finite record set](hyp:R), [a nonnegative integer $K$](hyp:K),
[a weighted support](hyp:c), [a nuisance subspace](hyp:H), and
[a family of $K$ regressors](hyp:X), the [rank condition](goal) holds exactly
when the residualized Gram determinant is nonzero.
...
/-- **Frisch–Waugh–Lovell at the WeightedSupport level.** Under
[the residualized-regressor rank condition](hyp:hRank),
[the coefficient of any supplied joint minimizer equals `thetaHat c X H Y`](goal).
