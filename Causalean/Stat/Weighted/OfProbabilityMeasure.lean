...
Bridge connecting the finite weighted-support tower (`Causalean/Stat/Weighted/*`) to a
probability measure on a finite measurable space. For a probability measure `μ` on a
`Fintype` `R` with measurable singletons, the point masses
...
/-- Given [a finite measurable record space with measurable singletons](hyp:R)
and [a probability measure](hyp:μ), the [induced weighted support](goal) uses
the positive-mass records as its observed set and each point mass as its weight.

A probability measure on a finite measurable space induces a weighted support
whose weights are point masses and whose observed records have positive mass.
...
/-- For [a finite probability measure `μ`](hyp:μ),
[its weighted inner product equals the corresponding `L²(μ)` integral pairing](goal). -/
