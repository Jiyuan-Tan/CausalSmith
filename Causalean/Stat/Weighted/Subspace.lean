...
public import Causalean.Mathlib.Analysis.InnerProductSpace.Projection
...
[a nuisance subspace](hyp:H), the [weighted orthogonal projection](goal) maps
each array into the subspace so that its residual is weighted-orthogonal to
every array in the subspace.

The `c.ip`-orthogonal projection onto `H`, chosen via classical choice from
`weighted_orthogonal_projection_exists`. -/
...
/-- For [a weighted support](hyp:c), [a subspace](hyp:H), and
[a point to project](hyp:X), if [a candidate lies in the subspace](hyp:Y,hY)
with [a residual orthogonal to that subspace](hyp:horth), then
[at each observed index](hyp:r,hr), [the projection equals the candidate](goal).

Off the observed support the projection is not unique, so no equality is claimed there: the
weighted inner product cannot see those coordinates. -/
...
/-- Given [a finite record set](hyp:R), [a weighted support](hyp:c), and
[a nuisance subspace](hyp:H), the [residual-maker linear map](goal) subtracts
the weighted orthogonal projection onto that subspace from each array.
...
[a nuisance subspace](hyp:H), and [a real-valued array](hyp:X), the
[residualized array](goal) is obtained by applying the subspace's residual maker.
...
/-- **Key orthogonality.** For
[any array `h` in the nuisance subspace `H`](hyp:hH),
[the residualized array is weighted-orthogonal to `h`](goal). -/
...
[a family of real-valued arrays](hyp:X), the [residualized array family](goal)
assigns each array its residual after projection onto the nuisance subspace.
