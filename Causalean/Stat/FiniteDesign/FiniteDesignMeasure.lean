...
Finite probability distributions can be viewed as ordinary probability measures, enabling reuse
of measure-theoretic probability results.
...
/-- For [a finite probability distribution on a finite type equipped with a σ-algebra](hyp:D),
...
/-- The measure induced by a finite probability distribution assigns each set the sum, over
...
/-- For [every finite probability distribution on a finite type equipped with a σ-algebra](hyp:D),
...
/-- **Integral against the induced measure is the expectation.** For [any
statistic `g`](hyp:g), [the measure-theoretic integral of `g` against the design's induced
probability measure equals its expectation](goal). -/
...
/-- **Measure-theoretic variance equals the variance.** For [any statistic `g`](hyp:g),
