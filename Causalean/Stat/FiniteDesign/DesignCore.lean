...
/-! # Finite probability distributions and moments

This file defines `FiniteDesign`, its finite-sum expectation, variance, covariance, event
probability, and pushforward, together with their elementary algebraic identities. Because the
sample space is finite, none of these results needs measure-theoretic integrability assumptions. -/
...
/-- A probability distribution on a finite type: [a probability mass function `p` on a finite
type `Ω`](hyp:p) whose values are [nonnegative](hyp:p_nonneg) and [sum to one](hyp:p_sum).

Design-based causal inference reads `Ω` as an assignment space and `p` as the experimenter's
randomization weights; nothing in this file depends on that reading. -/
structure FiniteDesign (Ω : Type*) [Fintype Ω] where
  /-- Probability mass at the outcome `z`. -/
  p : Ω → ℝ
...
/-- For [a finite probability distribution](hyp:D) and [a real-valued
statistic of the realized outcome](hyp:X), [the expectation](goal) is the
probability-weighted sum of that statistic over all assignments. -/
def E (X : Ω → ℝ) : ℝ := ∑ z, D.p z * X z

/-- For [a finite probability distribution](hyp:D) and [a real-valued
statistic of the realized outcome](hyp:X), [the variance](goal) is the design
expectation of the statistic's squared deviation from its expectation. -/
def Var (X : Ω → ℝ) : ℝ := D.E (fun z => (X z - D.E X) ^ 2)

/-- For [a finite probability distribution](hyp:D) and [two real-valued
statistics of the realized outcome](hyp:X,Y), [the covariance](goal) is the design
expectation of the product of their deviations from their respective expectations. -/
def Cov (X Y : Ω → ℝ) : ℝ := D.E (fun z => (X z - D.E X) * (Y z - D.E Y))

/-- For [an event with decidable membership](hyp:A), [its indicator
...
/-- For [a finite probability distribution](hyp:D) and [an event with
...
expectation of the finite sum `∑ᵢ∈s fᵢ` equals the finite sum of the expectations
`∑ᵢ∈s E[fᵢ]`](goal). -/
...
/-- **Variance identity.** For [any statistic `X`](hyp:X), [its variance equals the
expectation of `X²` minus the square of its expectation](goal). -/
...
[their covariance equals the expectation of `X·Y` minus the product of their
expectations](goal). -/
...
random-variable families `X`, `Y`](hyp:ι,κ,s,t,c,e,X,Y), [their covariance equals
...
variance equals the double sum of weighted covariances
...
/-- **Expectation of an indicator.** For [any event `A`](hyp:A), [the expectation of
...
/-- **Variance of an indicator.** For [any event `A`](hyp:A), [the variance of its
...
/-- For [a finite probability distribution](hyp:D) and [a transformation from
that space to another finite type](hyp:f), [the pushforward distribution](goal)
