...
This module transfers fixed-size i.i.d. empirical-CDF tail bounds through independent
Boolean marking and the resulting random sample size. Its main specialization applies the
proved VC--McDiarmid empirical-CDF bound and its matching count-dependent confidence radius.
-/
...
namespace Causalean.Stat.Quantile.MarkedSubsampleEmpiricalCDF

open Causalean.Stat.Quantile.VCMcDiarmid
...
/-- Given [a finite marked iid experiment](hyp:S), [a requested mark](hyp:a),
[a selected-outcome probability law](hyp:ρ), and [a count-indexed radius](hyp:radius),
[the selected-subsample empirical-CDF bad event is measurable](goal). -/
...
[a selected-outcome law](hyp:ρ), [a Boolean mark factorization](hyp:hfac),
[a count-indexed radius](hyp:radius), [a probability bound](hyp:β), and [the corresponding
fixed-size iid empirical-CDF bound at every positive sample size](hyp:hfixed), [the probability
that the selected arm is nonempty and its uniform empirical-CDF deviation exceeds its
count-indexed radius is at most β](goal). -/
...
/-- Given [a finite marked iid experiment](hyp:S), [a requested mark](hyp:a), [its
mass](hyp:e), [a selected-outcome law](hyp:ρ), [a Boolean mark factorization](hyp:hfac),
and [a confidence parameter between zero and one](hyp:α,hα,hα_one), [the probability that
the nonempty selected subsample's empirical CDF exceeds the VC radius
`sqrt (32 * log (16 * (m+1) / α) / m)` at selected count `m` is at most `α / 2`](goal). -/
theorem conditionalMarkedSubsample_vcMcDiarmidRadius
    {Ω : Type*} [MeasurableSpace Ω] {n : ℕ}
    {μ : Measure Ω} {ν : Measure (Bool × ℝ)}
    [IsProbabilityMeasure μ] [IsProbabilityMeasure ν]
    (S : MarkedIID Ω n μ ν) (a : Bool) (e : ℝ≥0∞) (ρ : Measure ℝ)
    ...
    μ (selectedCDFBadEvent S.Z a ρ (vcMcDiarmidRadius (α / 2))) ≤
      ENNReal.ofReal (α / 2) := by
  have hα_half : 0 < α / 2 := by positivity
  have hα_half_one : α / 2 ≤ 1 := by linarith
  apply conditionalMarkedSubsample_empiricalCDF_tail S a e ρ hfac
