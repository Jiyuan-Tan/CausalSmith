...
/-!
# Overlap couplings through a measurable compression

This file develops the common-submeasure and overlap-coupling construction used by
the coordinatewise direct-product argument.
-/
...
/-- Given [a measurable sample space](hyp:X) and [two probability measures on it](hyp:mu,nu), [the overlap coupling](goal) is the measure on pairs whose [first branch is the diagonal coupling when their total-variation distance is zero](step:1), and whose second branch otherwise combines their common part on the diagonal with the normalized product of their residual measures.

This construction is certified below to have the requested marginals and agreement mass at least
one minus total variation; no exact maximality or universal optimality claim is made. -/
-- @node: overlapCoupling
noncomputable def overlapCoupling
    {X : Type*} [MeasurableSpace X]
    (mu nu : Measure X) [IsProbabilityMeasure mu] [IsProbabilityMeasure nu] :
    Measure (X × X) :=
...
-- @node: overlapCoupling_map_fst
/-- The first marginal of the overlap coupling is the first law. -/
lemma overlapCoupling_map_fst
    {X : Type*} [MeasurableSpace X]
    (mu nu : Measure X) [IsProbabilityMeasure mu] [IsProbabilityMeasure nu] :
    (overlapCoupling mu nu).map Prod.fst = mu := by
  by_cases hzero : Causalean.Stat.tvDist mu nu = 0
  · rw [overlapCoupling, dif_pos hzero,
  ...
  · let xi := mu + nu
    ...
    rw [overlapCoupling, dif_neg hzero]
    dsimp only
...
-- @node: overlapCoupling_map_snd
/-- The second marginal of the overlap coupling is the second law. -/
lemma overlapCoupling_map_snd
    {X : Type*} [MeasurableSpace X]
    (mu nu : Measure X) [IsProbabilityMeasure mu] [IsProbabilityMeasure nu] :
    (overlapCoupling mu nu).map Prod.snd = nu := by
  by_cases hzero : Causalean.Stat.tvDist mu nu = 0
...
-- @node: overlapCoupling.instIsProbabilityMeasure
/-- For [two probability measures](hyp:mu,nu), [their overlap coupling is a probability
measure](goal), because [the coupling has unit total mass](step:1). -/
noncomputable instance overlapCoupling.instIsProbabilityMeasure
    {X : Type*} [MeasurableSpace X]
    (mu nu : Measure X) [IsProbabilityMeasure mu] [IsProbabilityMeasure nu] :
    IsProbabilityMeasure (overlapCoupling mu nu) := by
  constructor
  have h := congrArg (fun m : Measure X => m Set.univ)
    (overlapCoupling_map_fst mu nu)
  change ((overlapCoupling mu nu).map Prod.fst) Set.univ = mu Set.univ at h
  rw [Measure.map_apply measurable_fst MeasurableSet.univ] at h
  simpa using h

-- @node: overlapCoupling_eq_mass_ge
/-- [For two probability measures `mu` and `nu`](hyp:mu,nu) [on a standard Borel space
`X`](hyp:X), [the two coordinates of their overlap coupling agree with probability at
least one minus their total variation distance](goal). -/
lemma overlapCoupling_eq_mass_ge
    {X : Type*} [MeasurableSpace X] [MeasurableEq X]
    (mu nu : Measure X) [IsProbabilityMeasure mu] [IsProbabilityMeasure nu] :
    ENNReal.ofReal (1 - Causalean.Stat.tvDist mu nu) ≤
      overlapCoupling mu nu {p | p.1 = p.2} := by
  ...
  change ENNReal.ofReal (1 - Causalean.Stat.tvDist mu nu) ≤
    overlapCoupling mu nu D
  by_cases hzero : Causalean.Stat.tvDist mu nu = 0
  · rw [overlapCoupling, dif_pos hzero, Measure.map_apply
      (μ := mu) (f := fun x : X => (x, x))
...
/-- Given [two standard Borel measurable spaces, an observation space and a compressed-state space](hyp:Z,S), [two probability measures on the observation space](hyp:Q0,Q1), and [a measurable compression map from observations to compressed states](hyp:compress,hcompress), [the compression coupling](goal) is the joint law obtained by applying the overlap-coupling construction to the two compressed laws and then, conditional on each coupled compressed state, drawing each observation from its corresponding regular conditional distribution.

An overlap coupling of compressed laws, lifted through the two regular
...
noncomputable def compressionCoupling
  ...
  let gamma := overlapCoupling (Q0.map compress) (Q1.map compress)
...
lemma compressionCoupling_map_compress_pair
    ...
    (compressionCoupling Q0 Q1 compress hcompress).map
      ...
      overlapCoupling (Q0.map compress) (Q1.map compress) := by
  letI : Nonempty Z := nonempty_of_isProbabilityMeasure Q0
  ...
  have hK0gamma : ∀ᵐ p ∂gamma, K0.map compress p.1 = Kernel.id p.1 := by
    rw [← overlapCoupling_map_fst (Q0.map compress) (Q1.map compress)] at hK0
    exact ae_of_ae_map measurable_fst.aemeasurable hK0
  have hK1gamma : ∀ᵐ p ∂gamma, K1.map compress p.2 = Kernel.id p.2 := by
    rw [← overlapCoupling_map_snd (Q0.map compress) (Q1.map compress)] at hK1
...
/-- For [two probability measures](hyp:Q0,Q1) and [a measurable compression
map](hyp:compress,hcompress), [the associated compression coupling is a probability
measure](goal), because [the coupling has unit total mass](step:1). -/
...
-- @node: isCoupling_compressionCoupling
/-- Given [two probability measures on a standard Borel observation
space](hyp:Q0,Q1) and [a measurable compression map](hyp:compress,hcompress),
[the lifted compression coupling is a coupling of the original measures](goal).

Its probability property and two marginals are supplied by the preceding
construction lemmas. -/
theorem isCoupling_compressionCoupling
    {Z S : Type*} [MeasurableSpace Z] [StandardBorelSpace Z]
    [MeasurableSpace S] [StandardBorelSpace S]
    (Q0 Q1 : Measure Z) [IsProbabilityMeasure Q0] [IsProbabilityMeasure Q1]
    (compress : Z → S) (hcompress : Measurable compress) :
    IsCoupling (compressionCoupling Q0 Q1 compress hcompress) Q0 Q1 where
  isProbabilityMeasure := inferInstance
  map_fst := compressionCoupling_map_fst Q0 Q1 compress hcompress
  map_snd := compressionCoupling_map_snd Q0 Q1 compress hcompress
...
lemma compressionCoupling_equal_compression_mass_ge
  ...
  change ((compressionCoupling Q0 Q1 compress hcompress).map
    ...
    overlapCoupling (Q0.map compress) (Q1.map compress) D at happ
  ...
  rw [hpre] at happ
  rw [happ]
  exact overlapCoupling_eq_mass_ge (Q0.map compress) (Q1.map compress)
