...
/-- The certificate structure is specified using [the finite node index set](hyp:ι), [the moment-matching degree](hyp:L).  A normalized finite signed moment certificate consists of distinct real
