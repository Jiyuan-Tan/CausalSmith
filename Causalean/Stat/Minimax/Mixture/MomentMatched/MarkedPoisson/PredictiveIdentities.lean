...
/-!
# Predictive identities for label-gated marked-Poisson mixtures

This module proves predictive identities for the four-count experiment defined in
`PalmSplit`.  It identifies the branch-sensitive singleton masses and relates the
marked predictive laws to aggregate Poisson mixtures.
-/
...
/-- If [the offset is positive](hyp:ha), [the scaling constant is positive](hyp:hκ), and [the
certificate's variation measure is supported on the stated interval](hyp:hsupp), then [the two
marked predictive laws assign equal real mass to every singleton with both labeled counts
positive](goal). -/
...
/-- For [a sampling scale](hyp:ε), [an offset](hyp:a), and [an exposure](hyp:t), [the aggregate
Poisson kernel is a Markov kernel](goal), because [each of its output measures is a probability
measure](step:1). -/
...
/-- For [a finite latent index set, moment degree, and normalized moment-matching
certificate](hyp:ι,L,C), [an overlap fraction, support ratio, positive shift, support bound,
and labeled and auxiliary intensities](hyp:ε,κ,a,B,u,v), if [the overlap fraction is positive
and below one half](hyp:hε,hεhalf), [the support ratio has its required value](hyp:hκeq), [the
shift is positive](hyp:ha), [both intensities are nonnegative with positive sum](hyp:hu,hv,ht),
and [the signed prior is supported on the required interval](hyp:hsupp), then for [a selected
branch and three count indices](hyp:first,k,s,t), [the difference between the two marked
prior-predictive singleton masses equals the signed Palm-split difference scaled by the labeled
intensity, overlap, and shift](goal). -/
