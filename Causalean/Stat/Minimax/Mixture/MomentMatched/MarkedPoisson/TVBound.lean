...
/-!
# Total-variation bounds for label-gated marked-Poisson mixtures

This module derives total-variation bounds for the four-count experiment defined in
`PalmSplit`.  It first reduces the marked-law discrepancy to an aggregate Poisson
mixture discrepancy, then combines that reduction with moment cancellation to obtain
a geometric prior-predictive bound.
-/
...
/-- For [a finite signed certificate with its node set and matching degree](hyp:C,ι,L), [the
total-variation discrepancy between the two marked predictive laws is at most the labeled Palm
...
decay factor strictly between zero and one that uniformly bound the one-coordinate marked-Poisson
total-variation distance by a geometric term](goal), whenever total intensity times the support
bound is sufficiently small relative to the matching degree. -/
