...
/-- An [aggregate Poisson observation](goal) records [the treated and control counts as a
pair](step:1). -/
abbrev AggregatePoissonObservation := ℕ × ℕ

/-- The [aggregate treated rate](goal) at [latent mass](hyp:p) is the product of [aggregate
intensity](hyp:t), [overlap fraction](hyp:ε), and the mass plus [shift](hyp:a), as given by
[the displayed formula](step:1). -/
def aggregateTreatedRate (ε a t p : ℝ) : ℝ := t * ε * (p + a)

/-- The [aggregate control rate](goal) at [latent mass](hyp:p) is [aggregate intensity](hyp:t)
times the non-overlap mass after subtracting the contribution from [overlap fraction](hyp:ε)
and [shift](hyp:a), as given by [the displayed formula](step:1). -/
def aggregateControlRate (ε a t p : ℝ) : ℝ :=
  t * ((1 - ε) * p - ε * a)

/-- At [latent mass](hyp:p), [overlap fraction](hyp:ε), [shift](hyp:a), and [aggregate
intensity](hyp:t), the [aggregate Poisson law](goal) is [the product of independent Poisson laws
with the aggregate treated and control rates](step:1). -/
...
/-- The aggregate two-count law at any [overlap fraction](hyp:ε), [shift](hyp:a), [aggregate
intensity](hyp:t), and [latent mass](hyp:p) [is a probability measure](goal), because [it is a
product of Poisson probability measures](step:1). -/
...
negative rates truncated to zero. This kernel is the one-count building block for the aggregate
Poisson experiment. -/
noncomputable def aggregatePoissonKernelOfRealRate
    (r : ℝ → ℝ) : Kernel ℝ ℕ :=
...
/-- The [aggregate affine-Poisson experiment](goal) sends latent mass to treated and control
counts at [overlap fraction](hyp:ε), [shift](hyp:a), and [aggregate intensity](hyp:t), using
[the product of the corresponding one-count kernels](step:1). -/
...
/-- The fibre of the aggregate kernel at [latent mass](hyp:p) [equals the explicit independent
product-Poisson law](goal) for the same [overlap fraction](hyp:ε), [shift](hyp:a), and
[aggregate intensity](hyp:t). -/
...
/-- Mixing [a latent prior](hyp:π) through the aggregate experiment with [overlap
fraction](hyp:ε), [shift](hyp:a), and [aggregate intensity](hyp:t) gives the [prior-predictive
treated/control count law](goal), [defined by the usual prior-predictive mixture](step:1). -/
...
/-- For any [positive tail-scale constant](hyp:A,hA), [there are positive constants and a decay
factor strictly between zero and one that uniformly bound the square root of the scaled
exponential-series tail geometrically in the matching degree](goal), whenever the argument is
at most a sufficiently small multiple of that degree. -/
...
/-- If [the overlap fraction is positive and below one half](hyp:ε,hε,hεhalf) and [the support
ratio has the stated overlap-dependent value](hyp:κ,hκ), [one positive constant uniformly makes
the total-variation distance between the aggregate mixtures of the two Jordan priors no larger
than a square-root exponential-series tail](goal), with argument proportional to intensity
times the support bound. -/
...
/-- If [the overlap fraction is positive and below one half](hyp:ε,hε,hεhalf) and [the support
ratio has the stated overlap-dependent value](hyp:κ,hκ), [there are positive constants and a
decay factor strictly between zero and one that make the two aggregate Jordan-prior predictive
laws geometrically close](goal), whenever intensity times the support bound is sufficiently
small relative to the matching degree. -/
