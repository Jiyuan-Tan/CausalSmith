...
theorem Model.sInf_posteriorResidual_le_minimaxValueReal [Nonempty Theta] [Nonempty B]
  ...
  calc
    ...
    _ ≤ ∑ theta, nu.p theta *
        Causalean.Stat.worstCaseRiskReal (risk M.P M.tau) q := by
      apply Finset.sum_le_sum
      intro theta _
      exact mul_le_mul_of_nonneg_left
...
/-- With [ordered action bounds](hyp:hlu), the [nonnegative extension of the least
posterior residual is a lower bound on the standard extended minimax value](goal). -/
theorem Model.sInf_posteriorResidual_le_minimaxValueENNReal
    [Nonempty Theta] [Nonempty B]
    (M : Model Theta B X) (nu : FiniteDesign Theta) {l u : ℝ} (hlu : l ≤ u) :
    ENNReal.ofReal (sInf (Set.range (M.posteriorResidual nu))) ≤
      Causalean.Stat.minimaxValueENNRealOfReal
        (risk (l := l) (u := u) M.P M.tau) := by
  let _ : Nonempty (Procedure X l u) := procedure_nonempty hlu
  apply Causalean.Stat.le_minimaxValueENNReal
  intro q
  refine (ENNReal.ofReal_le_ofReal (M.sInf_posteriorResidual_le_priorRisk nu q)).trans ?_
  have hfinite : Causalean.Stat.worstCaseRiskOfReal (risk M.P M.tau) q ≠ ∞ :=
    Causalean.Stat.worstCaseRiskOfReal_ne_top_of_bddAbove
      (Finite.bddAbove_range (risk M.P M.tau q))
  change ENNReal.ofReal (nu.E (risk M.P M.tau q)) ≤
    Causalean.Stat.worstCaseRiskOfReal (risk M.P M.tau) q
  rw [ENNReal.ofReal_le_iff_le_toReal hfinite,
    Causalean.Stat.worstCaseRiskENNReal_ofReal_toReal
      (fun theta ↦ risk_nonneg M.P M.tau M.P_nonneg q theta) hfinite]
  calc
    nu.E (risk M.P M.tau q) ≤
        ∑ theta, nu.p theta *
          Causalean.Stat.worstCaseRiskReal (risk M.P M.tau) q := by
      apply Finset.sum_le_sum
      intro theta _
...
/-- [The barycenter procedure](goal) combines [a probability design](hyp:pi) with
[nonnegative grid weights](hyp:w,hw) whose [fiber totals equal the design
probabilities](hyp:hocc). Its decision rule averages [grid actions lying in the closed action
interval](hyp:gamma,l,u,hgamma), using [an in-range default action on zero-mass
fibers](hyp:d,hd). -/
...
theorem Model.minimaxValue_le_of_randomizedGridRiskReal [Nonempty Theta]
    ...
    Causalean.Stat.minimaxValueReal (risk (l := l) (u := u) M.P M.tau) ≤ c := by
  let q : Procedure X l u :=
    barycenterProcedure pi w gamma d hw hocc hd hgamma
  calc
    Causalean.Stat.minimaxValueReal (risk (l := l) (u := u) M.P M.tau) ≤
        Causalean.Stat.worstCaseRiskReal (risk M.P M.tau) q :=
...
/-- Under [nonnegative grid weights](hyp:w,hw), [the occupancy identities](hyp:hocc),
[bounded actions](hyp:hd,hgamma), and [a uniform risk bound](hyp:hrisk), the [standard
extended minimax value is bounded by the nonnegative extension of that bound](goal). -/
theorem Model.minimaxValueENNRealOfReal_le_of_randomizedGridRisk [Nonempty Theta]
    [∀ b, Fintype (X b)] (M : Model Theta B X) (pi : FiniteDesign B)
    (w : ∀ b, X b → G → ℝ) (gamma : G → ℝ) (d : ℝ)
    (hw : ∀ b x g, 0 ≤ w b x g)
    (hocc : ∀ b x, ∑ g, w b x g = pi.p b)
    ...
    (hrisk : ∀ theta, ∑ b, ∑ x, ∑ g,
      M.P theta b x * w b x g * (gamma g - M.tau theta) ^ 2 ≤ c) :
    Causalean.Stat.minimaxValueENNRealOfReal (risk (l := l) (u := u) M.P M.tau) ≤
      ENNReal.ofReal c := by
  let q : Procedure X l u :=
    barycenterProcedure pi w gamma d hw hocc hd hgamma
  calc
    Causalean.Stat.minimaxValueENNRealOfReal (risk (l := l) (u := u) M.P M.tau) ≤
        Causalean.Stat.worstCaseRiskOfReal (risk M.P M.tau) q :=
      Causalean.Stat.minimaxValueENNReal_le_worstCaseRisk q
    _ ≤ ENNReal.ofReal c := Causalean.Stat.worstCaseRiskENNReal_le fun theta ↦
