...
/-- Given a [positive sample size](hyp:hm), a [sample](hyp:z), and an [atom](hyp:c), the
[corresponding empirical probability-vector coordinate equals the empirical frequency](goal). -/
@[simp]
