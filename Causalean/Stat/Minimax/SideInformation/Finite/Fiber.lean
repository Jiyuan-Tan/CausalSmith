...
/-- For [label probabilities](hyp:p), a [target](hyp:tau), [side coordinates](hyp:q,hq),
[action bounds](hyp:l,u), and a [center parameter](hyp:theta0), the
[exact-fiber local minimax value](goal) minimizes finite squared risk over decisions and
maximizes over parameters with the same side law as the center. -/
noncomputable def fiberMinimaxValue (p : Theta → X → ℝ) (tau : Theta → ℝ)
  ...
  (minimaxValueENNRealOfReal (fun (d : BoundedDecision X l u)
    (theta : {theta : Theta // sidePmf q hq theta = sidePmf q hq theta0}) ↦
      finiteSquaredRisk p tau d theta.1)).toReal

/-- For [label probabilities](hyp:p), a [target](hyp:tau), [side coordinates](hyp:q,hq),
[action bounds](hyp:l,u), a [radius](hyp:r), and a [center parameter](hyp:theta0), the
[local minimax value](goal) minimizes finite squared risk over decisions and maximizes over
parameters whose side law is within that radius of the center law. -/
...
private theorem worstCaseRiskOfReal_ne_top_of_le {D S : Type*}
    {risk : D → S → ℝ} {bound : ℝ} (hupper : ∀ d s, risk d s ≤ bound) (d : D) :
    worstCaseRiskOfReal risk d ≠ ⊤ := by
  apply ne_of_lt
  exact (worstCaseRiskENNReal_le fun s =>
    ENNReal.ofReal_le_ofReal (hupper d s)).trans_lt ENNReal.ofReal_lt_top

private theorem minimaxValueENNRealOfReal_ne_top_of_le {D S : Type*} [Nonempty D]
    {risk : D → S → ℝ} {bound : ℝ} (hupper : ∀ d s, risk d s ≤ bound) :
    minimaxValueENNRealOfReal risk ≠ ⊤ := by
  let d : D := Classical.choice inferInstance
  exact ne_of_lt ((minimaxValueENNReal_le_worstCaseRisk d).trans_lt
    (lt_top_iff_ne_top.2 (worstCaseRiskOfReal_ne_top_of_le hupper d)))

/-- On a [compact parameter space](hyp:Theta), if [label and side coordinates and the target
are continuous](hyp:hpcont,hqcont,htau), [probabilities are simplex-valid](hyp:hp,hq), and
[the action interval contains the target](hyp:hlu,htau_mem), then [local minimax values over
radii `1/(n+1)` converge to the exact-fiber minimax value](goal). -/
theorem localMinimaxValue_tendsto_fiber (p : Theta → X → ℝ)
  ...
  have hfiber_le_local (r : ℝ) (hr : 0 ≤ r) :
    ...
    · apply minimaxValueENNRealOfReal_ne_top_of_le
      exact fun d theta ↦ (hrisk d theta.1).2
    · refine minimaxValueENNReal_mono_class
        (fun theta ↦ ⟨theta.1, ?_⟩) ?_
      · simpa [theta.property] using hr
      · exact fun _ _ ↦ ENNReal.ofReal_le_ofReal le_rfl
  ...
  · intro b hb
    ...
    have hfiniteFiber : ∀ d, worstCaseRiskOfReal fiberRisk d ≠ ⊤ :=
      worstCaseRiskOfReal_ne_top_of_le fun d theta ↦ (hrisk d theta.1).2
    have hbddBelow : BddBelow (Set.range (worstCaseRiskReal fiberRisk)) :=
      bddBelow_range_worstCaseRisk hnonnegFiber
    have hvalue : (⨅ d, worstCaseRiskReal fiberRisk d) < c := by
      have hbridge := minimaxValueENNRealOfReal_toReal hnonnegFiber hfiniteFiber
      have hvalue' : (minimaxValueENNRealOfReal fiberRisk).toReal < c := by
        simpa [fiberMinimaxValue, fiberRisk] using hvalue_lt_c
      rw [hbridge] at hvalue'
      simpa [minimaxValueReal] using hvalue'
    ...
    change (minimaxValueENNRealOfReal localRisk).toReal < b
    have hlocalNonneg : ∀ d theta, 0 ≤ localRisk d theta := by
      intro d theta
      exact (hrisk d theta.1).1
    have hlocalFinite : ∀ d, worstCaseRiskOfReal localRisk d ≠ ⊤ :=
      worstCaseRiskOfReal_ne_top_of_le fun d theta ↦ (hrisk d theta.1).2
    rw [minimaxValueENNRealOfReal_toReal hlocalNonneg hlocalFinite]
    calc
      minimaxValueReal localRisk ≤ worstCaseRiskReal localRisk d :=
