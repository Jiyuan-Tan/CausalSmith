...
noncomputable def measurableExactTableMinimaxValue (p : Theta → X → ℝ)
  ...
  minimaxValueReal (measurableExactTableRisk p q tau :
    MeasurableAmbientExactSideProcedure X C l u → Theta → ℝ)

/-- Given [label probabilities](hyp:p), [side-coordinate probabilities](hyp:q), a
[target](hyp:tau), and [action bounds](hyp:l,u), the [standard measurable exact-table
minimax value](goal) is the extended nonnegative infimum of worst-case squared risks. -/
noncomputable def measurableExactTableMinimaxValueENNReal (p : Theta → X → ℝ)
    (q : Theta → C → ℝ) (tau : Theta → ℝ) (l u : ℝ) : ℝ≥0∞ :=
  minimaxValueENNRealOfReal (measurableExactTableRisk p q tau :
    MeasurableAmbientExactSideProcedure X C l u → Theta → ℝ)
...
theorem minimaxValueReal_comp_surjective {E A B : Type*} (risk : E → B → ℝ)
    (phi : A → B) (hphi : Function.Surjective phi) :
...
theorem worstCaseRiskENNReal_comp_surjective {E A B : Type*}
    (risk : E → B → ℝ≥0∞) (e : E) (phi : A → B)
    (hphi : Function.Surjective phi) :
    worstCaseRiskENNReal (fun e a ↦ risk e (phi a)) e =
      worstCaseRiskENNReal risk e := by
  unfold worstCaseRiskENNReal
  apply le_antisymm
  · exact iSup_le fun a ↦ le_iSup (risk e) (phi a)
  · apply iSup_le
    intro b
    obtain ⟨a, rfl⟩ := hphi b
    exact le_iSup (fun a ↦ risk e (phi a)) a

/-- Given an [extended risk](hyp:risk), a [parameter map](hyp:phi), and
[surjectivity](hyp:hphi), [reindexing leaves the extended minimax value unchanged](goal). -/
theorem minimaxValueENNReal_comp_surjective {E A B : Type*}
    (risk : E → B → ℝ≥0∞) (phi : A → B) (hphi : Function.Surjective phi) :
    minimaxValueENNReal (fun e a ↦ risk e (phi a)) =
      minimaxValueENNReal risk := by
  unfold minimaxValueENNReal
  congr 1
  funext e
  exact worstCaseRiskENNReal_comp_surjective risk e phi hphi

/-- Given a [real-valued risk](hyp:risk), a [parameter map](hyp:phi), and
[surjectivity](hyp:hphi), [reindexing leaves its lifted minimax value unchanged](goal). -/
theorem minimaxValueENNRealOfReal_comp_surjective {E A B : Type*}
    (risk : E → B → ℝ) (phi : A → B) (hphi : Function.Surjective phi) :
    minimaxValueENNRealOfReal (fun e a ↦ risk e (phi a)) = minimaxValueENNRealOfReal risk := by
  unfold minimaxValueENNRealOfReal
  exact minimaxValueENNReal_comp_surjective
    (fun e b ↦ ENNReal.ofReal (risk e b)) phi hphi
...
theorem measurableExactTableMinimaxValue_comp_surjective {A B : Type*}
  ...
  exact minimaxValueReal_comp_surjective (measurableExactTableRisk p q tau) phi hphi
...
theorem exactSideMinimaxValue_comp_surjective {A B : Type*}
  ...
  exact minimaxValueReal_comp_surjective (exactSideRisk p q hq tau) phi hphi
...
theorem exactSideMinimaxValue_le_measurableExactTableMinimaxValue [Nonempty Theta]
  ...
  have htable_nonneg
    ...
    exact Finset.sum_nonneg fun x _ ↦
      mul_nonneg ((hp theta).1 x) (sq_nonneg _)
  have htable_bdd (d : MeasurableAmbientExactSideProcedure X C l u) :
  ...
  refine minimaxValue_le_minimaxValue
    (risk := exactSideRisk p q hq tau)
    (risk' := measurableExactTableRisk p q tau)
...
theorem measurableExactTableMinimaxValue_le_empiricalSideMinimaxValue [Nonempty Theta]
  ...
  refine minimaxValue_le_minimaxValue
    (risk := measurableExactTableRisk p q tau)
    (risk' := empiricalSideRisk p q hq tau m)
