...
/-- [The smooth quartic prior](goal) with [center `c`](hyp:c) and [scale `a`](hyp:a) has
density `(15/(16a)) (1 - ((θ-c)/a)²)²` at [parameter values `θ`](hyp:θ) satisfying
`|θ-c| < a`, and density zero elsewhere. -/
noncomputable def smoothPrior (c a θ : ℝ) : ℝ :=
  if |θ - c| < a then
    (15 / (16 * a)) * (1 - ((θ - c) / a) ^ 2) ^ 2
  else 0

/-- [The derivative of the smooth quartic prior](goal) with [center `c`](hyp:c) and
[scale `a`](hyp:a) equals `-(15/(4a³))(θ-c)(1-((θ-c)/a)²)` at [parameter values
`θ`](hyp:θ) satisfying `|θ-c| < a`, and equals zero elsewhere. -/
