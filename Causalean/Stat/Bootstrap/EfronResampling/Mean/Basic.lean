...
/-- Given [an observation statistic](hyp:ψ), [observed data](hyp:x), and [a same-size
resample](hyp:xstar), the [centered bootstrap sum](goal) is the resample's statistic total minus
the data-average total, normalized by the square root of the sample size. -/
def centeredBootstrapSum {n : ℕ} (ψ : X → ℝ) (x xstar : Fin n → X) : ℝ :=
  (Real.sqrt (n : ℝ))⁻¹ *
    ∑ i, (ψ (xstar i) - Causalean.Stat.finMean (fun j ↦ ψ (x j)))

/-- With [a nonzero sample size](hyp:hn), [an observation statistic](hyp:ψ), [observed data](hyp:x),
and [a resample](hyp:xstar), [the centered bootstrap sum equals the square-root-scaled difference
between their averages](goal). -/
theorem centeredBootstrapSum_eq_sqrt_mul_finAverage_sub {n : ℕ} (hn : n ≠ 0)
    ...
    centeredBootstrapSum ψ x xstar = Real.sqrt (n : ℝ) *
      (Causalean.Stat.finMean (fun i ↦ ψ (xstar i)) -
        Causalean.Stat.finMean (fun i ↦ ψ (x i))) := by
  -- Expand both finite averages and `Finset.sum_sub_distrib`; use `Real.sq_sqrt` and `hn` to
  -- rewrite `√n / n` as `(√n)⁻¹`.
  unfold centeredBootstrapSum Causalean.Stat.finMean
...
/-- Given [an observation statistic](hyp:ψ) that [is measurable](hyp:hψ) and [observed data](hyp:x),
[the centered bootstrap sum as a function of the resample is measurable](goal). -/
...
/-- Given [an iid sample](hyp:S), [an observation statistic](hyp:ψ), [a row size](hyp:n), and
[a realized outcome](hyp:ω), the [centered empirical row law](goal) is the empirical distribution
of statistic values centered at their sample average, with a point mass at zero for the zero row. -/
...
/-- For [an iid sample](hyp:S), [an observation statistic](hyp:ψ), [a row size](hyp:n), and
[a realized outcome](hyp:ω), [the centered empirical row law is a probability measure](goal). -/
...
/-- Given [an iid sample](hyp:S), [an observation statistic](hyp:ψ), [a row size](hyp:n), and
[a realized outcome](hyp:ω), the [probability-measure instance for the centered empirical row
law](goal) supplies its unit total mass. -/
...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [an observation statistic](hyp:ψ),
and [a realized outcome](hyp:ω), [the centered empirical row law has mean zero](goal). -/
...
/-- Given [an iid sample](hyp:S), [an observation statistic](hyp:ψ), [a row size](hyp:n), and
[a realized outcome](hyp:ω), [the identity function has a finite second moment under the centered
empirical row law](goal). -/
...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [an observation statistic](hyp:ψ),
and [a realized outcome](hyp:ω), [the centered empirical row law's second moment equals the plug-in
empirical variance](goal). -/
...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [an observation statistic](hyp:ψ),
[a realized outcome](hyp:ω), and [a measurable real set](hyp:A,hA), [the centered empirical law's
restricted second moment equals the corresponding finite empirical average](goal). -/
...
[a row size](hyp:n), and [a realized outcome](hyp:ω), the [conditional bootstrap law of the
centered normalized mean](goal) is Efron's resampling law at positive sizes and a point mass at
zero at size zero. -/
...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [a measurable observation
statistic](hyp:ψ,hψ), and [a realized outcome](hyp:ω), [the bootstrap mean law has the literal
Efron-bootstrap underlying measure](goal). -/
...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [a measurable observation
statistic](hyp:ψ,hψ), and [a realized outcome](hyp:ω), [the conditional bootstrap mean law equals
the normalized iid row-sum law from the centered empirical distribution](goal). -/
theorem bootstrapMeanLaw_eq_iidRowNormalizedSumLaw {n : ℕ} (hn : n ≠ 0)
  ...
  have hmean :
    ...
    unfold Causalean.Stat.finMean IIDSample.sampleVector IIDSample.sampleMean
    rw [smul_eq_mul, Fin.sum_univ_eq_sum_range (fun i ↦ ψ (S.Z i ω)) n]
  funext y
  unfold centeredBootstrapSum Causalean.Stat.iidRowSum g
  simp only [Function.comp_apply]
  rw [hmean]

/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [a row
size](hyp:n), the [sampling law of the square-root-scaled centered sample mean](goal) is its
distribution under repeated draws from the population. -/
...
/-- Given [an observation statistic](hyp:ψ) and [a measure on the observation space](hyp:P), the
[nonnegative population variance](goal) is the variance of the statistic represented as a
nonnegative real number for use as a Gaussian variance parameter. -/
