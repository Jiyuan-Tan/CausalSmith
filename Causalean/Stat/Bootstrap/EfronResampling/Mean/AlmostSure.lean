...
public import Causalean.Stat.Bootstrap.EfronResampling.Mean.Basic
...
/-- Given [an observation statistic](hyp:ψ), [a tail threshold](hyp:M), and [an observation](hyp:x),
the [squared tail contribution](goal) is the statistic's square when its absolute value exceeds
the threshold and zero otherwise. -/
def squaredTail (ψ : X → ℝ) (M : ℝ) (x : X) : ℝ :=
  (ψ x) ^ 2 * Set.indicator {y : X | M ≤ |ψ y|} (fun _ ↦ (1 : ℝ)) x

/-- If [an observation statistic](hyp:ψ) [is measurable](hyp:hψ) and [has an integrable square
under the population law](hyp:hψ2), [its squared-tail expectations vanish as the threshold grows
without bound](goal). -/
...
integrable population square](hyp:hψ2), [the transformed sample mean converges almost surely to
its population expectation](goal). -/
...
/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [an
integrable population square](hyp:hψ2), [the empirical second moment converges almost surely to
the population second moment](goal). -/
...
/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [an
integrable population square](hyp:hψ2), [all integer-threshold squared-tail sample averages
converge on one common probability-one event](goal). -/
...
/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [an
integrable population square](hyp:hψ2), [the plug-in empirical variance converges almost surely
to the population variance](goal). -/
...
/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [an
integrable population square](hyp:hψ2), [the centered empirical row law's second moment converges
...
theorem centeredEmpiricalSecondMoment_tendsto_ae
  ...
  exact (integral_sq_centeredEmpiricalLaw_eq_empiricalVar hn S ψ ω).symm

/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), and [an
integrable population square](hyp:hψ2), [every positive Lindeberg tail of the centered empirical
row law vanishes almost surely at the square-root sample-size scale](goal). -/
