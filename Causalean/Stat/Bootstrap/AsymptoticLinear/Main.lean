...
/-!
# Distributional validity from bootstrap asymptotic linearity

This module proves the distributional consequences of bootstrap asymptotic linearity.  The
sampling and conditional bootstrap laws approach the same centered Gaussian law, hence approach
one another in Kolmogorov distance, and conditional quantiles converge in sampling probability.
The downstream `Coverage` module turns these results into coverage theorems for percentile and
basic intervals.
-/
...
/-- For [a bootstrap-asymptotically-linear estimator](hyp:h), [the sampling distribution of the
root-sample-size-scaled estimator converges weakly to the centred Gaussian law whose variance is the
influence function's second moment](goal). -/
theorem sampling_tendsto_gaussian (h : BootstrapAsymLinear S est theta0 psi) :
  ...
  have hnormal' := (Tendsto_dist_iff _ _ _ hscaled).1 hnormal
...
/-- For [a bootstrap-asymptotically-linear estimator](hyp:h), [the conditional bootstrap
...
theorem bootstrap_tendsto_gaussian (h : BootstrapAsymLinear S est theta0 psi) :
  ...
  have hlinearLawMeas (n : ℕ) : Measurable (fun omega =>
    ...
    by_cases hn : n = 0
    · subst n
      simp only [bootstrapMeanLaw, ↓reduceDIte]
      exact measurable_const
    · rw [show (fun omega => cdfKolmogorov
            (bootstrapMeanLaw S psi h.meas.2 n omega)
            h.asymptoticGaussian) =
          fun omega => ⨆ q : ℚ,
            |bootstrapCDF
                (centeredBootstrapSum psi (S.sampleVector n omega))
          ...
          let _ : IsProbabilityMeasure
            ...
            bootstrapLaw_isProbabilityMeasure _ _ hn
              (measurable_centeredBootstrapSum psi h.meas.2 _)
      ...
      rw [show (fun omega =>
          ...
          fun omega => ((n : ℝ) ^ n)⁻¹ *
            ∑ j : Fin n → Fin n,
              if centeredBootstrapSum psi (S.sampleVector n omega)
      ...
      have hstat : Measurable (fun omega =>
        ...
        unfold centeredBootstrapSum finMean
  ...
  let nu (n : ℕ) (omega : Omega) : ProbabilityMeasure (ℕ → X) :=
    if hn : n = 0 then
      ...
      let _ : IsProbabilityMeasure
          (bootstrapResample (S.sampleVector n omega)) :=
        bootstrapResample_isProbabilityMeasure _ hn
      (⟨(bootstrapResample (S.sampleVector n omega)).map (extend n omega),
        Measure.isProbabilityMeasure_map (hextMeas n omega).aemeasurable⟩ :
        ProbabilityMeasure (ℕ → X))
  let linear (n : ℕ) (omega : Omega) (y : ℕ → X) : ℝ :=
    centeredBootstrapSum psi (S.sampleVector n omega) (fun i => y i)
  ...
  have hlinearMeas (n : ℕ) (omega : Omega) : Measurable (linear n omega) := by
    unfold linear
    exact (measurable_centeredBootstrapSum psi h.meas.2 _).comp
  ...
  have hsumlaw {n : ℕ} (hn : n ≠ 0) (omega : Omega) :
    ...
    simp [bootstrapEstimatorLaw, hn, bootstrapLaw]
    congr 1
    funext xstar
  ...
  have hlinearMapped : Tendsto_inProb (fun n omega =>
    ...
    exact Filter.Eventually.of_forall fun omega =>
      congrArg (fun law => cdfKolmogorov law h.asymptoticGaussian)
...
/-- **Bootstrap consistency.** For [a bootstrap-asymptotically-linear estimator](hyp:h), [the
...
theorem consistent (h : BootstrapAsymLinear S est theta0 psi) :
  ...
  have hGcont : Continuous (cdf (h.asymptoticGaussian : Measure ℝ)) := by
    rw [hG]
    exact continuous_cdf_gaussianReal_zero hv
  ...
  have htarget_nonneg := cdfKolmogorov_nonneg
    (bootstrapEstimatorLaw est h.meas.1 n (S.sampleVector n omega))
    (samplingEstimatorLaw S est theta0 h.meas.1 n)
  have hsum_nonneg : 0 ≤
      cdfKolmogorov
        ...
        cdfKolmogorov h.asymptoticGaussian
          (samplingEstimatorLaw S est theta0 h.meas.1 n) :=
    add_nonneg (cdfKolmogorov_nonneg _ _)
      (cdfKolmogorov_nonneg _ _)
  rw [abs_of_nonneg htarget_nonneg] at homega
  rw [abs_of_nonneg hsum_nonneg]
  exact homega.trans (cdfKolmogorov_triangle _ _ _)

/-- For [a bootstrap-asymptotically-linear estimator](hyp:h) and [an interior probability
level](hyp:hbeta0,hbeta1), [the lower conditional bootstrap quantile converges in sampling
probability to the corresponding centered-Gaussian quantile](goal), the influence standard
...
theorem quantile_tendsto (h : BootstrapAsymLinear S est theta0 psi)
  ...
  have hquantLaw : Tendsto_inProb
    ...
    apply quantile_tendsto_inProb_of_cdfKolmogorov
      ...
      G hbeta0 hbeta1
    · exact (continuous_cdf_gaussianReal_zero hv).continuousAt
  ...
  apply TendstoInMeasure.congr'
    (f := fun n omega => quantile
      (bootstrapEstimatorLaw est h.meas.1 n (S.sampleVector n omega) : Measure ℝ) beta)
    (g := fun _ => quantile (G : Measure ℝ) beta) _ _ hquantLaw
  · filter_upwards [eventually_ne_atTop 0] with n hn
    exact Filter.Eventually.of_forall fun omega =>
      (bootstrapQuantile_eq_quantile_bootstrapEstimatorLaw est h.meas.1
        (bne_iff_ne.mpr hn) beta (S.sampleVector n omega)).symm
  · exact Filter.Eventually.of_forall fun omega => by
      dsimp [G]
      rw [quantile_gaussianReal_zero hv hbeta0 hbeta1]
