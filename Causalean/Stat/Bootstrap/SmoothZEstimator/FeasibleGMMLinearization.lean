...
/-!
# Feasible GMM: the conditional bootstrap linearization

This module proves that the centered feasible-GMM bootstrap estimator has the GMM
influence-function linearization in bootstrap probability, in outer sampling probability.
-/
...
-- The nested conditional-event proof contains large dependent `Fin n` set expressions.
set_option maxHeartbeats 4000000 in
/-- For [a smooth GMM problem](hyp:prob,reg), [an iid sample, sample-function estimator, and
sample-function weight estimator](hyp:S,est,weightEst), [data and bootstrap parameter
consistency](hyp:hConsistent,hBootConsistent), [data and bootstrap weight
consistency](hyp:hWeight,hBootWeight), and [negligible data and bootstrap first-order-condition
residuals](hyp:hApproxFOC,hBootFOC), [the centered feasible-GMM bootstrap estimator has the GMM
influence-function linearization in bootstrap probability, in outer sampling probability](goal). -/
theorem feasibleGMM_bootstrapLinearization_of_smoothMoment
  ...
  have qJmeas (n : ℕ) : Measurable (qJ n) := by
    let A : Set ((Fin n → X) × (Fin n → X)) := {z |
      a / kJ < ‖finMean (fun i ↦ gJ (z.2 i)) -
        finMean (fun i ↦ gJ (z.1 i))‖}
  ...
  have qLmeas (n : ℕ) : Measurable (qL n) := by
    let A : Set ((Fin n → X) × (Fin n → X)) := {z |
      1 < |finMean (fun i ↦ L (z.2 i)) -
        finMean (fun i ↦ L (z.1 i))|}
    have hA : MeasurableSet A := by
      apply measurableSet_lt measurable_const
      unfold finMean
      fun_prop
  ...
  have hfoc := (hBootFOC a ha inner hinner).eventually
    (Metric.ball_mem_nhds 0 halpha)
  -- Name the consistency radius so the large `simp` calls below do not unfold it.
  let rcons : ℝ := min (a / ML) deltaL
  have hrcons : 0 < rcons := lt_min (div_pos ha hML) hdeltaL
  have hrcons_aML : rcons ≤ a / ML := min_le_left _ _
  have hrcons_delta : rcons ≤ deltaL := min_le_right _ _
  have hcons := (hBootConsistent rcons hrcons inner hinner).eventually
    (Metric.ball_mem_nhds 0 halpha)
  ...
  let badCons : Set Omega := {omega | inner <
    (bootstrapResample (S.sampleVector n omega)).real
      {xstar | rcons < ‖est n xstar - prob.θ₀‖}}
  ...
  have houterSubset :
    ...
    have hbadSubset : bad ⊆
      ...
      -- The good event also places the resampled estimator inside the ball on which the
      -- Lipschitz envelope is valid.
      have hball : est n xstar ∈ Metric.closedBall prob.θ₀ deltaL := by
        have hdmin : ‖est n xstar - prob.θ₀‖ ≤ rcons := le_of_not_gt hConsx
        simpa [Metric.mem_closedBall, dist_eq_norm] using
          hdmin.trans hrcons_delta
      have hθ₀ball : prob.θ₀ ∈ Metric.closedBall prob.θ₀ deltaL :=
        Metric.mem_closedBall_self hdeltaL.le
      have hJbridge : finMean
      ...
      have hLbridge : finMean (fun i ↦ L (data i)) =
      ...
      have hJstar : ‖finMean
        ...
        have heq : eJ (finMean
            ...
            finMean (fun i ↦ reg.deriv prob.θ₀ (data i))) =
            finMean (fun i ↦ gJ (xstar i)) -
            finMean (fun i ↦ gJ (data i)) := by
          simp [gJ, finMean, map_sub, map_sum]
        have horig : ‖finMean
            (fun i ↦ reg.deriv prob.θ₀ (xstar i)) -
            finMean (fun i ↦ reg.deriv prob.θ₀ (data i))‖ ≤ a := by
          calc
            _ = ‖eJ.symm (eJ (finMean
                (fun i ↦ reg.deriv prob.θ₀ (xstar i)) -
                finMean
                  (fun i ↦ reg.deriv prob.θ₀ (data i))))‖ := by simp
            _ ≤ ‖eJ.symm.toContinuousLinearMap‖ * ‖eJ (finMean
                (fun i ↦ reg.deriv prob.θ₀ (xstar i)) -
                finMean
                  (fun i ↦ reg.deriv prob.θ₀ (data i)))‖ :=
        ...
        calc
          _ ≤ ‖finMean
            ...
            convert norm_add_le
              (finMean (fun i ↦ reg.deriv prob.θ₀ (xstar i)) -
                S.sampleMeanVec (reg.deriv prob.θ₀) n omega)
      ...
      have hLstar : finMean (fun i ↦ L (xstar i)) ≤ ML := by
        have hdiff := le_trans (le_abs_self
          (finMean (fun i ↦ L (xstar i)) -
            finMean (fun i ↦ L (data i)))) (le_of_not_gt hLx)
        have hdataDev : |finMean (fun i ↦ L (data i)) -
            ∫ x, L x ∂P| ≤ 1 := by rw [hLbridge]; exact hLdataBound.le
        have hdataUpper : finMean (fun i ↦ L (data i)) ≤
            ∫ x, L x ∂P + 1 := by
          have := (le_abs_self (finMean (fun i ↦ L (data i)) -
      ...
      have hdmin : ‖est n xstar - prob.θ₀‖ ≤ rcons := le_of_not_gt hConsx
      have hd : ‖est n xstar - prob.θ₀‖ ≤ a / ML := hdmin.trans hrcons_aML
      let C : ℝ := ‖finMean
      ...
      have hC : C ≤ 3 * a := by
        have hprod : finMean (fun i ↦ L (xstar i)) *
      ...
      have hJtheta : ‖gmmSampleJacobianFn reg (est n xstar) xstar - prob.G‖ ≤ C := by
        calc
          ...
          _ ≤ C := by
            dsimp [C]
            gcongr
            unfold gmmSampleJacobianFn finMean
            ...
            calc
              ‖(n : ℝ)⁻¹ • ∑ i,
                ...
                exact mul_le_mul_of_nonneg_left (norm_sum_le _ _)
                  (inv_nonneg.2 hnnonneg)
              _ ≤ (n : ℝ)⁻¹ * ∑ i,
                    L (xstar i) * ‖est n xstar - prob.θ₀‖ := by
                gcongr with i
                exact hLbound _ hball _ hθ₀ball _
              _ = finMean (fun i ↦ L (xstar i)) *
      ...
      have hcenterG : ‖scaledBootstrapMeanDifference
        ...
        have heq : eG (scaledBootstrapMeanDifference
          ...
          simp [scaledBootstrapMeanDifference, gG, finMean,
            map_sub, map_sum]
        calc
