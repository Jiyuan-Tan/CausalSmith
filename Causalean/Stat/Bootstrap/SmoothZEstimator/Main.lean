...
/-- For [a smooth Z-estimator regularity package](hyp:reg), [an iid sample, measurable
sample-function estimator, and continuous linear contrast](hyp:S,est,hEstMeas,c), [data and
bootstrap consistency](hyp:hConsistent,hBootConsistent), [a negligible root-sample-size data-score
residual](hyp:hSolve), [a negligible root-sample-size bootstrap-score residual in conditional
probability, in outer sampling probability](hyp:hBootSolve), and [a positive contrast influence
variance](hyp:hVar), [the contrasted estimator is bootstrap asymptotically linear with influence
`-c(J₀⁻¹ psi(theta0,·))`](goal). -/
theorem zEstimator
  ...
  have hboot_eq : ∀ n (x xstar : Fin n → X),
    ...
    · rw [centeredBootstrapSum_eq_sqrt_mul_finAverage_sub hn]
      simp [centeredEstimatorBootstrapStatistic, zEstimatorBootstrapRemainder,
        ...
        finMean, hinfluence, map_sub, map_smul, map_sum]
...
/-- For [a score, target and population law](hyp:psi,theta0,P) with [smooth Z-estimator
regularity](hyp:reg), [an iid sample](hyp:S), [a measurable estimator sequence](hyp:est,hEstMeas),
[a linear contrast](hyp:c), [sampling consistency](hyp:hConsistent), [a negligible
root-sample-size sampling-score residual](hyp:hSolve), [bootstrap consistency](hyp:hBootConsistent),
[a negligible root-sample-size bootstrap-score residual in conditional probability, in outer
sampling probability](hyp:hBootSolve), [a positive influence variance](hyp:hVar), and
[an interior error level](hyp:halpha0,halpha1), [the percentile
bootstrap interval for the contrast covers the true contrast with probability tending to one minus
that level](goal). -/
...
[a negligible root-sample-size bootstrap-score residual in conditional probability, in outer
sampling probability](hyp:hBootSolve), [a positive influence variance](hyp:hVar), and
[an interior error level](hyp:halpha0,halpha1), [the basic
bootstrap interval for the contrast covers the true contrast with probability tending to one minus
that level](goal). -/
