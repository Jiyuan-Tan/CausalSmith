...
# Critical radius and star-shaped envelopes

The **critical radius** `δ_n` of a non-negative function `ψ : ℝ → ℝ` is
the smallest `δ > 0` with `ψ δ ≤ δ²`. Under the star-shaped-class envelope
condition of Wainwright (2019), Lemma 13.6 and Eq. (13.16), `ψ` is non-negative
and non-decreasing on non-negative radii, and `ψ(r)/r` is non-increasing on
positive radii. The
...
Reference: Wainwright, *High-Dimensional Statistics* (2019), Lemma 13.6 and
Eq. (13.16). Bartlett, Bousquet, and Mendelson (2005), Definition 3.1 uses the
different ratio `ψ(r)/√r`; `isStarShapedEnvelope_of_bbm` connects that stronger
condition to the one used here.
-/
...
This file defines deterministic critical radii for star-shaped envelopes of local
...
`criticalRadius`, the predicate `IsStarShapedEnvelope`, fixed-point and positivity
...
/-- For [a real-valued radius envelope](hyp:ψ), [the star-shaped-envelope condition](goal) holds exactly when (1) [the envelope is nonnegative at every nonnegative radius](step:1), (2) [for every two nonnegative radii with the first no larger than the second, the envelope at the first is no larger than the envelope at the second](step:2), and (3) [for every two positive radii with the first no larger than the second, the envelope divided by the radius is no smaller at the first than at the second](step:3).

This is the star-shaped-class property used by Wainwright (2019), Lemma 13.6
and Eq. (13.16). It is not the Bartlett--Bousquet--Mendelson sub-root condition,
which asks for `ψ(r)/√r` to be non-increasing. -/
def IsStarShapedEnvelope (ψ : ℝ → ℝ) : Prop :=
  (∀ r ≥ 0, 0 ≤ ψ r) ∧
  (∀ r₁ r₂, 0 ≤ r₁ → r₁ ≤ r₂ → ψ r₁ ≤ ψ r₂) ∧
  (∀ r₁ r₂, 0 < r₁ → r₁ ≤ r₂ → ψ r₁ / r₁ ≥ ψ r₂ / r₂)

/-- If [an envelope is nonnegative](hyp:hnonneg), [is nondecreasing](hyp:hmono),
and [satisfies the Bartlett--Bousquet--Mendelson sub-root ratio condition on
positive radii](hyp:hbbm), then [it is a Wainwright star-shaped envelope](goal). -/
theorem isStarShapedEnvelope_of_bbm {ψ : ℝ → ℝ}
    (hnonneg : ∀ r, 0 ≤ ψ r) (hmono : Monotone ψ)
    (hbbm : AntitoneOn (fun r => ψ r / Real.sqrt r) (Set.Ioi 0)) :
    IsStarShapedEnvelope ψ := by
  refine ⟨fun r _ => hnonneg r, fun r₁ r₂ _ hr => hmono hr, ?_⟩
  intro r₁ r₂ hr₁ hr₁₂
  have hr₂ : 0 < r₂ := lt_of_lt_of_le hr₁ hr₁₂
  have hs₁ : 0 < Real.sqrt r₁ := Real.sqrt_pos.2 hr₁
  have hs₂ : 0 < Real.sqrt r₂ := Real.sqrt_pos.2 hr₂
  have hsle : Real.sqrt r₁ ≤ Real.sqrt r₂ := Real.sqrt_le_sqrt hr₁₂
  have hratio : ψ r₂ / Real.sqrt r₂ ≤ ψ r₁ / Real.sqrt r₁ :=
    hbbm hr₁ hr₂ hr₁₂
  have hqnonneg : 0 ≤ ψ r₁ / Real.sqrt r₁ :=
    div_nonneg (hnonneg r₁) hs₁.le
  calc
    ψ r₂ / r₂ = (ψ r₂ / Real.sqrt r₂) / Real.sqrt r₂ := by
      rw [div_div, Real.mul_self_sqrt hr₂.le]
    _ ≤ (ψ r₁ / Real.sqrt r₁) / Real.sqrt r₂ :=
      div_le_div_of_nonneg_right hratio hs₂.le
    _ ≤ (ψ r₁ / Real.sqrt r₁) / Real.sqrt r₁ :=
      div_le_div_of_nonneg_left hqnonneg hs₁ hsle
    _ = ψ r₁ / r₁ := by
      rw [div_div, Real.mul_self_sqrt hr₁.le]

/-- **Star-shaped-envelope inequality.** If [`ψ` is a star-shaped envelope](hyp:h) and [`δ*` is a positive
...
lemma starShapedEnvelope_homogeneity {ψ : ℝ → ℝ} (h : IsStarShapedEnvelope ψ)
    {δ_star r : ℝ}
...
/-- **Star-shaped-envelope continuity.** If [`ψ` is a star-shaped envelope](hyp:h), then [`ψ` is continuous on the
    open ray `(0, ∞)`](goal). The non-increasing ratio condition `ψ(r)/r ↘` supplies
    the linear squeeze bounds, while monotonicity of `ψ` supplies the opposite
    side of the squeeze. -/
lemma isStarShapedEnvelope_continuousOn_Ioi {ψ : ℝ → ℝ} (h : IsStarShapedEnvelope ψ) :
...
/-- **Fixed-point property at the critical radius.** If [`ψ` is a star-shaped envelope](hyp:h) and
    ...
    Proof sketch: by star-shaped-envelope continuity (see
    `isStarShapedEnvelope_continuousOn_Ioi`)
    ...
    set). -/
lemma criticalRadius_fp_of_isStarShapedEnvelope {ψ : ℝ → ℝ} (h : IsStarShapedEnvelope ψ)
  ...
  have hcont : ContinuousAt ψ c :=
    ((isOpen_Ioi).continuousOn_iff.mp (isStarShapedEnvelope_continuousOn_Ioi h)) hcpos
  have hψ_tendsto :
...
/-- **Positivity of the critical radius.** If [`ψ` is a star-shaped envelope](hyp:h), [`r₀` is a
    ...
    (using `isStarShapedEnvelope_continuousOn_Ioi`), the infimum is `≥ ε > 0`. -/
lemma criticalRadius_pos_of_isStarShapedEnvelope {ψ : ℝ → ℝ}
    (h : IsStarShapedEnvelope ψ)
