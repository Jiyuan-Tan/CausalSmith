...
/-- Given [a natural dimension](hyp:d), [a maximal degree](hyp:p),
[a bandwidth and two relative annulus endpoints](hyp:q,a,b),
[a coefficient bound](hyp:B), [a center](hyp:x),
[a boxed coefficient vector](hyp:β), and [an evaluation point](hyp:z),
[the bounded radial polynomial](goal) is the coefficient-weighted sum of the
radial annulus monomials about the shared center. -/
...
/-- Given [an observation space equipped with a σ-algebra](hyp:Ω),
[a natural dimension](hyp:d), [a maximal degree](hyp:p),
[a Euclidean location map](hyp:loc),
[a bandwidth and two relative annulus endpoints](hyp:q,a,b),
[a coefficient bound](hyp:B), [a radial-polynomial parameter](hyp:θ), and
[an observation](hyp:ω), [the observation-indexed bounded radial
polynomial](goal) is evaluated at the Euclidean location of that observation. -/
...
/-- Given [an observation space equipped with a σ-algebra](hyp:Ω),
[an arm index set](hyp:A), [a natural dimension](hyp:d),
[a maximal degree](hyp:p), [a Euclidean location map](hyp:loc),
[an arm-valued function](hyp:arm), [a response function](hyp:response),
[a bandwidth and two relative annulus endpoints](hyp:q,a,b),
[a coefficient bound](hyp:B), [a degree index](hyp:j),
[a residual-score parameter](hyp:θ), and [an observation](hyp:ω),
[the radial residual score](goal) is the arm value times the selected radial
monomial times the residual from the shared-center polynomial fit. -/
...
/-- For [a Euclidean dimension and polynomial degree](hyp:d,p), [there are
entropy witnesses, depending only on those values and the finite arm type, that
cover every admissible radial residual-score class](goal). In particular, the
witnesses are uniform over location maps, arms, responses, bandwidths, annuli,
and envelope radii. -/
theorem radialResidualScore_hasUniformPolynomialL2CoverWith
  ...
  have hradialDegree (k : Fin (p + 1)) :
    ...
    exact Causalean.Stat.Concentration.HasUniformPolynomialL2CoverOver.pullback
  ...
  have hpoly :
    ...
    have hpull :=
      Causalean.Stat.Concentration.HasUniformPolynomialL2CoverOver.pullback
