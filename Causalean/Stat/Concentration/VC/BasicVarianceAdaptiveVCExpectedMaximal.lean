...
factor-two increase in the entropy base](goal). -/
...
/-- Given [an empirical polynomial-cover certificate](hyp:hF) and
[an enlarged envelope](hyp:hUV), [the larger envelope keeps the same centers,
entropy base, and exponent](goal). -/
...
/-- [Named arbitrary-law polynomial-cover witnesses](hyp:hF)
[give empirical witnesses after the displayed base and exponent normalization](goal). -/
