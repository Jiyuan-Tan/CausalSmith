...
/-!
# Finite-trace tools for VC and pseudo-dimension bounds

This module provides a homogeneous linear-sign VC bound, deliberately coarse but
explicit bounds for finite Boolean combinations and finite unions, and
reindexing lemmas for both Boolean VC dimension and pseudo-dimension.

The Boolean-combination theorem is trace-level: its combining formula may
depend on the sampled point.  In particular, it supports applications whose
formula changes with the sign of the sampled threshold.
-/
...
/-- Given [a number of Boolean component classes](hyp:m) and
[a common VC-dimension bound](hyp:d), [the Boolean-combination VC bound](goal)
is the explicit natural number $2^{m(d+1)+1}$. -/
def booleanCombinationVCBound (m d : ℕ) : ℕ :=
  2 ^ (m * (d + 1) + 1)

/-- Given [a number of component classes](hyp:m) and
[a common VC-dimension bound](hyp:d), [the finite-union VC bound](goal) is the
explicit natural number $2^{m(d+1)+1}$. -/
def finiteUnionVCBound (m d : ℕ) : ℕ :=
  2 ^ (m * (d + 1) + 1)

/-- Given [a Boolean class with a finite-trace VC bound](hyp:hπ) and
[a reparameterization map](hyp:e), [reparameterizing the class preserves that
VC bound](goal). -/
...
/-- Given [a Boolean class with a finite-trace VC bound](hyp:hπ) and
[a map into its observation domain](hyp:g), [precomposing the observation
argument preserves that VC bound](goal). -/
...
/-- Given [a real-valued class with a pseudo-dimension bound](hyp:hF) and
[a reparameterization map](hyp:e), [reparameterizing the class preserves that
pseudo-dimension bound](goal). -/
...
/-- Given [a real-valued class with a pseudo-dimension bound](hyp:hF) and
[a map into its observation domain](hyp:g), [precomposing every function
preserves that pseudo-dimension bound](goal). -/
...
/-- Given [a finite feature index set](hyp:K),
[a real-valued feature family](hyp:φ), [a real coefficient vector](hyp:θ), and
[an evaluation point](hyp:x), [the homogeneous linear-sign classifier](goal)
returns true exactly when the coefficient-feature inner product is positive. -/
noncomputable def linearSignClass {K : Type v} [Fintype K]
    (φ : K → 𝒳 → ℝ) (θ : K → ℝ) (x : 𝒳) : Bool :=
  decide (0 < ∑ k, θ k * φ k x)

/-- Given [a real-valued feature family](hyp:φ), [the associated homogeneous
linear threshold classifiers have VC dimension at most the number of
coordinates](goal). -/
...
/-- Given [Boolean classifier families](hyp:π), [a common VC-dimension
certificate for them](hyp:hπ), and [a point-dependent combining rule](hyp:combine),
[the combined class has VC dimension at most
`booleanCombinationVCBound m d`](goal). -/
...
/-- Given [a finite family of Boolean classifier families](hyp:π) and
[a common VC-dimension certificate for them](hyp:hπ), [their dependent union
has VC dimension at most `finiteUnionVCBound m d`](goal). -/
