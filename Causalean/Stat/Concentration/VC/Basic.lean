...
/-- For [a real-valued function class](hyp:F) and [a dimension bound](hyp:d),
[having pseudo-dimension at most that bound is equivalent to bounding every
finite threshold-trace VC dimension](goal). -/
...
/-- Given [a VC-dimension bound](hyp:d), [nonnegative coordinate weights](hyp:w,hw),
[a scale and positive separation below that scale](hyp:r,ε,hε,hεr),
[a bound on the total weight](hyp:hwsum), and [a finite Boolean class with that
VC bound and pairwise weighted-Hamming separation](hyp:P,hvc,hsep), [the class has
the stated coarse explicit polynomial packing bound](goal). -/
theorem coarse_vc_weightedHamming_packing_card_le
...
private theorem finiteMeasure_boolean_packing_card_le
  ...
  rw [← hPcard]
  exact coarse_vc_weightedHamming_packing_card_le
    d w hw r ε hε hεr hwsum P hvcP hsepP
...
/-- [A pseudo-dimension bound for a real-valued class](hyp:hpdim) implies that
[the subgraph labels on any chosen finite thresholded sample](hyp:T) [have VC
dimension at most the same bound](goal). -/
