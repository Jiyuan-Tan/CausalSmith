...
/-- If [the envelope bound is positive](hyp:hU), [every function in the class is
measurable](hyp:hmeas) and [bounded by that envelope](hyp:henv), and [the sample size is
positive](hyp:hn), then [the extended-nonnegative-real supremum along an enumeration equals
`ENNReal.ofReal` applied to the shared real-valued `uniformDeviation` supremum](goal) for [the
measure, class, enumeration, and sample under consideration](hyp:μ,g,g0,w). -/
lemma countableEmpiricalProcessSup_eq_of_envelope {Ω ι : Type*} [MeasurableSpace Ω]
    ...
    countableEmpiricalProcessSup μ g g0 w =
      ENNReal.ofReal (uniformDeviation n (fun k => g (g0 k)) μ id w) := by
  simp only [countableEmpiricalProcessSup, uniformDeviation,
...
private lemma uniformDeviation_enumeration_le_two_envelope {Ω ι : Type*} [MeasurableSpace Ω]
    (μ : Measure Ω) [IsProbabilityMeasure μ]
    (g : ι → Ω → ℝ) (g0 : ℕ → ι) {U : ℝ} (hU : 0 < U)
    (hmeas : ∀ i, Measurable (g i)) (henv : ∀ i x, |g i x| ≤ U)
    {n : ℕ} (hn : 0 < n) (w : Fin n → Ω) :
    uniformDeviation n (fun k => g (g0 k)) μ id w ≤ 2 * U := by
...
lemma vcEntropy_chaining_bound
  ...
  have hreal :
      ∫ w, uniformDeviation n F μ id w ∂μn ≤
  ...
  have hdevBound : ∀ w : Fin n → Ω, uniformDeviation n F μ id w ≤ 2 * U := by
    intro w
    exact uniformDeviation_enumeration_le_two_envelope μ g g0 hU hmeas henv hn0 w
