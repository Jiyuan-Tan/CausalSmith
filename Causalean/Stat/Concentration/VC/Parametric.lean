...
/-- For [a finite real-valued feature family](hyp:φ) and
[a coefficient bound](hyp:B), [the associated bounded-coefficient linear class
has pseudo-dimension at most the number of coordinates plus one](goal). -/
...
/-- Given [a Boolean classifier family](hyp:π), [a dimension bound](hyp:d), and
[a VC certificate at that bound](hyp:hπvc), [the corresponding real-valued
zero-one indicator class has the same pseudo-dimension bound](goal). -/
...
/-- If [a real-valued class has a polynomial covering certificate](hyp:hF),
[each set selected by an indicator family is measurable](hyp:hπmeas), and
[the indicator family has bounded VC dimension](hyp:hπvc), [the products of
the real-valued functions and indicators retain a polynomial cover](goal). -/
