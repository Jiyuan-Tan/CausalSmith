...
/-- [A finite sample](hyp:S) with [positive size](hyp:hn) [has an empirical law
that is a probability measure](goal). -/
...
pseudo-dimension](hyp:hpdim), [has a positive envelope](hyp:hU),
[is evaluated at a positive relative radius](hyp:hε,hε1), and
[obeys the envelope pointwise](hyp:henvelope), then [any finite sample](hyp:S)
of [positive size](hyp:hn) [inherits the stated VC-subgraph covering bound](goal). -/
...
/-- If [a class consists of measurable functions](hyp:hmeas), [has bounded
pseudo-dimension](hyp:hpdim), [has a positive envelope](hyp:hU), and
[obeys the envelope pointwise](hyp:henvelope), then [its restriction to any
finite sample](hyp:S) of [positive size](hyp:hn) [is totally bounded](goal). -/
...
pseudo-dimension](hyp:hpdim), [has a positive envelope](hyp:hU),
[is evaluated at a positive relative radius](hyp:hε,hε1), and
[obeys the envelope pointwise](hyp:henvelope), then [on any finite
sample](hyp:S) of [positive size](hyp:hn), [its empirical covering number has
the stated polynomial bound](goal). -/
