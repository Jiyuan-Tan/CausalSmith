...
/-- [Negating every member of a polynomial-cover class preserves its envelope
and uniform polynomial covering certificate](goal), provided that
[the original class has such a certificate](hyp:hF). -/
...
/-- If [each of two independently indexed classes has a polynomial covering
certificate](hyp:hF,hG), [their pointwise sums have a polynomial covering
certificate with the sum of the envelopes](goal). -/
...
/-- If [each of two independently indexed classes has a polynomial covering
certificate](hyp:hF,hG), [their pointwise products have a polynomial covering
certificate with the product of the envelopes](goal). -/
...
/-- If [every class in a nonempty finite family has a polynomial covering
certificate](hyp:hF), [the class of pointwise sums has a polynomial covering
certificate with the sum of the envelopes](goal). -/
...
/-- If [every class in a nonempty finite family has a polynomial covering
certificate](hyp:hF), [the class of pointwise products has a polynomial covering
certificate with the product of the envelopes](goal). -/
...
/-- A polynomial covering certificate records that [the class has the ordinary
covering property](hyp:toHasPolynomialL2Cover), [the named entropy base is at
least one](hyp:one_le_base), and [that base and the named exponent give the
covering bound for every probability law and relative radius](hyp:entropy).
...
/-- [A named polynomial-cover certificate](hyp:hF) [implies the corresponding
ordinary polynomial-cover certificate](goal). -/
...
/-- If [a class has bounded pseudo-dimension](hyp:hpdim),
[its functions are measurable](hyp:hmeas), and [a positive envelope](hyp:hU)
[bounds the class pointwise](hyp:henvelope), then
[the class has explicit named polynomial-cover witnesses](goal). -/
...
/-- [Named polynomial-cover witnesses for a class](hyp:hF) [also witness the
same bound for its pointwise negation](goal). -/
...
/-- [Named polynomial-cover witnesses for two classes](hyp:hF,hG) [combine into
explicit witnesses for their pointwise sums](goal). -/
...
/-- [Named polynomial-cover witnesses for two classes](hyp:hF,hG) [combine into
explicit witnesses for their pointwise products](goal). -/
...
/-- If [a class has named witnesses at an inflated envelope](hyp:hF),
[the inflation factor is at least one](hyp:hc), and [the smaller envelope is
positive](hyp:hV) and [directly bounds the class](hyp:hbound), [the envelope can
be tightened by charging the inflation factor to the entropy base](goal). -/
...
/-- [Uniform named polynomial-cover witnesses](hyp:hF) [remain valid after
pointwise negation](goal). -/
...
/-- [Uniform named polynomial-cover witnesses for two families](hyp:hF,hG)
[combine into uniform witnesses for their pointwise sums](goal). -/
...
/-- [Uniform named polynomial-cover witnesses for two families](hyp:hF,hG)
[combine into uniform witnesses for their pointwise products](goal). -/
...
/-- If [every summand family has uniform named polynomial-cover
witnesses](hyp:hF), [their nonempty finite pointwise sum has uniform witnesses
with the summed envelopes](goal). -/
