...
/-- In [a Euclidean space of dimension `d`](hyp:d), [variable-radius closed-ball
tests indexed by their center have VC dimension at most `d + 2`](goal). -/
...
/-- In [a Euclidean space of dimension `d`](hyp:d), [variable-radius strict
exterior tests indexed by their center have VC dimension at most `d + 2`](goal). -/
...
/-- In [a Euclidean space of dimension `d`](hyp:d), [closed-ball classifiers
have VC dimension at most `d + 2`](goal). -/
...
/-- Given [a natural dimension](hyp:d), [a bandwidth](hyp:q),
[two relative annulus endpoints](hyp:a,b), [a natural-number degree](hyp:k),
[a center point](hyp:x), and [an evaluation point](hyp:z),
[the radial annulus monomial](goal) equals $(\operatorname{dist}(z,x)/q)^k$
when the distance to the center lies between $aq$ and $bq$, and is zero otherwise. -/
...
[a center-and-degree parameter](hyp:θ), and
[a Euclidean evaluation point](hyp:z), [the radial-monomial class](goal) maps
that parameter and point to the corresponding radial annulus monomial. -/
