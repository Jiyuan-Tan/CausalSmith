...
include heL heR in
/-- **Gaussian residual-projection input for the GMM J test.** [Under a standard
Gaussian random vector on the moment space, the distribution of the whitened GMM
residual-maker's squared norm is chi-squared with `finrank F − finrank E = m − k`
degrees of freedom](goal).  The full feasible-sample Hansen–Sargan theorem is
`hansenJ_tendsto_chiSq`. -/
theorem gaussian_residualProjection_chiSq :
