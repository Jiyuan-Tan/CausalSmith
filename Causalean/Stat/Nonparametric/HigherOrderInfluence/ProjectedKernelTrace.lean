...
/-!
# Projected-kernel L²-energy equals the projection dimension

Trace and centering identities for the projected kernels used by higher-order influence
function (HOIF) estimators, showing that a rank-`J` projection kernel has L² energy equal to
the projection dimension under inverse-Gram weighting.

The rank-2 projection kernel associated with the `J`-dimensional projection is
...
With the inverse-Gram weighting `M = Σ⁻¹` this collapses to **`ζ = J`** — the projection
kernel's L²-energy is exactly the projection dimension. In applications where degeneracy is
established separately, this identity can be paired with the bandwidth scaling hypothesis
`ζ ≤ C·J/h²` used by
`Causalean.Stat.Nonparametric.HigherOrderInfluence.DegenerateUStatVariance`
(`Var = 2ζ/(n(n−1))`) as an input to a projection-kernel risk bound.
...
With the inverse-Gram weighting `M = Σ⁻¹` (`Σ = gram c P` invertible), the expanded L²-energy equals the
...
/-- **Projection-kernel L²-energy equals the dimension: `ζ = J`.** For a feature
...
inverse of that Gram matrix](hyp:hM), then [the squared `L²(P⊗P)`-norm of the
...
second-moment matrix). When degeneracy and the other kernel conditions are established
separately, it supplies the L²-energy input to a degenerate U-statistic variance bound. -/
