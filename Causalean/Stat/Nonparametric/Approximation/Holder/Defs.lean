...
space](hyp:x0), and [a real radius](hyp:r), the [closed coordinatewise neighbourhood](goal) is the
finite-dimensional specialization of the shared sup-norm ball from the monomial-Gram layer. -/
abbrev supBall {d : ℕ} (x0 : Fin d → ℝ) (r : ℝ) : Set (Fin d → ℝ) :=
  Causalean.Mathlib.Analysis.supBall x0 r
...
gives an empty cube and zero volume when `d > 0`; when `d = 0`, the cube is the singleton
whole space and both sides equal one. -/
