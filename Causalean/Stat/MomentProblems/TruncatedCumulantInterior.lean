...
Background references: Akhiezer (1965), *The Classical Moment Problem*; Curto–Fialkow (1991),
*Recursiveness, Positivity, and Truncated Moment Problems*; and Schmüdgen (2017), *The Moment
Problem*, Chapter 10. The result below is the separate constructive centered-cumulant
specialization described above; it does not use a flat-extension theorem.
-/
...
def TruncatedCumulantInterior (L : ℕ) : Prop :=
  ∃ (c : ℕ → ℝ) (ε : ℝ), 0 < ε ∧
...
private theorem truncatedCumulantInterior_zero : TruncatedCumulantInterior 0 := by
...
theorem truncatedCumulantInterior (L : ℕ) : TruncatedCumulantInterior L := by
  by_cases hL : L = 0
  · simpa [hL] using truncatedCumulantInterior_zero
