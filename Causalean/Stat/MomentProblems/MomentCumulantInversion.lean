...
Cumulants of positive order are polynomials in the moments, and the polynomial of order `r`
involves the moment of order `r` linearly (with coefficient one) plus a remainder built only from
strictly lower moments. The order-zero cumulant is set to zero. Triangularity at positive orders
means the change of coordinates inverts: prescribing a cumulant sequence determines a moment
sequence, recursively and uniquely, among sequences normalized to have total mass one and mean
zero.
...
that moment sequence](goal) is zero at order zero; at positive orders it is the signed,
factorial-weighted sum over all partitions of a set of that many elements, with each partition
contributing the product of the moments indexed by its block sizes. -/
...
/-- For [a real moment sequence](hyp:m), [its order-zero combinatorial cumulant is zero](goal). -/
@[simp]
theorem cumFromMom_zero (m : ℕ → ℝ) : cumFromMom 0 m = 0 := by
  simp [cumFromMom]
...
theorem sourceCumulant_eq_cumFromMom (ν : Measure ℝ) (r : ℕ) :
  ...
  simp only [Nat.add_zero, if_neg hr]
...
theorem cumFromMom_eq (r : ℕ) (hr : 1 ≤ r) (m : ℕ → ℝ) :
  ...
  have hfiber : Finset.univ.filter
    ...
    · rintro rfl
      simp [P₀]
  rw [cumFromMom, if_neg (by omega), ← Finset.sum_filter_add_sum_filter_not
...
theorem cumFromMom_congr (r : ℕ) {m m' : ℕ → ℝ}
  ...
  by_cases hr : r = 0
  · simp [hr]
  rw [cumFromMom, if_neg hr, cumFromMom, if_neg hr]
  apply Finset.sum_congr rfl
  intro π _
  congr 1
  apply Finset.prod_congr rfl
