...
This file solves the optimization problem that uses the projection residual from
`ResidualQuadratic.ProjectionResidual`. For a probability measure `μ` on `ℝ` with finite fourth
moment and positive variance, consider the **constrained minimum-norm score program**: among all
scores
