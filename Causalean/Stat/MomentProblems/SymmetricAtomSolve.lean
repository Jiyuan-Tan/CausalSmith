...
/-- For [a nonnegative integer L](hyp:L) and [a finite coordinate vector whose entries at orders
two through L are prescribed cumulants](hyp:y), the [zero-padded coordinate sequence](goal) agrees
with that vector through order L and equals zero at every higher order. Entries zero and one are
normalization placeholders and are ignored by the centered moment reconstruction. -/
...
/-- For [a nonnegative integer L](hyp:L) and [a coordinate vector prescribing cumulants at orders
two through L](hyp:y), the [cumulant-to-weights map](goal) reconstructs the centered raw moments
and returns the symmetric-atom weights solving their Vandermonde moment-matching system. The
entries at orders zero and one are ignored because the reconstructed mass and mean are fixed to
one and zero. -/
