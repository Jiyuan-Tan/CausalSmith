...
[a measurable two-prefix statistic](hyp:hT), [a certificate that the statistic stays in an
interval](hyp:hTmem), and [a fallback in that interval](hyp:hzOver), [conditional averaging
cannot increase squared risk](goal), for any target. -/
    ...
    sqRisk (fixedPoolsLaw P Q NX NY)
      ...
      sqRisk μ (cappedPrefixStatistic T zOver) theta := by
        exact sqRisk_independentPrefixRaoBlackwellStatistic_le_capped
          P Q lambdaX lambdaY NX NY hT hTmem hzOver
