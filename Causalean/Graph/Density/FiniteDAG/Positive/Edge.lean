    ...
    { factor := M.toFactorization.factor
      ...
      local_factor := by
        ...
        · have hxyi : ∀ l, l ∈ insert i (H.parents i) → x l = y l := by
            intro l hl
            apply hxy l
            have hl' : l ∈ (↑(insert i (H.parents i)) : Set V) := hl
            ...
            calc
              M.toFactorization.factor i x =
                ...
                (ENNReal.toReal_eq_toReal_iff'
                  (M.factor_ne_top i x)
                  (M.factor_ne_top i (Function.update x j (y j)))).mp
                    (hind x (y j)).symm
...
/-- For an edge `j → i` in a uniformly positive continuous DAG density on compact coordinates,
conditional independence of `i` and `j` given the other parents is equivalent to
reference-almost-everywhere independence of the child's local factor from `j`. -/
...
/-- A [uniformly positive continuous factorization on compact coordinates](hyp:M) and
[directed edge](hyp:hji) satisfy [the edge conditional-independence characterization](goal): the
endpoints are conditionally independent given the child's other parents exactly when changing the
parent coordinate leaves the child's local factor unchanged. -/
