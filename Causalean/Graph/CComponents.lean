...
The executable reachability side is `bidirectedBFS` and `cComponentOf`, while
`cComponentSet` is the canonical order-independent component collection. The
...
/-- [Bidirected reachability](goal) in [a single-world intervention graph](hyp:G) on
    [a finite base-variable population](hyp:N) [relates each observed node to
    itself](hyp:refl) and [extends a reachable chain by one direct-confounding
    link](hyp:step). Thus two observed nodes are related exactly when a finite chain of shared
    latent-parent links joins them, including a chain of length zero.

    Bidirected reachability is the reflexive-transitive closure of the
    directly-confounded relation, restricted to observed variables. Two observed
    variables are in the same C-component iff they are bidirected-reachable from
    each other. -/
...
/-- For [a single-world intervention graph](hyp:G), the [canonical set of c-components](goal) is the set of c-components associated with its observed nodes, with duplicate components identified.
    ...
    **The set of c-components** as a `Finset` of `Finset`s — the canonical,
    ...
    collapse). Used as an index set in genuine c-component density factorization results. -/
