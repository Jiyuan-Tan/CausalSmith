...
/-! # L²-ball linear predictors — squared-loss excess-risk rate

The genuine **squared-loss** statistical rate for empirical risk minimization over an
`L²`-norm-bounded class of linear predictors `a ↦ ⟪w, a⟫`. Unlike the
bare-linear-functional version, the loss here is the regression loss `(⟪w, x⟩ − y)²`.
...
the `O(1/√n)` squared-loss excess-risk rate `l2Ball_erm_squaredLoss_excess_rate`.
-/
...
/-- [The bounded feature--response space](goal) restricts
[features and responses to their closed balls](step:1). Its [feature dimension](hyp:d),
[feature-radius bound](hyp:Xb), and [response-radius bound](hyp:Yb) specify the learning domain. -/
abbrev L2BallFeat (d : ℕ) (Xb Yb : ℝ) : Type :=
  Metric.closedBall (0 : EuclideanSpace ℝ (Fin d)) Xb × Metric.closedBall (0 : ℝ) Yb

/-- [The constrained weight space](goal) is [the centered closed Euclidean ball](step:1), in
[the chosen feature dimension](hyp:d) with [weight-radius bound](hyp:W). -/
abbrev L2BallWeight (d : ℕ) (W : ℝ) : Type :=
  Metric.closedBall (0 : EuclideanSpace ℝ (Fin d)) W

/-- **L²-ball linear ERM squared-loss excess-risk rate.** For data pairing
...
theorem l2Ball_erm_squaredLoss_excess_rate {d n : ℕ} {Ω : Type*} [MeasurableSpace Ω]
  ...
  set f : L2BallWeight d W → L2BallFeat d Xb Yb → ℝ :=
    fun w a => φ (p w a) - 2 * (a.2 : ℝ) * p w a with hf
  set sqLoss : L2BallWeight d W → L2BallFeat d Xb Yb → ℝ :=
    fun w a => (p w a - (a.2 : ℝ)) ^ 2 with hsq
  have hXbW : 0 ≤ Xb * W := mul_nonneg hXb hW
  -- pointwise bounds on the predictor
  have hpbound : ∀ (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb), |p w a| ≤ Xb * W := by
    intro w a
    ...
    calc |p w a| ≤ ‖(w : EuclideanSpace ℝ (Fin d))‖ * ‖(a.1 : EuclideanSpace ℝ (Fin d))‖ :=
      ...
      _ = Xb * W := by ring
  have hybound : ∀ a : L2BallFeat d Xb Yb, |(a.2 : ℝ)| ≤ Yb := by
    intro a
    simpa using (mem_closedBall_zero_iff.mp a.2.2)
  -- `f` is the centered squared loss: `sqLoss = f + y²`
  have hsqf : ∀ (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb), sqLoss w a = f w a + ((a.2 : ℝ)) ^ 2 := by
    intro w a
  ...
  have hfbound : ∀ (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb),
  ...
  have hRC : rademacherComplexity n f μ X ≤ Cf / Real.sqrt (n : ℝ) := by
    ...
    have hemp : ∀ S : Fin n → L2BallFeat d Xb Yb,
      ...
      set featCross : L2BallFeat d Xb Yb → EuclideanSpace ℝ (Fin d) :=
      ...
      have hcrossfun : (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => 2 * (a.2 : ℝ) * p w a)
      ...
      have hcrossS : empiricalRademacherComplexity n
          (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => 2 * (a.2 : ℝ) * p w a) S
      ...
      have hcrossbd : ∀ (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb),
          |2 * (a.2 : ℝ) * p w a| ≤ 2 * Yb * (Xb * W) := by
        intro w a
        rw [abs_mul, abs_mul, show |(2 : ℝ)| = 2 from by norm_num]
        nlinarith [hybound a, hpbound w a, abs_nonneg ((a.2 : ℝ)), abs_nonneg (p w a), hYb, hXbW]
      have hsub := empiricalRademacherComplexity_sub_le (ι := L2BallWeight d W)
        (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => φ (p w a))
        (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => 2 * (a.2 : ℝ) * p w a)
      ...
      have hfeq : empiricalRademacherComplexity n f S
          = empiricalRademacherComplexity n
              (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => φ (p w a) - 2 * (a.2 : ℝ) * p w a) S :=
        rfl
      rw [hfeq]
      calc empiricalRademacherComplexity n
            (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => φ (p w a) - 2 * (a.2 : ℝ) * p w a) S
            ≤ empiricalRademacherComplexity n
                (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => φ (p w a)) S
              + empiricalRademacherComplexity n
                (fun (w : L2BallWeight d W) (a : L2BallFeat d Xb Yb) => 2 * (a.2 : ℝ) * p w a) S := hsub
  ...
  have hsqcont_a : ∀ w : L2BallWeight d W, Continuous (fun a : L2BallFeat d Xb Yb => sqLoss w a) := by
    intro w
