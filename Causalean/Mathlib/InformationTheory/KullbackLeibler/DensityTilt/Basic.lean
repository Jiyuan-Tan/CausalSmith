...
For differentiability in quadratic mean and local asymptotic normality, see
van der Vaart, *Asymptotic Statistics*, Ch. 7. This file proves only the
elementary one-observation tilt facts used by the local KL expansion.
-/
...
/-- On a [measurable sample space](hyp:Z), given [a base measure](hyp:μ),
[a real-valued score function](hyp:s), and [a real tilt strength](hyp:h), the
[linear density tilt](goal) is the measure having density `max(1 + h s(z), 0)`
with respect to the base measure.

When `1 + h s` is nonnegative almost everywhere and the score has mean zero,
this is again a probability measure (see `isProbabilityMeasure_tiltMeasure`). -/
