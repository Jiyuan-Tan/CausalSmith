module
public import Causalean.Mathlib.Probability.Poisson.FinitePartition.Partition.Basic
...
/-- For [a finite label sequence](hyp:w) and [a label](hyp:j), the [histogram count](goal) is the
number of positions carrying that label. -/
noncomputable def wordHistogram {n : ℕ} (w : Fin n → ι) (j : ι) : ℕ := by
  classical
...
/-- For [a finite label sequence](hyp:w) and [a label](hyp:j), the [label-index set](goal)
contains exactly the positions carrying that label. -/
noncomputable def wordIndices {n : ℕ} (w : Fin n → ι) (j : ι) :
    Finset (Fin n) := by
  classical
  exact Finset.univ.filter fun k => w k = j

/-- For [a finite label sequence](hyp:w) and [a label](hyp:j),
[the number of its positions equals its histogram count](goal). -/
lemma wordIndices_card {n : ℕ} (w : Fin n → ι) (j : ι) :
    (wordIndices w j).card = wordHistogram w j := by
  rfl

/-- For [a finite label sequence](hyp:w) and [a label](hyp:j), the [ordered correspondence](goal)
matches occurrence numbers for that label with exactly the positions where it occurs. -/
noncomputable def wordFiberEquiv {n : ℕ} (w : Fin n → ι) (j : ι) :
...
/-- For [a finite label sequence](hyp:w), the [unshuffling correspondence](goal) matches all
label-specific occurrence slots with the original sample positions. -/
noncomputable def wordUnshuffleEquiv {n : ℕ} (w : Fin n → ι) :
...
/-- Given [an assignment of positions to cell indices](hyp:w) and [a tuple of points](hyp:z),
the [cell-wise regrouping](goal) assigns each cell the points with its label in their original
order. -/
...
/-- For [a finite cell-label sequence](hyp:w),
[regrouping a tuple of points by those labels is measurable](goal). -/
@[fun_prop]
...
/-- Given [a count for every cell](hyp:c) and [a matching point tuple for every cell](hyp:z), the
[fixed partition embedding](goal) assigns each cell the finite sample with that count and tuple. -/
noncomputable def fixedPartitionEmbed {Y : Type*} [MeasurableSpace Y]
    (c : ι → ℕ) (z : ∀ j, Fin (c j) → Y) :
    ι → FiniteSample Y :=
  fun j => fixedSizeEmbed (c j) (z j)

/-- For [a prescribed count in every cell](hyp:c),
[embedding one tuple per cell as finite samples is measurable](goal). -/
@[fun_prop]
...
/-- **Partition splitting.** For [a finite measurable partition](hyp:p),
[an observation probability law](hyp:P), [a real-valued mark law](hyp:R), and
[a nonnegative intensity](hyp:lam),
[cell restrictions are independent marked Poisson samples with mass-scaled intensities](goal). -/
