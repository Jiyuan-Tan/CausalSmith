...
/-- For [an observation probability law](hyp:P), [an atomless real-valued mark law](hyp:R),
[a nonnegative Poisson intensity](hyp:lam), [a fallback observation](hyp:x₀), and
[a retained sample size](hyp:n),
[retaining enough smallest-mark points gives the event mass times the product law](goal). -/
...
/-- **Conditioning on enough points gives i.i.d. draws.** For
[an observation probability law](hyp:P), [an atomless real-valued mark law](hyp:R),
[a nonnegative intensity](hyp:lam), [a fallback observation](hyp:x₀), and
[a retained sample size](hyp:n), suppose [the enough-points event has positive mass](hyp:hpos).
[The conditional retained-observation law is the corresponding product law](goal). -/
