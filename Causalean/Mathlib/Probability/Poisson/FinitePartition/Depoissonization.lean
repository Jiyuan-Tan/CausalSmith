...
/-- For [a measurable partition](hyp:p), [two probability laws](hyp:μ,ν), and
[a selected cell](hyp:j), if [the first law gives the cell positive mass](hyp:hpos),
[the laws give it equal mass](hyp:hmass), and [their restrictions to it agree](hyp:hrest),
[their normalized within-cell laws agree](goal). -/
...
/-- For [an observation probability law](hyp:P), [an atomless mark law](hyp:R),
[a nonnegative intensity](hyp:lam), [a fallback observation](hyp:x₀), and
[a prefix length](hyp:n),
[the successful canonical prefix has the event mass times the product law](goal). -/
...
/-- [Finite samples form a standard Borel space](goal) whenever
[the observation space is nonempty and standard Borel](hyp:X).

The padded-stream presentation supplies the compatible Polish topology missing from the generic
dependent-sum instance. -/
