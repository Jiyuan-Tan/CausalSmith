...
theorem condIndepFun_threeBlock_of_density_factors
  ...
  /-
  A direct route is to compute the conditioning marginal and the two
  random-block-with-conditioning marginals by Tonelli, prove the cross-multiplied
  density identity, and use Mathlib's conditional-expectation characterization of
  `CondIndepFun`. The empty `Y` and empty `Z` cases must be discharged separately;
  no positivity or nonemptiness assumption is valid here.
  -/
  classical
