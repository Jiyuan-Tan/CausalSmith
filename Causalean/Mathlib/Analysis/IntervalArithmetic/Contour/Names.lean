...
This module represents complex values by nested rational rectangles equipped
with supplied refinement rules and derives certified modulus names from them.
-/
...
value](hyp:contains), together with [a supplied rule selecting, for any requested positive
...
structure CertifiedComplex where
  ...
  /-- The fuel selected by the supplied rule for a requested positive rational width. -/
  modulus : PosRat → ℕ
  /-- The selected rectangle meets the requested coordinate-width tolerance. -/
  width_modulus : ∀ ε, ComplexRatInterval.width (approx (modulus ε)) ≤ ε.1
...
/-- Refinement by the supplied rule preserves containment and meets its requested width. -/
theorem refine_spec (z : CertifiedComplex) (ε : PosRat) :
    (z.refine ε).Contains z.value ∧ ComplexRatInterval.width (z.refine ε) ≤ ε.1 := by
...
def add (z w : CertifiedComplex) : CertifiedComplex where
  ...
  width_modulus := by
    ...
    have hz : ComplexRatInterval.width (z.approx k) ≤ δ.1 :=
      (rectangle_width_mono (approx_mono z (le_max_left _ _))).trans (z.width_modulus δ)
    have hw : ComplexRatInterval.width (w.approx k) ≤ δ.1 :=
...
def normApprox (z : CertifiedComplex) : ℕ → RatInterval
  | 0 => ComplexRatInterval.normInterval (z.approx 0) 0
  | fuel + 1 => (normApprox z fuel).tighten
      (ComplexRatInterval.normInterval (z.approx (fuel + 1)) (fuel + 1))
...
def normPrecision (z : CertifiedComplex) (ε : PosRat) : ℕ :=
  let M := ComplexRatInterval.maxAbs (z.approx 0)
  let δ : PosRat :=
    ⟨ε.1 ^ 2 / (16 * (M + 1)), by
      have hM : 0 ≤ M := by
...
private theorem normApprox_subinterval_current (z : CertifiedComplex) (fuel : ℕ) :
    (normApprox z fuel).Subinterval
      (ComplexRatInterval.normInterval (z.approx fuel) fuel) := by
  cases fuel with
...
theorem norm_width_at_precision (z : CertifiedComplex) (ε : PosRat) :
  ...
  let K := z.approx k
  let K0 := z.approx 0
  let D := ComplexRatInterval.normSq K
  have hKK0 : K.Subinterval K0 := approx_mono z (Nat.zero_le k)
  have hKw : ComplexRatInterval.width K ≤ δ.1 :=
  ...
  have hsqrtspan : (Real.sqrt (D.hi : ℝ) - Real.sqrt (D.lo : ℝ)) ≤ (ε.1 : ℝ) / 2 := by
    ...
    calc
      ...
      _ ≤ (ε.1 : ℝ) ^ 2 / 4 := hw
      _ = ((ε.1 : ℝ) / 2) ^ 2 := by ring
  have hcurrent : (ComplexRatInterval.normInterval K k).width ≤ ε.1 := by
