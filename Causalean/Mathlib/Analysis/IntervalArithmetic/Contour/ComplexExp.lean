...
def complexExp (I : ComplexRatInterval) : ℕ → ComplexRatInterval
  | 0 => complexExpRaw I 0
  | fuel + 1 =>
      ComplexRatInterval.tighten (complexExp I fuel) (complexExpRaw I (fuel + 1))
...
theorem complexExp_width (I : ComplexRatInterval) (fuel : ℕ) :
    ComplexRatInterval.width (complexExp I fuel) ≤
  ...
  have hsub : (complexExp I fuel).Subinterval (complexExpRaw I fuel) := by
    ...
    | succ fuel =>
        ...
        exact ⟨
          ...
          Transcendental.tighten_subinterval_right
            hprev.2 hraw.2⟩
  have hwidth : ComplexRatInterval.width (complexExp I fuel) ≤
      ComplexRatInterval.width (complexExpRaw I fuel) :=
    max_le_max (RatInterval.width_mono hsub.1) (RatInterval.width_mono hsub.2)
  have hraw_eq : complexExpRaw I fuel = ComplexRatInterval.mul A T := by
...
def complexExpNameApprox (z : CertifiedComplex) : ℕ → ComplexRatInterval
  ...
  | n + 1 =>
      ComplexRatInterval.tighten (complexExpNameApprox z n) (complexExpStage z (n + 1))
...
private theorem expScalar_subinterval_of_le (q : ℚ) {m k : ℕ} (hmk : m ≤ k) :
  ...
  | succ k hmk ih =>
      exact RatInterval.subinterval_trans
        (Transcendental.expScalar_nested
          q k) ih
...
theorem complexExpStage_width (z : CertifiedComplex) (n : ℕ) :
    ComplexRatInterval.width (complexExpStage z n) ≤
      (complexExpStageTolerance n).1 := by
  ...
  have hKw : ComplexRatInterval.width K ≤ τ / (4 * B) := by
  ...
  have hout' : ComplexRatInterval.width (complexExp K N) ≤
...
theorem complexExp_width_at_precision (z : CertifiedComplex) (ε : PosRat) :
    ComplexRatInterval.width
      (complexExpNameApprox z (complexExpPrecision z ε)) ≤ ε.1 := by
  ...
  have hmono : ComplexRatInterval.width (complexExpNameApprox z N) ≤
      ComplexRatInterval.width (complexExpStage z N) :=
    max_le_max (RatInterval.width_mono hsub.1) (RatInterval.width_mono hsub.2)
