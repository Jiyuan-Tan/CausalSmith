...
This module defines executable Machin/Taylor enclosures for π, sine and
cosine; real interval exponential evaluation lives in
`Causalean.Mathlib.Analysis.IntervalArithmetic.Contour.IntervalExp`. Raw
rational bounds are recursively intersected, so adjacent-fuel nesting follows
from finite intersection rather than from an order asserted between unrelated
tolerance parameters. Complex exponential composition and its
magnitude-sensitive name semantics live in
`Causalean.Mathlib.Analysis.IntervalArithmetic.Contour.ComplexExp`.
-/
...
private theorem piInterval_subinterval_raw (fuel : ℕ) :
  ...
  | succ fuel =>
      rw [piInterval]
      exact Transcendental.tighten_subinterval_right
        (piInterval_sound fuel) (piRaw_sound (fuel + 1))
