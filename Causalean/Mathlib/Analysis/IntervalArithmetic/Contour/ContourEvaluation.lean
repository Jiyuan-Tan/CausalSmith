  ...
  calc
    ...
    _ = ∫ u in (0 : ℝ)..1, program.normalizedIntegrand schedule u := by
      ...
      simp only [CircleMesh.circleIntegrand, CircleMesh.circleMap,
        circleMap_zero, CircleMesh.circleTangent, ContourProgram.normalizedIntegrand]
...
theorem integrandNodes_width_le_propagation (program : ContourProgram)
    ...
    ComplexRatInterval.width
        (program.integrandNodes scheduled.schedule certificate k) ≤
  ...
  have hnodeWidth : ComplexRatInterval.width node ≤ scheduled.target.1 := by
  ...
  have hnumWidth : ComplexRatInterval.width numerator ≤ numeratorWidth := by
    calc
      ComplexRatInterval.width numerator ≤
          program.numerator.algorithmError schedule.fuel +
            program.numerator.amplification * ComplexRatInterval.width node :=
  ...
  have hdenWidth : ComplexRatInterval.width denominator ≤ denominatorWidth := by
    calc
      ComplexRatInterval.width denominator ≤
          program.denominator.algorithmError schedule.fuel +
            program.denominator.amplification * ComplexRatInterval.width node :=
  ...
  have hnumMax : ComplexRatInterval.maxAbs numerator ≤ numeratorMax := by
    exact (ComplexRatInterval.maxAbs_le_of_contains_width hnumContains
      hnumValueBound hnumWidth)
  have hdenMax : ComplexRatInterval.maxAbs denominator ≤ denominatorMax := by
  ...
  have hnumDenMax :
      ComplexRatInterval.maxAbs
        (ComplexRatInterval.mul numerator (ComplexRatInterval.conj denominator)) ≤
          divisionMax := by
    calc
      ComplexRatInterval.maxAbs
          (ComplexRatInterval.mul numerator (ComplexRatInterval.conj denominator)) ≤
            2 * ComplexRatInterval.maxAbs numerator *
              ComplexRatInterval.maxAbs (ComplexRatInterval.conj denominator) :=
        ComplexRatInterval.mul_maxAbs numerator (ComplexRatInterval.conj denominator)
      _ = 2 * ComplexRatInterval.maxAbs numerator *
          ComplexRatInterval.maxAbs denominator := by
  ...
  have hnumDenWidth :
    ...
    calc
      ComplexRatInterval.width
          (ComplexRatInterval.mul numerator (ComplexRatInterval.conj denominator)) ≤
            2 * (ComplexRatInterval.maxAbs numerator *
                ComplexRatInterval.width (ComplexRatInterval.conj denominator) +
              ComplexRatInterval.maxAbs (ComplexRatInterval.conj denominator) *
                ComplexRatInterval.width numerator) :=
        ComplexRatInterval.mul_width numerator (ComplexRatInterval.conj denominator)
      _ = 2 * (ComplexRatInterval.maxAbs numerator *
            ComplexRatInterval.width denominator +
          ComplexRatInterval.maxAbs denominator * ComplexRatInterval.width numerator) := by
      ...
      _ ≤ 2 * (numeratorMax * denominatorWidth +
        ...
        have hnumA0 : 0 ≤ ComplexRatInterval.maxAbs numerator :=
          (abs_nonneg numerator.re.lo).trans
          ((le_max_left _ _).trans (le_max_left _ _))
        have hdenA0 : 0 ≤ ComplexRatInterval.maxAbs denominator :=
          (abs_nonneg denominator.re.lo).trans
          ((le_max_left _ _).trans (le_max_left _ _))
        nlinarith [mul_le_mul hnumMax hdenWidth
            (show 0 ≤ ComplexRatInterval.width denominator from
              (RatInterval.width_nonneg denominator.re).trans (le_max_left _ _)) hnMax0,
          mul_le_mul hdenMax hnumWidth
            (show 0 ≤ ComplexRatInterval.width numerator from
              (RatInterval.width_nonneg numerator.re).trans (le_max_left _ _)) hdMax0]
      _ = divisionWidth := rfl
  have hnormWidth : (ComplexRatInterval.normSq denominator).width ≤ normWidth := by
    calc
      (ComplexRatInterval.normSq denominator).width ≤
          4 * ComplexRatInterval.maxAbs denominator *
            ComplexRatInterval.width denominator :=
      ...
      _ = normWidth := rfl
  have hsep : separation.1 ≤ (ComplexRatInterval.normSq denominator).lo :=
    hseparation.trans (certificate.lower k hk)
  have hquotientWidth :
      ComplexRatInterval.width
          (ComplexRatInterval.div numerator denominator (certificate.away k hk)) ≤
        quotientWidth := by
    calc
      _ ≤ ComplexRatInterval.maxAbs
              (ComplexRatInterval.mul numerator (ComplexRatInterval.conj denominator)) *
            (ComplexRatInterval.normSq denominator).width / separation.1 ^ 2 +
          ComplexRatInterval.width
              (ComplexRatInterval.mul numerator (ComplexRatInterval.conj denominator)) /
            separation.1 :=
      ...
      _ ≤ divisionMax * normWidth / separation.1 ^ 2 +
        ...
        have hproduct := mul_le_mul hnumDenMax hnormWidth
          (RatInterval.width_nonneg (ComplexRatInterval.normSq denominator))
          hdivisionMax0
        exact add_le_add
  ...
  have hnodeMax : ComplexRatInterval.maxAbs node ≤
      program.radius + scheduled.target.1 := by
  ...
  change ComplexRatInterval.width
      (ComplexRatInterval.mul
        (ComplexRatInterval.div numerator denominator (certificate.away k hk)) node) ≤ _
  calc
    ComplexRatInterval.width
        (ComplexRatInterval.mul
          (ComplexRatInterval.div numerator denominator (certificate.away k hk)) node) ≤
        2 * (ComplexRatInterval.maxAbs
              (ComplexRatInterval.div numerator denominator (certificate.away k hk)) *
            ComplexRatInterval.width node +
          ComplexRatInterval.maxAbs node * ComplexRatInterval.width
              (ComplexRatInterval.div numerator denominator (certificate.away k hk))) :=
    ...
    _ ≤ 2 * ((divisionMax / separation.1) * scheduled.target.1 +
      ...
      have hquotientA0 :
          0 ≤ ComplexRatInterval.maxAbs
            (ComplexRatInterval.div numerator denominator (certificate.away k hk)) :=
        (abs_nonneg
          (ComplexRatInterval.div numerator denominator (certificate.away k hk)).re.lo).trans
          ((le_max_left _ _).trans (le_max_left _ _))
      have hnodeA0 : 0 ≤ ComplexRatInterval.maxAbs node :=
        (abs_nonneg node.re.lo).trans
        ((le_max_left _ _).trans (le_max_left _ _))
      nlinarith [mul_le_mul hquotientMax hnodeWidth
          (show 0 ≤ ComplexRatInterval.width node from
        ...
        mul_le_mul hnodeMax hquotientWidth
          (show 0 ≤ ComplexRatInterval.width
              (ComplexRatInterval.div numerator denominator (certificate.away k hk)) from
            (RatInterval.width_nonneg
              (ComplexRatInterval.div numerator denominator
                (certificate.away k hk)).re).trans
                (le_max_left _ _))
...
def ContourProgram.evaluate (program : ContourProgram) (schedule : Schedule)
  ...
  (CircleMesh.integralEnclosure
    ...
    schedule.lipschitzConstant schedule.lipschitzConstant_nonneg
      schedule.mesh schedule.mesh_pos).expand
...
theorem evaluate_contains (program : ContourProgram) (schedule : Schedule)
    ...
    (hLip : ∀ s ∈ Icc (0 : ℝ) 1, ∀ t ∈ Icc (0 : ℝ) 1,
      ‖program.normalizedIntegrand schedule s - program.normalizedIntegrand schedule t‖ ≤
        (schedule.lipschitzConstant : ℝ) * |s - t|) :
  ...
  exact CircleMesh.integralEnclosure_sound schedule.lipschitzConstant_nonneg schedule.mesh_pos
...
theorem evaluate_width (program : ContourProgram)
    ...
    ComplexRatInterval.width
        (program.evaluate scheduled.schedule certificate) ≤
...
theorem certified_contour_evaluation (program : ContourProgram)
    ...
    (hLip : ∀ s ∈ Icc (0 : ℝ) 1, ∀ t ∈ Icc (0 : ℝ) 1,
      ‖program.normalizedIntegrand scheduled.schedule s -
        ...
        (scheduled.schedule.lipschitzConstant : ℝ) * |s - t|) :
...
/-- [The evaluated contour rectangle has real-coordinate width at most the reciprocal of the
larger of an integer and one](goal) for [a contour program](hyp:program) with
[uniform value bounds](hyp:bounds) and [a positive separation target](hyp:separation), when
[its certified schedule](hyp:scheduled) [uses that reciprocal tolerance](hyp:hschedule).
Here [the integer determines the reciprocal rate](hyp:n), [a denominator certificate is
available](hyp:certificate) and [covers the target separation](hyp:hseparation),
[the exact denominator never vanishes on the circle](hyp:hden), and
[the normalized integrand obeys the scheduled Lipschitz bound](hyp:hLip). -/
