...
/-!
# Certified real names and width-controlled refinement

This module represents a real quantity by nested rational enclosures together
with a precision rule, which is an arbitrary function whose computability is
not encoded. It turns any positive rational error target into a concrete
interval that still contains the quantity and is no wider than that target.
-/
...
value](hyp:contains), together with [an arbitrary rule, not required to be computable, that
selects a precision level for any requested positive rational error](hyp:modulus) and
[guarantees that the selected enclosure is no wider than that error](hyp:width_modulus). -/
structure CertifiedReal where
  ...
  /-- An arbitrary, not necessarily computable rule selecting a precision for a requested width. -/
  modulus : PosRat → ℕ
