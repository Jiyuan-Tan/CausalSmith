...
public import Causalean.Mathlib.Analysis.IntervalArithmetic.Logarithm
...
def expIntervalLo (I : RatInterval) (n : ℕ) : ℚ :=
  (expScalar I.lo n).lo

/-- The upper endpoint of the certified exponential interval is obtained from the scalar enclosure
at the input interval's upper endpoint. It is used with the corresponding lower endpoint to give
a sound rational enclosure for the exponential function. -/
def expIntervalHi (I : RatInterval) (n : ℕ) : ℚ :=
  (expScalar I.hi n).hi

/-- For [a rational interval](hyp:I) and [a precision index](hyp:n), [the lower endpoint of the
certified exponential enclosure does not exceed its upper endpoint](goal). -/
theorem expInterval_wellFormed (I : RatInterval) (n : ℕ) :
...
theorem logInterval_wellFormed (I : RatInterval) (hI : 0 < I.lo) (n : ℕ) :
