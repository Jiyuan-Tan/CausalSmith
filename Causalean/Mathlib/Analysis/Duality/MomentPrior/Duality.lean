...
private theorem uniformApproxErrorAbs_eq_dual_norm (p : Polynomial ℝ) :
    uniformApproxErrorAbs p = ‖dualAbs - p.toContinuousMapOn symmUnitInterval‖ := by
  apply le_antisymm
  · apply uniformApproxErrorAbs_le_iff.mpr
    intro x hx
    have h := ContinuousMap.norm_coe_le_norm
      (dualAbs - p.toContinuousMapOn symmUnitInterval) ⟨x, by simpa [symmUnitInterval] using hx⟩
...
private theorem exists_dual_abs_certificate (K : ℕ) :
  ...
  have hnorm : ‖qabs‖ = bestUniformApproxErrorAbs K := by
    ...
    · obtain ⟨p, hp, herr⟩ := exists_bestPolynomialAbs K
      ...
      let y : C(symmUnitInterval, ℝ) := p.toContinuousMapOn symmUnitInterval
      have hy : y ∈ V := ⟨⟨p, hpLT⟩, rfl⟩
      calc
        Metric.infDist dualAbs (V : Set C(symmUnitInterval, ℝ)) ≤ dist dualAbs y :=
    ...
    · apply (Metric.le_infDist (show (V : Set C(symmUnitInterval, ℝ)).Nonempty from
        ⟨0, V.zero_mem⟩)).mpr
      intro y hy
      obtain ⟨p, hp⟩ := hy
      ...
      have hp' : (p : Polynomial ℝ).toContinuousMapOn symmUnitInterval = y := hp
      rw [hp'] at hbest
      simpa [dist_eq_norm] using hbest
  obtain ⟨g, hg, hgabs⟩ := exists_dual_vector'' ℝ qabs
  let L : C(symmUnitInterval, ℝ) →L[ℝ] ℝ := g.comp V.mkQL
  have hL : ‖L‖ ≤ 1 := by
  ...
  have hVzero (u : C(symmUnitInterval, ℝ)) (hu : u ∈ V) : L u = 0 := by
    change g (Submodule.Quotient.mk u) = 0
    have hq : Submodule.Quotient.mk u = (0 : C(symmUnitInterval, ℝ) ⧸ V) :=
...
private theorem symmPush_mass (μ : Measure symmUnitInterval) [IsFiniteMeasure μ]
    (hμ : μ Set.univ = (1 / 2 : ENNReal)) :
    symmPush μ Set.univ = (1 / 2 : ENNReal) := by
  have hm₁ : Measure.map (Subtype.val : symmUnitInterval → ℝ) μ Set.univ =
      (1 / 2 : ENNReal) := by
    rw [Measure.map_apply measurable_subtype_coe MeasurableSet.univ]
    exact hμ
  have hm₂ : Measure.map ((fun x : ℝ => -x) ∘
      (Subtype.val : symmUnitInterval → ℝ)) μ Set.univ =
      (1 / 2 : ENNReal) := by
...
private theorem symmPush_supported (μ : Measure symmUnitInterval) [IsFiniteMeasure μ] :
    IsSupportedOnUnitInterval (symmPush μ) := by
  have hset : MeasurableSet symmUnitIntervalᶜ := by
    simpa [symmUnitInterval] using (measurableSet_Icc : MeasurableSet (Icc (-1 : ℝ) 1)).compl
  simp only [IsSupportedOnUnitInterval, symmPush, Measure.smul_apply,
    hset, Measure.add_apply]
  have hpos : Measure.map (Subtype.val : symmUnitInterval → ℝ) μ symmUnitIntervalᶜ = 0 := by
    rw [Measure.map_apply_of_aemeasurable measurable_subtype_coe.aemeasurable
      hset]
    have hpre : (Subtype.val : symmUnitInterval → ℝ) ⁻¹' symmUnitIntervalᶜ = ∅ := by
      ext x
  ...
  have hneg : Measure.map ((fun x : ℝ => -x) ∘
      (Subtype.val : symmUnitInterval → ℝ)) μ symmUnitIntervalᶜ = 0 := by
    rw [Measure.map_apply_of_aemeasurable
      (measurable_neg.comp measurable_subtype_coe).aemeasurable
      hset]
    have hpre : ((fun x : ℝ => -x) ∘ (Subtype.val : symmUnitInterval → ℝ)) ⁻¹'
        symmUnitIntervalᶜ = ∅ := by
      ext x
      simp only [mem_preimage, Function.comp_apply, mem_compl_iff, mem_empty_iff_false,
        iff_false]
      simpa [symmUnitInterval] using
...
private theorem symmPush_symmetric (μ : Measure symmUnitInterval) [IsFiniteMeasure μ] :
    IsSymmetric (symmPush μ) := by
  simp only [IsSymmetric, symmPush, Measure.map_smul]
  rw [Measure.map_add _ _ measurable_neg]
  have hm₁ : Measure.map (fun x : ℝ => -x)
      (Measure.map (Subtype.val : symmUnitInterval → ℝ) μ) =
      Measure.map ((fun x : ℝ => -x) ∘ (Subtype.val : symmUnitInterval → ℝ)) μ := by
    rw [Measure.map_map measurable_neg measurable_subtype_coe]
  have hm₂ : Measure.map (fun x : ℝ => -x)
      (Measure.map ((fun x : ℝ => -x) ∘ (Subtype.val : symmUnitInterval → ℝ)) μ) =
      Measure.map (Subtype.val : symmUnitInterval → ℝ) μ := by
...
private theorem integrable_comp_unitInterval (μ : Measure symmUnitInterval) [IsFiniteMeasure μ]
    (f : ℝ → ℝ) (hf : Continuous f) (sgn : ℝ → ℝ) (hsgn : Continuous sgn) :
    Integrable (fun x : symmUnitInterval => f (sgn (x : ℝ))) μ := by
  let g : C(symmUnitInterval, ℝ) := ⟨fun x => f (sgn (x : ℝ)), by fun_prop⟩
  apply Integrable.of_bound g.continuous.aestronglyMeasurable ‖g‖
  filter_upwards [] with x
  exact ContinuousMap.norm_coe_le_norm g x

private theorem integral_symmPush (μ : Measure symmUnitInterval) [IsFiniteMeasure μ]
    (f : ℝ → ℝ) (hf : Continuous f) :
    ∫ x : ℝ, f x ∂symmPush μ =
      (1 / 2 : ℝ) *
        ((∫ x : symmUnitInterval, f (x : ℝ) ∂μ) +
          ∫ x : symmUnitInterval, f (-(x : ℝ)) ∂μ) := by
  have hi₁ : Integrable f (Measure.map (Subtype.val : symmUnitInterval → ℝ) μ) := by
  ...
  have hi₂ : Integrable f (Measure.map ((fun x : ℝ => -x) ∘
      (Subtype.val : symmUnitInterval → ℝ)) μ) := by
    apply (integrable_map_measure hf.aestronglyMeasurable
...
theorem exists_absExtremalDecomposition (K : ℕ) :
  ...
  let forgetC : CompactlySupportedContinuousMap symmUnitInterval ℝ →ₗ[ℝ]
      C(symmUnitInterval, ℝ) :=
    { toFun := fun f => f.toContinuousMap
      map_add' := by intro f g; ext x; rfl
      map_smul' := by intro c f; ext x; rfl }
  let Pc : CompactlySupportedContinuousMap symmUnitInterval ℝ →ₚ[ℝ] ℝ :=
    { toLinearMap := P.toLinearMap.comp forgetC
      monotone' := by
        intro f g hfg
        exact P.monotone' (fun x => hfg x) }
  let Nc : CompactlySupportedContinuousMap symmUnitInterval ℝ →ₚ[ℝ] ℝ :=
    { toLinearMap := N.toLinearMap.comp forgetC
      monotone' := by
        intro f g hfg
        exact N.monotone' (fun x => hfg x) }
  let μP : Measure symmUnitInterval := RealRMK.rieszMeasure Pc
  let μN : Measure symmUnitInterval := RealRMK.rieszMeasure Nc
  haveI : IsFiniteMeasure μP := by dsimp [μP]; infer_instance
  haveI : IsFiniteMeasure μN := by dsimp [μN]; infer_instance
  have hμP (c : C(symmUnitInterval, ℝ)) : ∫ x, c x ∂μP = P c := by
    ...
    calc
      ...
      _ = Pc cc := h
      _ = P c := rfl
  have hμN (c : C(symmUnitInterval, ℝ)) : ∫ x, c x ∂μN = N c := by
    let cc := CompactlySupportedContinuousMap.continuousMapEquiv c
  ...
  have hmassP : μP Set.univ = (1 / 2 : ENNReal) := by
    have h := hμP (1 : C(symmUnitInterval, ℝ))
    change (∫ _ : symmUnitInterval, (1 : ℝ) ∂μP) = P 1 at h
  ...
  have hmassN : μN Set.univ = (1 / 2 : ENNReal) := by
    have h := hμN (1 : C(symmUnitInterval, ℝ))
    change (∫ _ : symmUnitInterval, (1 : ℝ) ∂μN) = N 1 at h
    rw [integral_const] at h
  ...
  · intro j hj
    ...
    have h₂ := hPN rpow
    have hp₁ : (∫ x : symmUnitInterval, (x : ℝ) ^ j ∂μP) = P (dualPow j) :=
      hμP (dualPow j)
    have hp₂ : (∫ x : symmUnitInterval, (-(x : ℝ)) ^ j ∂μP) = P rpow :=
      hμP rpow
    have hn₁ : (∫ x : symmUnitInterval, (x : ℝ) ^ j ∂μN) = N (dualPow j) :=
      hμN (dualPow j)
    have hn₂ : (∫ x : symmUnitInterval, (-(x : ℝ)) ^ j ∂μN) = N rpow :=
      hμN rpow
    ...
    rw [hLrpow] at h₂
    linarith
  · let rabs : C(symmUnitInterval, ℝ) :=
      ⟨fun x => |-(x : ℝ)|, by fun_prop⟩
    have hrabs : rabs = dualAbs := by ext x; simp [rabs, dualAbs]
    have h₁ := hPN dualAbs
    have h₂ := hPN rabs
    have hp₁ : (∫ x : symmUnitInterval, |(x : ℝ)| ∂μP) = P dualAbs := hμP dualAbs
    have hp₂ : (∫ x : symmUnitInterval, |-(x : ℝ)| ∂μP) = P rabs := hμP rabs
    have hn₁ : (∫ x : symmUnitInterval, |(x : ℝ)| ∂μN) = N dualAbs := hμN dualAbs
    have hn₂ : (∫ x : symmUnitInterval, |-(x : ℝ)| ∂μN) = N rabs := hμN rabs
...
/-- For [any degree](hyp:K), [a symmetric moment-matched probability-prior pair with the
exact absolute-moment gap exists](goal).

The pair consists of symmetric Borel probability measures supported on `[-1,1]`, matching
all moments through that degree, whose absolute first moments differ by exactly twice the best
approximation error. -/
theorem exists_symmetric_momentMatched_absGap_for_degree (K : ℕ) :
    Nonempty (AbsMomentMatchedPriors K) := by
  obtain ⟨D⟩ := exists_absExtremalDecomposition K
  exact ⟨D.toMomentMatchedPriors⟩

/-- For [a degree](hyp:K), [a symmetric moment-matched probability-prior pair with the exact
absolute-moment gap exists](goal). -/
theorem exists_symmetric_momentMatched_absGap
    {K : ℕ} :
    Nonempty (AbsMomentMatchedPriors K) :=
  exists_symmetric_momentMatched_absGap_for_degree K
