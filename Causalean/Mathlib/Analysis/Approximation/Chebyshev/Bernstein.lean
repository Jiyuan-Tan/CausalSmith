...
/-- The [cosine-parametrized transform of a real polynomial](hyp:R) is
[continuous on the real line](goal). -/
@[fun_prop]
theorem czTrig_continuous (R : Polynomial ℝ) : Continuous (czTrig R) := by
  unfold czTrig
  fun_prop

/-- The [cosine-transform sup norm of a real polynomial](hyp:R) is
[attained at some angle between zero and π](goal). This is the extreme-value theorem applied to
the continuous absolute transform on that compact interval. -/
...
/-- The [cosine-transform sup norm of a real polynomial](hyp:R) is [nonnegative](goal). -/
theorem czSup_nonneg (R : Polynomial ℝ) : 0 ≤ czSup R := by
  rcases czSup_attained R with ⟨t₀, _ht₀, hsup⟩
  rw [← hsup]
  exact abs_nonneg _

/-- At [any angle between zero and π](hyp:t,ht), the absolute cosine-parametrized value of
[a real polynomial](hyp:R) is [bounded by its cosine-transform sup norm](goal). -/
...
/-- **Szegő's differential inequality.** The cosine-parametrized transform of [a real
polynomial](hyp:R) whose [degree is at most a chosen bound](hyp:β,hβ), evaluated at [any real
angle](hyp:t), has [derivative magnitude at most the degree bound times the square root of the
sup-norm squared minus the squared transform value](goal).
...
/-- The arccosine of the cosine transform of [a real polynomial](hyp:R) with [a chosen degree
bound](hyp:β,hβ), normalized by its sup norm plus [a positive regularizer](hyp:δ,hδ), is
evaluated at [any two specified angles in the interval from zero to π](hyp:s,t,hs,ht), with
[the difference of the resulting arccosine values bounded by the degree times their
distance](goal). -/
...
/-- **Arccos-Lipschitz reformulation of Szegő's inequality.** The normalized cosine transform of
[a real polynomial](hyp:R) with [a chosen degree bound](hyp:β,hβ) and [strictly positive sup
norm](hyp:hM) has [arccosine values at any two specified angles in the interval from zero to
π](hyp:s,t,hs,ht) whose [difference is bounded by the degree times the distance between those
angles](goal).
...
/-- **Maximizer node bound.** For [a real polynomial](hyp:R) with [a chosen degree
bound](hyp:β,hβ) and [strictly positive cosine-transform sup norm](hyp:hM), let [one angle in the
interval from zero to π](hyp:t₀,ht₀) be [a point where the transform attains that norm](hyp:hmax)
and [another angle in the same interval](hyp:s,hs) be [close enough that the degree times their
distance is at most π/2](hyp:hclose). Then [the transform at the second angle is at least the sup
norm times the cosine of the degree-scaled distance](goal).
