...
/-- For [a nonnegative integer degree bound](hyp:n) and [a real-valued function of a real
argument](hyp:f), [the assertion that the function is a real trigonometric polynomial of degree
at most the bound](goal) means [that it has a finite cosine-sine expansion through that
degree](step:1).
...
/-- The constant function with [a chosen real value](hyp:c) is, for [any specified nonnegative
degree bound](hyp:n), [a real trigonometric polynomial within that bound](goal); it uses only the
zero-frequency term. -/
...
/-- A [real trigonometric polynomial with one degree bound](hyp:n,f,hf) is, for [a second degree
bound](hyp:m), [also a trigonometric polynomial within that second bound](goal) whenever [the
first bound does not exceed the second](hyp:hnm); the additional coefficients can be set to zero. -/
...
/-- The pointwise difference of [two real trigonometric polynomials with a common degree
bound](hyp:n,f,g,hf,hg) is [a real trigonometric polynomial with the same bound](goal), obtained by
subtracting their coefficient sequences term by term. -/
...
/-- In [the half-open real interval of one full period beginning at a chosen point](hyp:c),
[two angles in that interval](hyp:s,t,hs,ht) [are equal](goal) if [their unit-circle complex
exponentials agree](hyp:h). -/
...
/-- **Zero-count lemma for real trigonometric polynomials.** If [a real-valued function is a
trigonometric polynomial with a chosen degree bound](hyp:n,f,hf) and [is not identically
zero](hyp:hne), then [any finite set in a specified half-open period whose points are all
zeros](hyp:c,S,hS,hzero) [has cardinality at most twice the degree bound](goal).
...
/-- **Multiplicity-refined zero-count lemma.** Suppose [a real-valued function is a
trigonometric polynomial with a chosen degree bound](hyp:n,f,hf) and [is not identically
zero](hyp:hne). If [a finite set in a specified half-open period consists only of
zeros](hyp:c,S,hS,hzero) and [a further point in that period but outside the
set](hyp:t₀,ht₀mem,ht₀S)
is [both a zero](hyp:hf0) and [a stationary point](hyp:hderiv), then [the size of the finite set
plus two for the double zero is at most twice the degree bound](goal).
