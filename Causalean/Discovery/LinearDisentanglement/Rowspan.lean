...
module
public import Causalean.Discovery.LinearDisentanglement.KeyIdentity
public import Causalean.Discovery.LinearDisentanglement.PartialOrderRQ
public import Mathlib.LinearAlgebra.Span.Basic

/-!
# Linear causal disentanglement: rowspan inclusions and the corrected third clause

This file links the *observable* precision differences to the *latent* graph
structure. By `key_identity`, `rowspan(Θₖ − Θ₀)` is spanned by
...
* an ancestor-closed correction to the paper's printed Lemma 1(c): if the intervention
  changes the precision matrix, `𝓘` is ancestor-closed, and `Pa(iₖ) ⊄ 𝓘`, then
  `rowspan(Θₖ − Θ₀) ⊄ ⟨qᵢ : i ∈ 𝓘⟩`.

The printed clause quantifies over arbitrary `𝓘`; the result formalized here deliberately
does not claim that unrestricted statement.
...
/-- [The row span](goal) captures every observable direction identified by the rows of [matrix
`M`](hyp:M) in [observed dimension `p`](hyp:p). -/
noncomputable def rowSpan (M : Matrix (Fin p) (Fin p) ℝ) : Submodule ℝ (Fin p → ℝ) :=
  Submodule.span ℝ (Set.range (fun i => M i))

/-- [The selected mixing-row span](goal) is the observable subspace generated by [latent index set
`I`](hyp:I) through [solution `S`](hyp:S), with [latent dimension `d`](hyp:d), [observed dimension
...
/-- [The selected RQ-row span](goal) is the observable subspace carried by [rows `I`](hyp:I) of
[matrix `Q`](hyp:Q), from [latent dimension `d`](hyp:d) into [observed dimension `p`](hyp:p). -/
...
/-- **Ancestor-closed correction to the paper's Lemma 1(c).** For [a solution
`S`](hyp:S), [an intervention `k`](hyp:k), and [a set of latent indices `I`](hyp:I),
given [a partial order RQ decomposition of `S`'s latent-direction
...
The formal result requires `I` to be **ancestor-closed** (`hIclosed`). This is a corrected
specialization of the paper's printed clause for arbitrary `I`; the unrestricted clause is
not claimed here. We split on whether `j ∈ ⋃_{i∈I} An(i)`:
