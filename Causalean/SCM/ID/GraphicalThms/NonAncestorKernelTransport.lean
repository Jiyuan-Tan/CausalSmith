...
/-! # Non-Ancestor Kernel Transport

This file transports observed conditional kernels between an induced SCM and a
fixed SCM under an ancestral-closure and Rule-3-style non-ancestry condition.
The results apply to an arbitrary observed node set; they neither assume that
the set is a c-component nor establish Tian's c-component factorization. The
file also provides intervention-target simplification.
...
/-- For [a finite node-label set](hyp:N), [measurable node-value spaces](hyp:Ω),
    ...
    [an intervention target set](hyp:Wn), [the non-ancestor marginal-transport
...
/-- For [a finite node-label set](hyp:N), [measurable node-value spaces](hyp:Ω),
    ...
    intervention target set](hyp:Wn), [the conditional-kernel transport conclusion](goal)
    ...
    The left side is the `condKernelFactor` of the induced sub-SCM on `R`, i.e. the
  ...
  ∀ (hR_ac : M.isAncestrallyClosedSCM R)
    ...
    ∀ [StandardBorelSpace (ValuesOn T (swigΩ Ω))]
      ...
      ∀ᵐ c ∂((MI.obsKernel sInduce).map
        ...
        (MI.condKernelFactor T hT_induce sInduce) c =
...
/-- The marginal non-ancestor transport identity.
...
/-- Equal pair laws give equal conditional-kernel slices a.e.
    ...
    This is the measure-theoretic uniqueness step used by
    `condKernel_transport_of_nonAncestor`:
...
/-- For [a structural causal model `M`](hyp:M), [an ancestrally closed node
    set `R`](hyp:R), [an observed node set `T` of the induced model](hyp:T), and
    [an intervention target set `Wn`](hyp:Wn), [the induced conditional-kernel
    factor agrees almost everywhere with the corresponding conditional kernel
    of the fixed model](goal), provided the stated Rule-3-style non-ancestor,
    finite-measure, standard-Borel, and countable-generation assumptions hold.
    No c-component assumption is present. -/
theorem condKernel_transport_of_nonAncestor
    (M : Causalean.SCM N Ω)
    (R : Finset (SWIGNode N))
    (T : Finset (SWIGNode N))
    (Wn : Finset N) :
...
-- ============================================================
-- Full-observed specialization
-- ============================================================

/-- For [a finite node-label set](hyp:N), [measurable node-value spaces](hyp:Ω),
    [a structural causal model](hyp:M), [a node set](hyp:T), and [an
    intervention target set](hyp:Wn), [the full-observed transport conclusion](goal)
    is the conditional-kernel transport conclusion obtained by taking the
    ancestral node set to be all observed nodes of the model. -/
def DistrictIdConclusion
    (M : Causalean.SCM N Ω) (T : Finset (SWIGNode N))
    (Wn : Finset N) : Prop :=
  QFactorIdentityConclusion M M.observed T Wn

/-- For [a structural causal model `M`](hyp:M), [an observed node set
    `T`](hyp:T), and [an intervention set `Wn`](hyp:Wn), [the fixed-model
    conditional kernel equals almost everywhere the corresponding kernel of the
    induced full-observed model](goal), under the non-ancestor assumptions of
    `QFactorIdentityConclusion`.

    This specializes `condKernel_transport_of_nonAncestor` at
    `R := M.observed`. -/
theorem condKernel_fixSet_identity_of_nonAncestor
    (M : Causalean.SCM N Ω)
    (T : Finset (SWIGNode N))
    (Wn : Finset N) :
    DistrictIdConclusion M T Wn :=
  condKernel_transport_of_nonAncestor M M.observed T Wn
