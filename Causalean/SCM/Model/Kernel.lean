...
map `M.evalMap` (see `Causalean/SCM/Model/Evaluation.lean`), with the fixed-value argument `s`
...
plumbing) have been factored out to:
  * `Causalean/SCM/Do/Rule2.lean` — Rule 2 + `fillZrW` helpers.
  * `Causalean/SCM/Do/Rule3.lean` — Rule 3* non-ancestor transport + compat bridge lemmas.
