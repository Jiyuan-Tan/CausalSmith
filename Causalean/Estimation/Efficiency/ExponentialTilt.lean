...
/-!
# Normalized exponential tilts

This module defines normalized exponential-tilt laws, proves that they are
probability measures dominated by the base law, and proves the derivative of
tilted expectations. `TiltTangent.lean` uses these analytic facts to establish
the DQM property required of regular submodels.

Reference: van der Vaart, *Asymptotic Statistics* (1998), §25.3;
Bickel--Klaassen--Ritov--Wellner (1993), §3.3.
-/
...
lemma tiltMeasure_eq_withDensity_div
    ...
    (hs_meas : Measurable s) (hsM : ∀ z, |s z| ≤ M) :
    tiltMeasure P s t = P.withDensity (fun z => ENNReal.ofReal
      (Real.exp (t * s z) / ∫ y, Real.exp (t * s y) ∂P)) := by
  have hc : 0 < tiltNorm P s t := tiltNorm_pos hs_meas hsM
  have he_meas : Measurable (fun z => ENNReal.ofReal (Real.exp (t * s z))) := by
    fun_prop
  rw [tiltMeasure, ← withDensity_smul
    (ENNReal.ofReal (tiltNorm P s t))⁻¹ he_meas]
  congr 1
  funext z
