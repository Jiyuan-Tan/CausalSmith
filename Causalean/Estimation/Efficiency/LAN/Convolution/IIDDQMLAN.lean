...
private theorem iid_sqrtLikelihoodIncrement_array_limits
  ...
  refine ⟨?_, ?_, ?_⟩
  · rw [tendstoInProbability_iff_real] at hlinear' ⊢
    intro ε hε
    exact (hlinear' ε hε).congr' (Filter.Eventually.of_forall fun n => by
      ...
      ring)
  · have hs := @iid_sum_sq_tendstoInProbability_of_lindeberg X _ P _ W
      (qmd.information M h h) hWmeas hW2
      (by simpa [P, W] using hmoments.1) hlindeberg
    rw [tendstoInProbability_iff_real] at hs ⊢
    simpa [P, W, DominatedIIDModel.iidLaw] using hs
...
/-- The generalized Radon--Nikodym log likelihood ratio of the finite product experiment has
