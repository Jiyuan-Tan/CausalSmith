...
# Structure-agnostic ATE lower bound under a common ancillary product

The structure-agnostic lower bound (`ChiSquaredCore.minimax_lower_bound`) is
stated for a *finite* covariate `Fin K × Bool`. This file tensors both testing
hypotheses with the same ancillary probability law. The ancillary observations
therefore carry no information for distinguishing the hypotheses.

The enlarged experiment is the finite-cell `n`-sample law tensored with `n`
independent draws from an arbitrary probability space `(W, ρ)`:
...
finite-cell indistinguishability `χ²(Qtrue‖Qfalse) ≤ 1` transfers verbatim to the product
experiment (`chiSqDiv_QtrueCont_QfalseCont_le_one`). The two-point method then
gives the identical `1/4` lower bound (`two_point_lower_bound_ancillary_product`) at the
same supplied two-point separation. Thus the extra ancillary information does not
improve this finite-sample testing bound.
-/

module
public import Causalean.Estimation.MinimaxATE.ConstCenterHalf.ChiSquaredCore

/-! # Common Ancillary Product

This file tensors the finite-cell null and mixed alternative laws with a common
ancillary product law. The law `ancillaryLaw` models independent ancillary draws, while
`QfalseCont` and `QtrueCont` attach that same factor to both hypotheses.
...
statement `two_point_lower_bound_ancillary_product` is a two-hypothesis
miss-probability lower bound for estimators that observe both the finite cell data
and the ancillary sample. It does not embed these laws into a continuous nuisance class. -/
...
/-- For every [measurable ancillary space](hyp:W), [measure $\rho$
...
/-- For every [measurable ancillary space](hyp:W), [probability measure
$\rho$ on that space](hyp:ρ), and [sample size $n$](hyp:n), [the ancillary
$n$-sample law](goal) is [a probability measure](step:1).
...
/-- For every [measurable ancillary space](hyp:W), [positive number $K$ of paired
cells](hyp:K), [sample size $n$](hyp:n), and [measure $\rho$ on the position space](hyp:ρ), the
[ancillary-product null law](goal) is the product of the finite-cell null $n$-sample law and
...
position space](hyp:ρ), the [ancillary-product alternative law](goal) is the product of the
...
/-- For [an ancillary measure `ρ`](hyp:ρ), [the product null law is a probability
measure](goal). -/
...
/-- For [an ancillary measure `ρ`](hyp:ρ) and [nonnegative bumps satisfying
`α + 2β ≤ 1/2`](hyp:hα,hβ,hαβ), [the product alternative law is a probability
measure](goal). -/
...
/-- For [an ancillary measure `ρ`](hyp:ρ), [nonnegative bumps satisfying
`α + 2β ≤ 1/2`](hyp:hα,hβ,hαβ), and [the finite-cell chi-squared
regime](hyp:hγ,hreg), [the ancillary-product alternative has chi-squared
divergence at most one from the null](goal).

The common ancillary factor cancels, so the finite-cell indistinguishability bound
transfers verbatim. -/
...
/-- The squared density deviation for the ancillary-product experiment is integrable under the null.
...
/-- For [an ancillary measure `ρ`](hyp:ρ), [nonnegative bumps satisfying
`α + 2β ≤ 1/2`](hyp:hα,hβ,hαβ), and [the finite-cell chi-squared
regime](hyp:hγ,hreg), [the ancillary-product null and alternative have total-variation
distance at most one half](goal).

The ancillary observations do not help separate the two hypotheses. -/
...
sample-size regime `2n²γ² ≤ K·log 2` with `γ = α²+2αβ+3β²` and `2γ ≤ 1`](hyp:hγ,hreg).
For [an ancillary measure `ρ`](hyp:ρ) and [any measurable estimator `est` of the
average treatment effect that observes both the finite paired-cell data and `n`
independent ancillary draws](hyp:hest), [the larger of the two miss probabilities — under the
...
The arbitrary ancillary sample cannot improve testing because it has the same law
...
theorem two_point_lower_bound_ancillary_product [NeZero K]
    (ρ : Measure W) [IsProbabilityMeasure ρ]
    (hα : 0 ≤ α) (hβ : 0 ≤ β) (hαβ : α + 2 * β ≤ 1 / 2)
    (hγ : 2 * (α ^ 2 + 2 * α * β + 3 * β ^ 2) ≤ 1)
    (hreg : 2 * (n : ℝ) ^ 2 * (α ^ 2 + 2 * α * β + 3 * β ^ 2) ^ 2 ≤ (K : ℝ) * Real.log 2)
