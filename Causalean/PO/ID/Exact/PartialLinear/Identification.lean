...
/-- The [treatment nuisance regression](goal) in [a partially linear model](hyp:M)
[predicts the realized dose from observed covariates](step:1) in [the ambient
potential-outcome population](hyp:P) with [covariate space](hyp:γ).
...
/-- The [outcome nuisance regression](goal) in [a partially linear model](hyp:M)
[predicts the observed response from covariates alone](step:1) in [the ambient
potential-outcome population](hyp:P) with [covariate space](hyp:γ).
...
/-- The [residualized treatment](goal) in [a partially linear model](hyp:M) [removes
the covariate-predicted component from the realized dose](step:1), isolating variation
orthogonal to covariates in [the ambient population](hyp:P) with [covariate
space](hyp:γ).
...
/-- **The causal bridge.** In [a partially linear causal model](hyp:M), [the Robinson
partialling-out estimand equals the structural treatment effect `θ`](goal) when [the
treatment has residual variation, i.e. `E[(D − m₀(X))²] ≠ 0`](hyp:hV), [the factual
treatment is integrable](hyp:hD), [the structural covariate baseline `b(X)` is
integrable](hyp:hbX), [the structural error `U` is integrable](hyp:hU), [the squared
treatment residual is integrable](hyp:hVsq), and [the product of the structural error and
the treatment residual is integrable](hyp:hUV). This is what makes the partially linear
DML estimator target the causal parameter. -/
