...
module
public import Causalean.PO.ID.Partial.Sensitivity.MSM.CutoffConstruct.ConstantCutoff
...
/-- **Existence of a calibrating cutoff (treated arm).** Fix [a sensitivity parameter Λ](hyp:Λ).
Assume [the treated propensity score is almost surely strictly between zero and one
...
theorem exists_calibrating_cutoff (Λ : ℝ)
...
theorem msmUpperCalib_eq_cutoff_of_universal_cutoff_integrability (Λ : ℝ) (hΛ : 1 < Λ)
  ...
  have heq : S.msmUpperCalib true Λ = S.candMean true (S.cutoffProp Λ c) :=
