...
/-!
# Finite-design fourth-moment-premise bridge

This module turns a finite product randomization design into the product probability space and
reveal filtration needed by a supplied fourth-moment Kolmogorov-bound premise. It identifies the
finite and measure-theoretic conditional expectations, predictable variations, moments, and CDF
expressions, then transports that supplied premise without proving a Heyde--Brown inequality.
-/
...
then [their sum satisfies the finite-design Kolmogorov bound transported from that premise](goal). -/
theorem finiteDesign_kolmogorovBound_of_heydeBrownPremise
    (D : ∀ i, FiniteDesign (alpha i)) (pi : Equiv.Perm (Fin N))
