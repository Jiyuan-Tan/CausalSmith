...
export async function stageP2(io: StageIO): Promise<void> {
  ...
  const paper = [
    ...
    // Pinned to the paper's revision date, not the compile date. `\cspaperdate` is defined by
    // paper_macros.tex (defaulting to \today) and overridden by the generated paper_stamp.tex,
    // so the printed date is the same one the page-1 identity stamp carries and stops drifting
    // every time the bundle is recompiled.
    "\\date{\\cspaperdate}",
    "\\begin{document}",
    "\\maketitle",
    front,
