...
/-- A positive covariance on a finite coordinate type is a real square matrix together with a
proof that it is positive definite. -/
abbrev PositiveCovariance (V : Type*) [Fintype V] :=
  {K : Matrix V V ℝ // Matrix.PosDef K}

/-- The Gaussian covariance discrepancy at candidate `K` and target second-moment matrix `T` is
`log det K + trace (K⁻¹ T)`. -/
noncomputable def gaussianCovarianceDiscrepancy [Fintype V] [DecidableEq V]
    (K T : Matrix V V ℝ) : ℝ :=
  Real.log K.det + Matrix.trace (K⁻¹ * T)

/-- The normalized Gaussian covariance discrepancy subtracts its value at the positive-definite
target itself. -/
...
/-- The Gaussian discrepancy is jointly continuous in a positive-definite candidate and an
arbitrary target matrix. -/
...
/-- The normalized Gaussian discrepancy is jointly continuous in two positive-definite
covariances. -/
...
/-- The normalized Gaussian covariance discrepancy is nonnegative for every positive-definite
...
/-- The normalized Gaussian covariance discrepancy vanishes exactly when the candidate covariance
equals the target covariance. -/
...
/-- Among positive-definite covariance matrices, the Gaussian discrepancy is no larger than its
value at the truth exactly when the candidate is the truth. -/
...
/-- A discrepancy is attained on a covariance model when some member has criterion no larger than
every other member at the given target. -/
...
/-- A model has a compact discrepancy sublevel at a target when one nonempty sublevel cut of the
model is compact. -/
...
/-- A nonempty compact covariance model attains its Gaussian discrepancy minimum at every target
matrix. -/
...
/-- A covariance model with a nonempty compact discrepancy sublevel attains its global Gaussian
discrepancy minimum; this is the coercivity/attainment interface. -/
...
/-- If a covariance model attains its discrepancy minimum and omits the positive-definite truth,
then every model covariance has a common strictly positive normalized population gap. -/
...
/-- A compact nonempty covariance model that omits the positive-definite truth is separated from it
by a strictly positive Gaussian population gap. -/
...
/-- Positive separation of a compact covariance model from a truth is stable under sufficiently
small positive-definite perturbations of that truth. -/
...
/-- The population loss of a covariance model is the infimum Gaussian discrepancy over its
positive-definite candidate matrices. -/
...
/-- The population loss of a fixed nonempty compact covariance model is continuous in the target
...
/-- For a finite family of nonempty compact covariance models containing the truth somewhere, the
population-minimizer indices are exactly the models that contain the truth. -/
