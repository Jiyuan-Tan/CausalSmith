...
/-- Evaluating a rational scalar substitutes the source coordinates and divides its numerator by
its denominator. -/
noncomputable def eval (r : RationalScalar S) (x : S → ℝ) : ℝ :=
  MvPolynomial.eval x r.num / MvPolynomial.eval x r.den

/-- A rational scalar is defined on a domain when its denominator is nonzero throughout that
domain. -/
def DefinedOn (r : RationalScalar S) (D : Set (S → ℝ)) : Prop :=
  ∀ x ∈ D, MvPolynomial.eval x r.den ≠ 0

/-- The cleared quotient-rule numerator for the partial derivative in coordinate `i` is
`(∂ᵢ num) den - num (∂ᵢ den)`. -/
...
/-- A rational scalar is formally nonconstant when at least one cleared formal partial derivative
numerator is a nonzero polynomial. -/
def IsFormallyNonconstant (r : RationalScalar S) : Prop :=
  ∃ i, r.derivativeNumerator i ≠ 0

/-- The full derivative certificate is the sum of squares of all cleared quotient-rule partial
...
/-- At a point where the denominator is nonzero, the Fréchet derivative of a rational scalar
vanishes exactly when its derivative certificate polynomial vanishes. -/
...
/-- Formal nonconstancy of a rational scalar implies that its full derivative certificate is a
nonzero multivariate polynomial. -/
...
/-- A point and coordinate where a cleared quotient-rule derivative numerator evaluates nonzero
witness that the full derivative certificate polynomial is nonzero. -/
...
/-- If a rational scalar is defined on a convex domain and takes distinct values at two points of
that domain, its derivative certificate polynomial is nonzero. -/
