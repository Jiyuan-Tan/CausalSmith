...
/-- A polynomial coordinate map, represented with denominator one, evaluates to ordinary
...
/-- Where both denominators are nonzero, equality of one rational coordinate is equivalent to
vanishing of its cleared equality polynomial. -/
...
/-- The conjunction polynomial vanishes at a real point exactly when every constituent polynomial
vanishes there. -/
...
/-- A conjunction polynomial is nonzero if one constituent has a nonzero value at one explicit
witness point. -/
...
/-- The union-of-conjunctions polynomial vanishes exactly when at least one finite branch has all
of its constituent equations vanish. -/
...
/-- The union-of-conjunctions polynomial is nonzero when every branch has an explicit point and
coordinate witnessing failure of that conjunction. -/
...
/-- On a common definition domain, a finite union of whole-output rational-map equality conditions
is the zero locus of one product-of-sums-of-squares polynomial. -/
...
/-- The rational-map representation of a polynomial matrix evaluates to entrywise polynomial
evaluation. -/
...
/-- If the evaluated determinant is nonzero, the adjugate-over-determinant rational matrix map
evaluates to the inverse of the evaluated polynomial matrix. -/
...
/-- The adjugate-over-determinant inverse representation is defined exactly on any domain where
the determinant polynomial never vanishes. -/
...
/-- A source-renamed rational map evaluates at an assignment exactly as the original map evaluates
at the assignment pulled back along the coordinate map. -/
...
/-- The image-intersection condition for rational matrix maps is the same single polynomial locus,
with row-column pairs used as the finite output-coordinate type. -/
