...
`Stat/MEstimation/GaussianCovariance.lean` turns the Gaussian positive-definite
matrix discrepancy into a finite-model-selection interface.  It keeps compact
attainment and population-gap assumptions explicit, proving the stable gap and
the probability-convergence bridge consumed by finite penalized argmin results.

<!-- GEN:Causalean.Stat.MEstimation.GaussianCovariance -->
<!-- /GEN -->
...
## Mathlib/AlgebraicGeometry — rational-map loci

These modules compile equality, image intersections, matrix inversion, and
vanishing derivatives of finite-dimensional real rational maps into single
multivariate-polynomial zero loci, with denominator hypotheses kept explicit.
