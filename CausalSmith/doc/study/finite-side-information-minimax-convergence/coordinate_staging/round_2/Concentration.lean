...
/-- Given a [finite alphabet](hyp:C), [positive sample size](hyp:hm), [ordered finite sample](hyp:z),
and an [atom](hyp:c), the [empirical atom frequency](goal) is its sample count divided by sample size. -/
noncomputable def empiricalFrequency {m : ℕ} (hm : 0 < m) (z : Fin m → C) (c : C) : ℝ :=
  (∑ i, if z i = c then (1 : ℝ) else 0) / m

/-- Given a [finite alphabet](hyp:C), [positive sample size](hyp:hm), a [sample](hyp:z), and a [side law](hyp:w),
the [empirical L1 error](goal) is the sum of absolute atom-frequency errors. -/
noncomputable def empiricalL1 {m : ℕ} (hm : 0 < m) (z : Fin m → C) (w : FinitePmf C) : ℝ :=
  ∑ c, |empiricalFrequency C hm z c - w.1 c|

/-- Every [empirical atom frequency](hyp:C,hm,z,c) on a finite alphabet is [nonnegative](goal). -/
...
/-- A [finite alphabet](hyp:C) and [positive finite sample](hyp:hm,z) determine the
[probability-simplex point](goal) whose coordinates are its empirical atom frequencies. -/
...
/-- Every [atom coordinate](hyp:c) of the [empirical probability vector](hyp:C,hm,z)
...
/-- For a [finite alphabet](hyp:C) and every [positive tolerance](hyp:hε), the
[uniform finite-category empirical L1 tail bound](goal) tends to zero as sample size tends to infinity. -/
