...
import Causalean.Stat.Minimax.FiniteSideInformation.Main
...
/-- A [closed subset](hyp:hK) of the [finite probability simplex](hyp:K) is [compact](goal). -/
...
/-- Given a [model class](hyp:K), its [closedness and nonemptiness](hyp:hK,hKne),
[label probabilities](hyp:p), [side coordinates](hyp:q), a [target](hyp:tau),
[simplex-valid probabilities](hyp:hp,hq), [continuous coordinates and target](hyp:hpcont,hqcont,htau),
[ordered action bounds](hyp:hlu), and a [target in those bounds](hyp:htau_mem), the
[empirical-side minimax values converge to the exact-side benchmark](goal). -/
