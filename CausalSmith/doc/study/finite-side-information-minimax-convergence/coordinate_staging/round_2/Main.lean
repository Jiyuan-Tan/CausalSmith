...
import Causalean.Stat.Minimax.FiniteSideInformation.Approximation
...
with finite labeled and nonempty side alphabets, the [bounded squared-loss empirical-side minimax values
converge to the exact-side minimax benchmark](goal) as iid side-sample size tends to infinity. -/
