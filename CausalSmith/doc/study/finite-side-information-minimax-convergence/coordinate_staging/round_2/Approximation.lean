...
import Causalean.Stat.Minimax.FiniteSideInformation.Concentration
import Causalean.Stat.Minimax.FiniteSideInformation.Selector
...
[ordered action bounds](hyp:hlu), a [target in those bounds](hyp:htau_mem), and a [positive tolerance](hyp:hε)
on a compact parameter space, a [finite family of local decisions](goal) covers the model with a positive margin
and stays within the tolerance of exact-side minimax risk on each larger ball. -/
theorem finiteCover_approximateDecision
  ...
  -- all conditional-completeness hypotheses for `worstCaseRisk` and `minimaxValue`.
  classical
  ...
  have hfiber_bdd : BddAbove
    ...
    unfold fiberMinimaxValue
    calc
      Causalean.Stat.minimaxValue fiberRisk ≤
          Causalean.Stat.worstCaseRisk fiberRisk d0 :=
  ...
  have hlocal (theta0 : Theta) : ∃ (d : BoundedDecision X l u) (r : ℝ),
    ...
    have hbddBelow : BddBelow
        (Set.range (Causalean.Stat.worstCaseRisk localRisk)) :=
      Causalean.Stat.bddBelow_range_worstCaseRisk
        (fun d theta ↦ (hrisk d theta.1).1)
    have hvalue : (⨅ d, Causalean.Stat.worstCaseRisk localRisk d) <
        exactSideMinimaxValue p q hq tau l u + ε := by
      ...
      simpa [localMinimaxValue, Causalean.Stat.minimaxValue, localRisk] using hlocal_lt
...
/-- Given [label probabilities](hyp:p), [side coordinates](hyp:q), a [target](hyp:tau),
[simplex-valid probabilities](hyp:hp,hq), [continuous coordinates and target](hyp:hpcont,hqcont,htau),
[ordered action bounds](hyp:hlu), and a [target in those bounds](hyp:htau_mem) on a compact finite model,
...
theorem empiricalSideMinimax_limsup_le_exact
  ...
  have hminimax_le (m : ℕ) :
    ...
    unfold empiricalSideMinimaxValue
    calc
      Causalean.Stat.minimaxValue (empiricalSideRisk p q hq tau m) ≤
          Causalean.Stat.worstCaseRisk (empiricalSideRisk p q hq tau m) d0 :=
  ...
  calc
    Causalean.Stat.minimaxValue (empiricalSideRisk p q hq tau m) ≤
        Causalean.Stat.worstCaseRisk (empiricalSideRisk p q hq tau m) d :=
