...
/-- The weighted side-ball mass is the two-dimensional Lebesgue integral of the radial power
weight over a chosen one-sided open ball. -/
...
/-- An annular tube of width δ consists of the planar points whose radial distance from the unit
circle is strictly less than δ. -/
...
/-- The weighted annular normalizer is the two-dimensional Lebesgue integral of the radial power
weight over an annular tube. -/
def annularNormalizer (κ δ : ℝ) : ℝ :=
  ∫ z in annularTube δ, powerWeight κ z ∂volume

/-- When [the exponent κ exceeds two](hyp:hκ) and [the tube width is positive and below one]
(hyp:hδpos,hδlt), [the radial power weight is integrable on that annular tube and its weighted
