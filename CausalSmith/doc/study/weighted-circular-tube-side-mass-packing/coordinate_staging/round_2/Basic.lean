...
/-- The Euclidean plane, represented by complex numbers with their standard Euclidean metric and
two-dimensional Lebesgue measure. -/
abbrev Plane := ℂ

/-- The unit circle consists of the planar points at Euclidean distance one from the origin. -/
def unitCircle : Set Plane := {z | ‖z‖ = 1}

/-- The signed radial offset is negative inside the unit circle and positive outside it. -/
def radialOffset (z : Plane) : ℝ := ‖z‖ - 1

/-- The two radial sides adjacent to the unit circle: its interior and its strict exterior. -/
inductive CircleSide where
  | inside
  | outside
  deriving DecidableEq

/-- The region on a chosen side is the closed interior for `inside` and the strict exterior for
`outside`. -/
def CircleSide.region : CircleSide → Set Plane
  | .inside => {z | radialOffset z ≤ 0}
  | .outside => {z | 0 < radialOffset z}

/-- The radial power weight raises the absolute distance from the unit circle to the exponent
κ minus two. -/
def powerWeight (κ : ℝ) (z : Plane) : ℝ := |radialOffset z| ^ (κ - 2)

/-- A one-sided ball is an open Euclidean ball restricted to one chosen radial side of the unit
circle. -/
