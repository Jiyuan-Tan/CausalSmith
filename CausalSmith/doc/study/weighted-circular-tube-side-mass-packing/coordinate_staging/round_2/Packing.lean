...
/-- A planar set is separated at scale `r` when any two distinct points are at least that distance
apart. -/
def IsSeparatedAtLeast (r : ℝ) (s : Set Plane) : Prop :=
  s.Pairwise fun x y => r ≤ dist x y

/-- A finite site set is maximal separated on the unit circle when no further circle point can be
added without violating its separation scale. -/
...
/-- A finite site set covers the unit circle at radius `r` when every circle point lies in an open
radius-`r` ball around one of its sites. -/
