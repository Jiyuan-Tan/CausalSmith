...
/-- A Borel measure on the real line is symmetric about zero when reflection
through zero leaves it unchanged. -/
def IsSymmetric (ν : Measure ℝ) : Prop :=
  Measure.map (fun x : ℝ => -x) ν = ν

/-- A measure is concentrated on `[-1,1]` when the complement of that interval
has measure zero. -/
...
/-- Doubling the two already oriented Jordan parts of an extremal decomposition
produces the symmetric probability-prior pair. -/
