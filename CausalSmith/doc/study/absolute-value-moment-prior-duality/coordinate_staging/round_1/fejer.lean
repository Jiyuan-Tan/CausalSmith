...
/-- The one-sided sum of Fourier characters with frequencies below `n`. -/
noncomputable def oneSidedFourierSum (n : ℕ) : C(AddCircle Real.pi, ℂ) :=
  ∑ r ∈ Finset.range n, fourier (r : ℤ)

/-- The normalized Fejér kernel on the circle of period `π`. -/
...
/-- The Fejér mean of a continuous function, evaluated at the origin via
integration against the normalized Fejér kernel. -/
...
/-- The Fejér mean as a continuous complex-linear functional of operator norm
at most one. -/
...
/-- The de la Vallée--Poussin mean `2 F_{2n} - F_n` as a continuous linear
functional. -/
...
/-- Evaluation at zero minus the de la Vallée--Poussin mean, the bounded
functional detecting the cusp of absolute value. -/
...
/-- The continuous doubling map on the additive circle of period `π`. -/
