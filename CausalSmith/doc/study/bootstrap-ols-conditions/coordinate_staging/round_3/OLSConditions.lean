...
/-!
# Bootstrap OLS conditions from population moments

This module discharges the resampling-side conditions for the pairs bootstrap of ordinary least
squares.  Conditional convergence of raw resample moments, together with nonsingularity of the
population design moment, yields resample normal equations, coefficient consistency, and
moment-only bootstrap coverage for OLS contrasts.
-/
...
This is the finite-dimensional continuity bridge from resample raw moments to the OLS
coefficient. -/
...
/-- Given [regressors and an outcome](hyp:x,y), [a finite data set of size `n`](hyp:n,data), and
[a nonsingular empirical Gram matrix](hyp:hdet), [the closed-form sample OLS estimator satisfies
its empirical score equation exactly](goal).

This deterministic identity applies directly to each nonsingular bootstrap resample. -/
...
/-- Under [an iid sample](hyp:S), [measurable regressors and outcome](hyp:hx,hy), and
...
The proof transports the finite raw-moment coordinate type to Euclidean coordinates and applies
the vector conditional bootstrap weak law. -/
...
/-- Under [an iid sample](hyp:S), [measurable regressors and outcome](hyp:hx,hy),
[integrable raw OLS moments](hyp:hraw), and [a positive-definite population Gram
matrix](hyp:hQ), [the resampled OLS estimator solves its own normal equations in bootstrap
probability, in sampling probability](goal).

Nonsingularity holds on a neighborhood of the population raw moments, and the previous weak law
makes the probability of leaving that neighborhood vanish. -/
...
/-- Under [an iid sample](hyp:S), [measurable regressors and outcome](hyp:hx,hy),
[integrable raw OLS moments](hyp:hraw), and [a positive-definite population Gram
matrix](hyp:hQ), [the resampled OLS estimator converges to the population OLS coefficient in
bootstrap probability, in sampling probability](goal).

Continuity of the closed-form OLS map at the population raw moments transfers the conditional
raw-moment weak law to coefficient consistency. -/
...
/-- Under [an iid sample](hyp:S), [measurable regressors and outcome](hyp:hx,hy),
[integrable raw OLS moments](hyp:hraw), and [a positive-definite population Gram
matrix](hyp:hQ), and for [a linear contrast with positive influence variance](hyp:c,hVar), [the
OLS contrast is bootstrap asymptotically linear without additional resampling assumptions](goal).
-/
...
/-- Under [an iid sample](hyp:S), [measurable regressors and outcome](hyp:hx,hy),
[integrable raw OLS moments](hyp:hraw), [a positive-definite population Gram matrix](hyp:hQ),
and [a linear contrast with positive influence variance](hyp:c,hVar), at [an interior nominal
error level](hyp:alpha,halpha0,halpha1), [the percentile bootstrap interval for the OLS contrast
has limiting coverage one minus that error level](goal).
-/
