...
| `Mathlib.LinearAlgebra.FinitePerronFrobeniusPositiveEigenvector.sphereRayleighValue` | `Matrix ι ι ℝ → ℝ` | The greatest quadratic Rayleigh-form value among the Euclidean unit vectors for a finite real matrix, called the Euclidean-sphere top Rayleigh value, is given by a supremum. For an empty coordinate type there are no unit vectors and the supremum of the empty set is zero by convention. |
...
| `Mathlib.LinearAlgebra.FinitePerronFrobeniusPositiveEigenvector.iSupRayleighValue` | `Matrix ι ι ℝ → ℝ` | Mathlib's supremum Rayleigh quotient for a finite real matrix, called the nonzero-vector top Rayleigh value, is given by the matrix's Euclidean linear map. For an empty coordinate type there are no nonzero vectors and the empty supremum is zero by convention. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.LinearAlgebra.FinitePerronFrobeniusPositiveEigenvector.restrictMatrix` | `Matrix ι ι ℝ → (s : Finset ι) → Matrix ↥s ↥s ℝ` | The principal submatrix of a real square matrix on a finite set of coordinates, called its restricted matrix, is given by selecting those rows and columns. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Algorithms.MonotoneWindowDeque.Deque` | `Type` | A deque is represented by its front-to-back list of natural-number indices. The type itself imposes no ordering or monotonicity; the monotone-deque invariant is carried separately by the validity predicates. |
...
| `Mathlib.Algorithms.MonotoneWindowDeque.ValidAt` | `ℕ → ℕ → Causalean.Mathlib.Algorithms.MonotoneWindowDeque.Deque → Prop` | A stream, two raw endpoints, and a deque satisfy the validity invariant when every retained index is active for the endpoints and within the stream, retained indices strictly increase from front to back, their stream values strictly decrease from front to back, and every active in-stream index that is not retained is dominated by a later retained index whose value is at least as large. |
...
| `Mathlib.Analysis.AbsoluteValueMomentPriorDuality.instCompactSpaceElemRealSymmUnitInterval` | `CompactSpace ↑Causalean.Mathlib.Analysis.AbsoluteValueMomentPriorDuality.symmUnitInterval` | — |
| `Mathlib.Analysis.AbsoluteValueMomentPriorDuality.instNonemptyElemRealSymmUnitInterval` | `Nonempty ↑Causalean.Mathlib.Analysis.AbsoluteValueMomentPriorDuality.symmUnitInterval` | — |
...
| `Mathlib.Analysis.AbsoluteValueMomentPriorDuality.exists_symmetric_momentMatched_absGap` | `0 < K → Even K → Nonempty (Causalean.Mathlib.Analysis.AbsoluteValueMomentPriorDuality.AbsMomentMatchedPriors K)` | For a positive even degree, a symmetric moment-matched probability-prior pair with the exact absolute-moment gap exists. The positivity and evenness assumptions are not used by the proof; the same conclusion holds for every degree via `exists_absExtremalDecomposition`. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight` | `(Fin n → ℝ) → Fin n → ℝ` | The normalized signed Lagrange weight at a node is its barycentric nodal weight divided by the sum of the absolute barycentric weights. the stated inputs establish the defined object. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.sum_abs_normalizedLagrangeWeight_eq_one` | `Function.Injective nodes → ∑ i, \|Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight nodes i\| = 1` | For `L+2` distinct real nodes, the normalized signed Lagrange weights have total absolute mass one. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight_alternates` | `StrictMono nodes → ∀ (i : Fin (L + 2)), Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight nodes i = (-1) ^ (L + 1 - ↑i) * \|Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight nodes i\|` | For `L+2` strictly increasing real nodes, the normalized Lagrange weight at index `i` has sign `(-1)^(L+1-i)`. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.sum_normalizedLagrangeWeight_mul_pow_eq_zero` | `Function.Injective nodes → j ≤ L → ∑ i, Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight nodes i * nodes i ^ j = 0` | For `L+2` distinct real nodes, the normalized signed Lagrange weights annihilate the monomial `x^j` whenever `j ≤ L`. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.sum_normalizedLagrangeWeight_mul_eval_eq_zero` | `Function.Injective nodes → ∀ {Q : Polynomial ℝ}, Q.natDegree ≤ L → ∑ i, Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.normalizedLagrangeWeight nodes i * Polynomial.eval (nodes i) Q = 0` | For `L+2` distinct real nodes, the normalized signed Lagrange weights annihilate the values of every real polynomial of degree at most `L`. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.AlternationDualCertificate` | `(ℝ → ℝ) → ℝ → ℝ → ℕ → Type` | An alternation dual certificate consists of a best degree-`L` approximant, `L+2` ordered extrema of its residual, and the normalized signed Lagrange weights on those nodes, with the exact oriented target evaluation. the stated inputs establish the described certificate. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.exists_alternationDualCertificate` | `r < s → ContinuousOn f (Set.Icc r s) → ∀ (L : ℕ), Nonempty (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.AlternationDualCertificate f r s L)` | Every continuous real target on a nondegenerate compact interval admits a finite Chebyshev alternation dual certificate for degree-`L` approximation. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.FiniteMomentDual` | `(ℝ → ℝ) → ℝ → ℝ → ℕ → Type` | A finite moment dual packages `L+2` ordered interval nodes and normalized signed weights that annihilate moments through degree `L` and separate the target by exactly its best uniform approximation error. the stated inputs establish the described certificate. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.AlternationDualCertificate.toFiniteMomentDual` | `Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.FiniteMomentDual f r s L` | Forgetting the best approximant and orientation from an alternation certificate yields the paper-independent finite moment-dual package. the stated inputs establish the defined object. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.exists_finiteMomentDual` | `r < s → ContinuousOn f (Set.Icc r s) → ∀ (L : ℕ), Nonempty (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.FiniteMomentDual f r s L)` | Every continuous real target on a nondegenerate compact interval admits `L+2` ordered, normalized finite weights annihilating moments through degree `L` and attaining the best approximation error in absolute value. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.rationalTarget` | `ℝ → ℝ → ℝ` | The rational target with parameter `a` is the function `x ↦ x/(x+a)`. the stated inputs establish the defined object. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.continuousOn_rationalTarget` | `-a ∉ Set.Icc r s → ContinuousOn (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.rationalTarget a) (Set.Icc r s)` | If the pole `-a` is outside a closed interval, the rational target `x ↦ x/(x+a)` is continuous on that interval. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.exists_rationalFiniteMomentDual` | `r < s → -a ∉ Set.Icc r s → ∀ (L : ℕ), Nonempty (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.FiniteMomentDual (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.rationalTarget a) r s L)` | On a nondegenerate interval avoiding `-a`, there are `L+2` ordered, normalized finite weights that match moments through degree `L` and whose rational-target separation is the exact best degree-`L` uniform error. the stated inputs establish the stated conclusion. |
<!-- /GEN -->
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.adjacentPairProduct` | `ℝ → List ℝ → ℝ` | `adjacentPairProduct t xs` multiplies `a * b + t` over consecutive pairs `a, b` of `xs`. A final unpaired entry contributes no factor. the stated inputs establish the defined object. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.crossingPairProduct_le_adjacentPairProduct` | `0 ≤ t → b ≤ a → c ≤ b → d ≤ c → (a * c + t) * (b * d + t) ≤ (a * b + t) * (c * d + t)` | For four decreasing real numbers, replacing the crossing pairs `(a,c)` and `(b,d)` by the adjacent pairs `(a,b)` and `(c,d)` cannot decrease the product after adding the same nonnegative constant to each pair. the stated inputs establish the stated conclusion. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.exists_int_abs_sub_mul_pi_div_le` | `0 < L → ∀ (θ : ℝ), ∃ m, \|θ - ↑m * (Real.pi / ↑L)\| ≤ Real.pi / (2 * ↑L)` | Every real angle differs from an integer multiple of `π/L` by at most half of that positive step. the stated inputs establish the stated conclusion. |
| `Mathlib.Analysis.FinitePolynomialAlternationDuality.chebyshevPairedChordList_add_int_mul_pi_div_perm` | `0 < L → ∀ (θ : ℝ) (m : ℤ), (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.chebyshevPairedChordList L (θ + ↑m * (Real.pi / ↑L))).Perm (Causalean.Mathlib.Analysis.FinitePolynomialAlternationDuality.chebyshevPairedChordList L θ)` | Shifting an angle by any integer multiple of `π/L` permutes the paired list of the `2L` Chebyshev chord squares. the stated inputs establish the stated conclusion. |
...
| `Mathlib.Analysis.SymmetricTensorPencil.leastColumnSingularValue` | `Matrix ι κ ℝ → ℝ` | The last domain-indexed singular value of a finite real matrix, i.e. the singular value at index one less than the number of columns. For a matrix with no columns the index is zero by natural subtraction; the results using it assume a nonempty column index. With its explicit inputs, the defined object is given by the displayed formula. |
...
| `Mathlib.Analysis.SymmetricTensorPencil.pencilProbes` | `Causalean.Mathlib.Analysis.SymmetricTensorPencil.Vec p → Fin q → Causalean.Mathlib.Analysis.SymmetricTensorPencil.Vec p` | The probe family containing `q-1` copies of `u` followed by one copy of `w`, for positive `q`; for `q = 0` it is the empty family. With its explicit inputs, the defined object is given by the displayed formula. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Analysis.SymmetricTensorPencil.PairwiseGap` | `(ι → ℝ) → ℝ → Prop` | A real scalar family, over an arbitrary index type, has pairwise gap at least `gap`. With its explicit inputs, the defined object is given by the displayed formula. |
| `Mathlib.Analysis.SymmetricTensorPencil.diagonalizableMatrix` | `Matrix (Fin n) (Fin n) ℝ → (Fin n → ℝ) → Matrix (Fin n) (Fin n) ℝ` | The conjugation of the diagonal matrix of the prescribed real entries by `S`, that is `S` times the diagonal times the inverse of `S`. It is diagonalized by `S` with those entries when `S` is invertible; for singular `S` the inverse is zero by convention and the value is the zero matrix. With its explicit inputs, the defined object is given by the displayed formula. |
| `Mathlib.Analysis.SymmetricTensorPencil.coordinateProjector` | `Matrix (Fin n) (Fin n) ℝ → Fin n → Matrix (Fin n) (Fin n) ℝ` | The coordinate projector selected by coordinate `j` of a diagonalizer `S`: `S` times the `j`-th standard diagonal unit matrix times the inverse of `S`. It is the rank-one spectral projector when `S` is invertible; for singular `S` the inverse is zero by convention and the value is the zero matrix. With its explicit inputs, the defined object is given by the displayed formula. |
...
| `Mathlib.Analysis.SymmetricTensorPencil.normalizeVec` | `Causalean.Mathlib.Analysis.SymmetricTensorPencil.Vec p` | Normalize a finite coordinate vector by its Euclidean norm, with the conventional zero value when the input is zero. With its explicit inputs, the defined object is given by the displayed formula. |
...
| `Mathlib.Analysis.SymmetricTensorPencil.traceCoordinates` | `(Causalean.Mathlib.Analysis.SymmetricTensorPencil.Vec p → Matrix (Fin n) (Fin n) ℝ) → (Fin n → Matrix (Fin n) (Fin n) ℝ) → Fin n → Causalean.Mathlib.Analysis.SymmetricTensorPencil.Vec p` | Trace coordinates evaluate each coordinate pencil against the `j`-th matrix of a supplied matrix family, by the trace of their product. No condition is imposed on the family; in the recovery argument it is a family of rank-one spectral projectors. With its explicit inputs, the defined object is given by the displayed formula. |
...
## Mathlib/Probability/PilotSelection — independent binary pilot selection

This module computes exact first, second, and variance formulas when an independent pilot event
selects between two square-integrable branch statistics.

<!-- GEN:Causalean.Mathlib.Probability.PilotSelection -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.memLp_mul_prod_two` | `MeasureTheory.MemLp f 2 muS → MeasureTheory.MemLp g 2 muK → MeasureTheory.MemLp (fun z => f z.1 * g z.2) 2 (muS.prod muK)` | A product of independent square-integrable scalar functions remains square-integrable on the product space. The two factors are square-integrable, and their coordinatewise product is square-integrable under the product measure. |
| `Mathlib.Probability.pilotSelect` | `Set J → (E → ℝ) → (E → ℝ) → J × E → ℝ` | Given an event, two branch statistics, and a pilot-tail pair, the selected statistic uses the first branch on the event and the second branch off the event. |
| `Mathlib.Probability.integral_pilotSelect` | `MeasurableSet A → ∀ (P H : E → ℝ), MeasureTheory.Integrable P μE → MeasureTheory.Integrable H μE → ∫ (z : J × E), Causalean.Mathlib.Probability.pilotSelect A P H z ∂μJ.prod μE = μJ.real A * ∫ (e : E), P e ∂μE + (1 - μJ.real A) * ∫ (e : E), H e ∂μE` | For two probability laws, a measurable pilot event, and two integrable branch statistics, the selected statistic's mean is the event-probability mixture of the two branch means. |
| `Mathlib.Probability.integral_pilotSelect_sq` | `MeasurableSet A → ∀ (P H : E → ℝ), MeasureTheory.MemLp P 2 μE → MeasureTheory.MemLp H 2 μE → ∫ (z : J × E), Causalean.Mathlib.Probability.pilotSelect A P H z ^ 2 ∂μJ.prod μE = μJ.real A * ∫ (e : E), P e ^ 2 ∂μE + (1 - μJ.real A) * ∫ (e : E), H e ^ 2 ∂μE` | For two probability laws, a measurable pilot event, and two square-integrable branch statistics, the selected statistic's second moment is the event-probability mixture of the two branch second moments. |
| `Mathlib.Probability.memLp_pilotSelect` | `MeasurableSet A → ∀ (P H : E → ℝ), MeasureTheory.MemLp P 2 μE → MeasureTheory.MemLp H 2 μE → MeasureTheory.MemLp (Causalean.Mathlib.Probability.pilotSelect A P H) 2 (μJ.prod μE)` | For two probability laws, a measurable pilot event, and two square-integrable branch statistics, the pilot-selected statistic is square-integrable under the product law. |
| `Mathlib.Probability.variance_pilotSelect` | `MeasurableSet A → ∀ (P H : E → ℝ), MeasureTheory.MemLp P 2 μE → MeasureTheory.MemLp H 2 μE → ProbabilityTheory.variance (Causalean.Mathlib.Probability.pilotSelect A P H) (μJ.prod μE) = μJ.real A * ProbabilityTheory.variance P μE + (1 - μJ.real A) * ProbabilityTheory.variance H μE + μJ.real A * (1 - μJ.real A) * (∫ (e : E), P e ∂μE - ∫ (e : E), H e ∂μE) ^ 2` | For two probability laws, a measurable pilot event, and two square-integrable branch statistics, the selected statistic's variance is the mixture of branch variances plus the between-branch mean term. |
<!-- /GEN -->
...
This companion to the existing finite-Markov normal-CDF interface verifies caller-supplied rational cells using central Gaussian power series and Mills-ratio tails, avoiding the uniform-mesh scaling limitation.
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.targetWidth` | `ℚ` | The requested maximum width of a high-precision caller cell is exactly `10⁻¹²`, represented as a rational rather than a floating-point number. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.supportedEndpointBound` | `ℚ` | The reusable checker is required to support endpoints of absolute value at most `193 / 5`. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.reflectInterval` | `Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | Reflecting an interval `I` across one half produces the interval `1 - I`. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.central_or_tail` | `\|q\| ≤ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.centralCutoff ∨ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.centralCutoff < \|q\|` | Every endpoint lies either in the central regime or strictly in the tail regime; this is the exhaustive case split used by the top-level checker. |
...
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.expTaylorInterval` | `ℚ → ℕ → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The finite Taylor interval for `exp z`, using Causalean's exact rational partial sum and explicit remainder radius. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.expTaylorInterval_width` | `(Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.expTaylorInterval z degree).width = 2 * Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.Transcendental.expRemainder z degree` | The width of the raw reduced exponential interval is exactly twice its explicit rational Taylor remainder radius. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.expCertificateInterval` | `Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The exact interval computed from the caller's split and Taylor degree. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalizationInterval` | `Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The raw normalization interval is the reciprocal square-root enclosure when its exact rational guards succeed, and a harmless fallback otherwise. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalizationInterval_sound` | `Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalizationScheduleCheck s = true → (Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalizationInterval s).Contains (1 / √(2 * Real.pi))` | Every raw normalization interval contains the real number `1 / sqrt (2π)`. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.gaussianIntegralInterval` | `ℚ → ℕ → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The alternating enclosure is the rational interval whose endpoints are two consecutive Gaussian-integral partial sums. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.centralMagnitudeInterval` | `ℚ → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.CentralCertificate → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The positive-magnitude central evaluator returns `1/2` plus the product of the normalization enclosure and alternating integral enclosure. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.centralInterval` | `ℚ → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.CentralCertificate → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The signed central evaluator uses CDF symmetry when the endpoint is negative. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.centralCheck` | `ℚ → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.CentralCertificate → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Bool` | The central checker validates normalization, the cutoff, the exact alternating-tail condition, and refinement into the caller-supplied interval. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.tailDensityInterval` | `(q : ℚ) → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.TailCertificate q → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The certified rational interval for the density `φ(\|q\|)`. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.millsTailInterval` | `ℚ → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | A rational Mills tail enclosure is the hull of the lower-factor product and upper-factor product with the certified density interval, the factors being `x/(x²+1)` and `1/x`. It encloses the upper tail only for positive `x`, as required by its soundness theorem; at `x = 0` both factors are zero by the division convention. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.millsTailInterval_sound` | `0 < x → ∀ (density : Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval), density.Contains (Causalean.Mathlib.stdNormalPDF ↑x) → (Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.millsTailInterval x density).Contains (1 - Causalean.Mathlib.stdNormalCDF ↑x)` | A density enclosure and positivity of `x` imply that the rational Mills interval contains the exact standard-normal upper-tail probability. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.tailInterval` | `(q : ℚ) → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.TailCertificate q → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval` | The signed tail evaluator returns the upper-tail interval directly for a negative endpoint and reflects it for a nonnegative endpoint. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.tailCheck` | `(q : ℚ) → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.TailCertificate q → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Bool` | The tail checker validates normalization and exponential subcertificates, the strict cutoff, and exact refinement into the caller-supplied interval. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalCDFCheck` | `(q : ℚ) → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.MethodCertificate q → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Bool` | The top-level executable checker dispatches to the selected method and validates the caller-supplied rational enclosure. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalCDFCheck_sound` | `Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.normalCDFCheck q method reported = true → reported.Contains (Causalean.Mathlib.stdNormalCDF ↑q)` | When the selected top-level certificate check succeeds, the caller-supplied rational interval contains the exact standard-normal CDF at its endpoint. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.fineNormalCDFCheck` | `(q : ℚ) → Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.MethodCertificate q → Causalean.Mathlib.Analysis.CertifiedContourIntervalArithmetic.RatInterval → Bool` | The executable high-precision checker accepts exactly when the analytic checker accepts and the caller's endpoint and reported width satisfy the exact rational limits `\|q\| ≤ 193/5` and `width ≤ 10⁻¹²`. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.fineNormalCDFCheck_sound` | `Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.fineNormalCDFCheck q method reported = true → reported.Contains (Causalean.Mathlib.stdNormalCDF ↑q) ∧ \|q\| ≤ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.supportedEndpointBound ∧ reported.width ≤ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.targetWidth` | Soundness of the executable high-precision checker: an accepted cell contains the exact CDF value and satisfies both advertised rational limits. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.NormalCDFCertificate` | `ℚ → Type` | A proof-producing endpoint certificate packages caller data with the successful result of the executable checker. |
...
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.FineNormalCDFCertificate` | `ℚ → Type` | A fine endpoint certificate packages its data with successful evaluation of the public executable high-precision checker; range and width are checked, not separately trusted as caller-provided proof fields. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.FineNormalCDFCertificate.toNormalCDFCertificate` | `Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.NormalCDFCertificate q` | Forgetting the fine limits yields an ordinary checked CDF certificate. |
| `Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.FineNormalCDFCertificate.sound_range_and_width` | `c.enclosure.Contains (Causalean.Mathlib.stdNormalCDF ↑q) ∧ \|q\| ≤ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.supportedEndpointBound ∧ c.enclosure.width ≤ Causalean.Mathlib.Probability.CertifiedFiniteMarkovExpectation.CertifiedNormalCDFEnclosure.targetWidth` | A fine certificate simultaneously gives CDF containment, the supported endpoint range, and the requested `10⁻¹²` rational width guarantee. |
...
| `Mathlib.Analysis.SymmetricTensorPencil.liftedConditionEnvelope` | `ℕ → ℝ → ℝ` | The condition-number envelope for a lifted matrix with `n` unit columns and least singular value at least `sigma`, namely the square root of `n` divided by `sigma`. The formula is unguarded: it is a condition-number envelope only for positive `sigma`, and a zero margin gives zero by the division-by-zero convention. With its explicit inputs, the defined object is given by the displayed formula. |
| `Mathlib.Analysis.SymmetricTensorPencil.localInverseRadius` | `ℕ → ℕ → ℝ → ℝ → ℝ → ℝ` | The admissible tensor perturbation radius for the quantitative tensor-pencil inverse. The formula is unguarded: it is an admissible radius only in the positive-margin regime assumed by the inverse theorem, and degenerate margins give zero by the division-by-zero convention. With its explicit inputs, the defined object is given by the displayed formula. |
| `Mathlib.Analysis.SymmetricTensorPencil.traceRecoveryConstant` | `ℕ → ℕ → ℝ → ℝ → ℝ → ℝ` | The coordinatewise trace-recovery Lipschitz factor for the quantitative tensor-pencil inverse. The formula is unguarded: it is a valid Lipschitz factor only in the positive-margin regime assumed by the recovery theorems, and degenerate margins give zero by the division-by-zero convention. With its explicit inputs, the defined object is given by the displayed formula. |
| `Mathlib.Analysis.SymmetricTensorPencil.factorRecoveryConstant` | `ℕ → ℕ → ℕ → ℝ → ℝ → ℝ → ℝ` | The final factor-matrix Frobenius Lipschitz factor for the quantitative tensor-pencil inverse. It is a fixed multiple of the trace-recovery factor and inherits its unguarded conventions: it is a valid Lipschitz factor only in the positive-margin regime of the recovery theorems. With its explicit inputs, the defined object is given by the displayed formula. |
...
## `Graph/DSep/OrderedLocalSG.lean` (addendum) — active+directed-arm join
...
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramAverage_productRisk_le` | `∫ (z : (Fin NX → X) × (Fin NY → Y)), (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramAverage est (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finiteSampleHistogram z.1) (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finiteSampleHistogram z.2) (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramAverage_productRisk_le._proof_1 hX z) (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramAverage_productRisk_le._proof_2 hY z) - theta) ^ 2 ∂(MeasureTheory.Measure.pi fun x => P).prod (MeasureTheory.Measure.pi fun x => Q) ≤ ∫ (z : Fin n → X × Y), (est z - theta) ^ 2 ∂MeasureTheory.Measure.pi fun x => P.prod Q` | Given two finite-alphabet probability laws, a paired-sample estimator, a real target, and certificates that both fixed totals contain the requested sample length, independent histogram averaging followed by coordinatewise pairing has no greater squared risk than the paired iid experiment. |
<!-- /GEN -->
...
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionPMF` | `(X → ℕ) → PMF (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X)` | Given a finite count vector, the histogram reconstruction distribution is uniform over all ordered samples having exactly those counts. Its definition depends only on the count vector, not on a sampling law or intensity. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel` | `(X : Type u_1) → [Fintype X] → [inst : MeasurableSpace X] → [MeasurableSingletonClass X] → [DecidableEq X] → ProbabilityTheory.Kernel (X → ℕ) (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X)` | On a finite measurable alphabet, the histogram reconstruction kernel sends every count vector to the uniform distribution on its ordered histogram fibre. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel_isMarkovKernel` | `ProbabilityTheory.IsMarkovKernel (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X)` | On a finite measurable alphabet, the histogram reconstruction channel is a Markov kernel. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel_singleton` | `((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X) c) {Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.fixedSizeEmbed (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramTotal c) ↑x} = (↑(Fintype.card (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.HistogramFiber c)))⁻¹` | Given a count vector and one compatible ordering, the reconstruction kernel assigns that ordered sample the reciprocal of the histogram-fibre cardinality. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel_histogram_eq` | `((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X) c) {s \| Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finiteSampleHistogram s.points = c} = 1` | Given a count vector, reconstruction is supported entirely on finite ordered samples whose histogram is exactly that vector. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.map_histogram_histogramReconstructionKernel` | `MeasureTheory.Measure.map (fun s => Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finiteSampleHistogram s.points) ((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X) c) = MeasureTheory.Measure.dirac c` | Given a count vector, pushing its reconstruction through the histogram map returns the point mass at the original count vector. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.PairedHistogram.HistogramReconstruction.CountLaw -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw` | `(P : MeasureTheory.Measure X) → [MeasureTheory.IsProbabilityMeasure P] → NNReal → MeasureTheory.Measure (X → ℕ)` | Given a finite-alphabet probability law and a Poisson intensity, the independent Poisson count law is the product over alphabet symbols of Poisson laws with rates equal to intensity times singleton probability. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw_isProbabilityMeasure` | `MeasureTheory.IsProbabilityMeasure (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw P lambda)` | Given a finite-alphabet probability law and a Poisson intensity, the independent Poisson count law is a probability law. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finitePoissonSampleLaw_map_histogram` | `MeasureTheory.Measure.map (fun s => Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.finiteSampleHistogram s.points) (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambda) = Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw P lambda` | Given a finite-alphabet probability law and a Poisson intensity, the histogram of its finite Poisson sample has the product law of independent Poisson cell counts. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.PairedHistogram.HistogramReconstruction.Reconstruction -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.multinomialHistogramLaw` | `(P : MeasureTheory.Measure X) → [MeasureTheory.IsProbabilityMeasure P] → ℕ → MeasureTheory.Measure (X → ℕ)` | Given a finite-alphabet probability law and a fixed sample size, the multinomial histogram law is the count-vector image of the ordered iid product law. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.multinomialHistogramLaw_comp_reconstruction` | `(Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.multinomialHistogramLaw P n).bind ⇑(Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X) = MeasureTheory.Measure.map (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.fixedSizeEmbed n) (MeasureTheory.Measure.pi fun x => P)` | Given a finite-alphabet probability law and a fixed sample size, uniformly reconstructing its multinomial histogram recovers the embedded ordered iid sample law exactly. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw_comp_reconstruction` | `(Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw P lambda).bind ⇑(Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.histogramReconstructionKernel X) = Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambda` | Given a finite-alphabet probability law and a Poisson intensity, passing its independent cell counts through the parameter-free reconstruction kernel recovers the ordered finite Poisson sample law exactly. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.PairedHistogram.HistogramReconstruction.PairedTransport -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramReconstructionKernel` | `(X : Type u_1) → (Y : Type u_2) → [Fintype X] → [inst : MeasurableSpace X] → [MeasurableSingletonClass X] → [DecidableEq X] → [Fintype Y] → [inst_4 : MeasurableSpace Y] → [MeasurableSingletonClass Y] → [DecidableEq Y] → ProbabilityTheory.Kernel ((X → ℕ) × (Y → ℕ)) (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y)` | On two finite measurable alphabets, the paired histogram reconstruction kernel independently reconstructs the two ordered samples from their two count vectors. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramReconstructionKernel_isMarkovKernel` | `ProbabilityTheory.IsMarkovKernel (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramReconstructionKernel X Y)` | On two finite measurable alphabets, the independent paired histogram reconstruction channel is a Markov kernel. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedIndependentPoissonCountLaw_comp_reconstruction` | `((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw P lambdaX).prod (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw Q lambdaY)).bind ⇑(Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.pairedHistogramReconstructionKernel X Y) = (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambdaX).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw Q lambdaY)` | Given two finite-alphabet probability laws and possibly different Poisson intensities, independently reconstructing their product count law recovers the product of their ordered finite Poisson sample laws exactly. |
| `Stat.FiniteRaoBlackwell.PairedPoissonHistogram.tvDist_paired_finitePoissonSampleLaw_le_counts` | `Causalean.Stat.tvDist ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw PX0 lambdaX0).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw PY0 lambdaY0)) ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw PX1 lambdaX1).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw PY1 lambdaY1)) ≤ Causalean.Stat.tvDist ((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw PX0 lambdaX0).prod (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw PY0 lambdaY0)) ((Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw PX1 lambdaX1).prod (Causalean.Stat.FiniteRaoBlackwell.PairedPoissonHistogram.independentPoissonCountLaw PY1 lambdaY1))` | Given two pairs of probability laws on possibly different finite alphabets and arbitrary, potentially unequal Poisson intensities, the total variation distance between the paired ordered Poisson experiments is no larger than the distance between their paired independent count experiments. |
<!-- /GEN -->
...
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools` | `ℕ → ℕ → Type (max uX uY)` | Given the first pool capacity and the second pool capacity, two fixed observation pools are given by ordered pools whose capacities may differ. They are represented by the product of the two ordered pools. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools` | `ℕ → ℕ → Type (max uX uY)` | Given the first pool capacity and the second pool capacity, the randomized two-pool experiment attaches an independent requested prefix length to each fixed pool. It is represented by the pair of pool-length pairs. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw` | `(P : MeasureTheory.Measure X) → [MeasureTheory.IsProbabilityMeasure P] → (Q : MeasureTheory.Measure Y) → [MeasureTheory.IsProbabilityMeasure Q] → (NX NY : ℕ) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools NX NY)` | Given a probability law for the first observations, a probability law for the second observations, the first pool capacity, and the second pool capacity, the independent fixed-pool law is the product of the two iid pool laws. It is given by their product measure. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.randomizedPoolsLaw` | `(P : MeasureTheory.Measure X) → [MeasureTheory.IsProbabilityMeasure P] → (Q : MeasureTheory.Measure Y) → [MeasureTheory.IsProbabilityMeasure Q] → NNReal → NNReal → (NX NY : ℕ) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY)` | Given probability laws for the first and second observations, the first Poisson intensity, the second Poisson intensity, the first pool capacity, and the second pool capacity, the randomized-pool law has independent iid pools and independent Poisson requested lengths. It is given by the product of both pool-count laws. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw` | `(P : MeasureTheory.Measure X) → [MeasureTheory.IsProbabilityMeasure P] → (Q : MeasureTheory.Measure Y) → [MeasureTheory.IsProbabilityMeasure Q] → NNReal → NNReal → MeasureTheory.Measure (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y)` | Given a probability law for the first observations, a probability law for the second observations, the first Poisson intensity, and the second Poisson intensity, the independent Poisson-prefix law is the joint law of two untruncated iid prefixes. It is given by their product law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet` | `(NX NY : ℕ) → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY)` | Given the first pool capacity and the second pool capacity, the joint nonoverflow event says that both requested prefixes fit in their respective pools. It is given by the conjunction of the two capacity bounds. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.overflowSet` | `(NX NY : ℕ) → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY)` | Given the first pool capacity and the second pool capacity, the overflow event says that at least one requested prefix is too long. It is given by the union of the two marginal overflow events. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.measurableSet_nonoverflowSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet NX NY)` | Given the first pool capacity and the second pool capacity, the joint nonoverflow event is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.measurableSet_overflowSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.overflowSet NX NY)` | Given the first pool capacity and the second pool capacity, the union of the two overflow events is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet_eq_compl_overflowSet` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet NX NY = (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.overflowSet NX NY)ᶜ` | Given the first pool capacity and the second pool capacity, joint nonoverflow is exactly the complement of either marginal overflow. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.totalizedPrefixPair` | `Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X → Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY → Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y` | Given a fallback first prefix, a fallback second prefix, and a randomized two-pool outcome, the totalized prefix pair retains both requested prefixes when possible and uses the corresponding fallback otherwise. It is given by applying the one-pool totalization in each coordinate. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.measurable_totalizedPrefixPair` | `Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.totalizedPrefixPair overflowX overflowY)` | Given a fallback first prefix and a fallback second prefix, the totalized prefix pair is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic` | `(Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y → ℝ) → ℝ → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY → ℝ` | Given a two-prefix statistic, a fallback value, and a randomized two-pool outcome, the capped-prefix statistic uses the genuine prefixes when both fit and the fallback when either overflows. It is given by the two capacity tests. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.measurable_cappedPrefixStatistic` | `Measurable T → ∀ (zOver : ℝ), Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic T zOver)` | Given a measurable two-prefix statistic and a fallback value, the capped-prefix statistic is measurable on the two fixed pools and their independent requested lengths. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic_eq_off_overflow` | `∀ z ∉ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.overflowSet NX NY, Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic T zOver z = T (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.totalizedPrefixPair overflowX overflowY z)` | Given a two-prefix statistic, a fallback value, fallback first and second prefixes, a randomized two-pool outcome, and a certificate of no overflow, the capped statistic equals the statistic of the corresponding totalized prefixes. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.map_independent_streams_eq_independentPoissonPrefixLaw` | `MeasureTheory.Measure.map (Prod.map Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.streamToFiniteSample Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.streamToFiniteSample) ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.poissonIIDStreamLaw P lambdaX).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.poissonIIDStreamLaw Q lambdaY)) = Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY` | Given a probability law for the first stream, a probability law for the second stream, the first Poisson intensity, and the second Poisson intensity, mapping independent iid streams to finite prefixes gives exactly the product untruncated-prefix law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.map_totalizedPrefixPair_restrict_nonoverflow` | `MeasureTheory.Measure.map (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.totalizedPrefixPair overflowX overflowY) ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.randomizedPoolsLaw P Q lambdaX lambdaY NX NY).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet NX NY)) = (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY).restrict ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NX) ×ˢ (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NY))` | Given probability laws for the first and second pools, the first and second Poisson intensities, the first and second pool capacities, and fallback first and second prefixes, mapping the jointly nonoverflowing fixed pools to their totalized prefixes gives the correspondingly restricted product Poisson-prefix law. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.IndependentPrefix.Risk -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonCountKernel` | `NNReal → NNReal → ProbabilityTheory.Kernel (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools NX NY) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomizedPools NX NY)` | Given the first Poisson intensity and the second Poisson intensity, the independent Poisson-count kernel keeps both fixed pools and independently attaches one requested length to each. It is given by the product Poisson mechanism with the output rearranged into pool-count pairs. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonCountKernel.isMarkovKernel` | `ProbabilityTheory.IsMarkovKernel (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonCountKernel lambdaX lambdaY)` | Given the first Poisson intensity and the second Poisson intensity, the independent Poisson-count kernel is a Markov kernel. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonCountKernel_comp_fixedPoolsLaw` | `(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q NX NY).bind ⇑(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonCountKernel lambdaX lambdaY) = Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.randomizedPoolsLaw P Q lambdaX lambdaY NX NY` | Given probability laws for the first and second pools, the first and second Poisson intensities, and the first and second pool capacities, mixing the independent count kernel over the fixed pools gives exactly the randomized-pool law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic` | `NNReal → NNReal → (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y → ℝ) → ℝ → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools NX NY → ℝ` | Given the first Poisson intensity, the second Poisson intensity, a two-prefix statistic, and a fallback value, the independent-prefix Rao--Blackwell statistic averages the capped statistic over the two auxiliary Poisson counts conditional on the fixed pools. It is given by the kernel mean. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.measurable_independentPrefixRaoBlackwellStatistic` | `Measurable T → ∀ (zOver : ℝ), Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic lambdaX lambdaY T zOver)` | Given the first and second Poisson intensities, a measurable two-prefix statistic, and a fallback value, the independent-prefix Rao--Blackwell statistic is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic_eq_integral` | `Measurable T → ∀ (zOver : ℝ) (pools : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools NX NY), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic lambdaX lambdaY T zOver pools = ∫ (counts : ℕ × ℕ), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic T zOver ((pools.1, counts.1), pools.2, counts.2) ∂(ProbabilityTheory.poissonMeasure lambdaX).prod (ProbabilityTheory.poissonMeasure lambdaY)` | Given the first and second Poisson intensities, a two-prefix statistic, a certificate that it is measurable, a fallback value, and fixed pools, the Rao--Blackwell statistic equals the integral of the capped statistic over the independent Poisson counts. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.sqRisk_independentPrefixRaoBlackwellStatistic_le_capped` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q NX NY) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic lambdaX lambdaY T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.randomizedPoolsLaw P Q lambdaX lambdaY NX NY) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic T zOver) theta` | Given probability laws for the first and second observations, the first and second Poisson intensities, the first and second pool capacities, a measurable two-prefix statistic, an ordered outcome interval, a certificate that the statistic stays in that interval, a target in the interval, and a fallback in the interval, conditional averaging cannot increase squared risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.sqRisk_cappedPrefixStatistic_restrict_nonoverflow_eq` | `Measurable T → ∀ (zOver theta : ℝ), Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.randomizedPoolsLaw P Q lambdaX lambdaY NX NY).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.nonoverflowSet NX NY)) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.cappedPrefixStatistic T zOver) theta = Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY).restrict ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NX) ×ˢ (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NY))) T theta` | Given probability laws for the first and second pools, the first and second Poisson intensities, the first and second pool capacities, a measurable two-prefix statistic, a fallback value, and a target, the capped risk on the joint nonoverflow event equals the restricted untruncated-prefix risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.sqRisk_independentPoissonPrefixLaw_restrict_nonoverflow_le` | `Measurable T → Causalean.Stat.UniformlyBounded T → ∀ (theta : ℝ), Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY).restrict ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NX) ×ˢ (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Iic NY))) T theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY) T theta` | Given probability laws for the first and second prefixes, the first and second Poisson intensities, the first and second capacities, a measurable two-prefix statistic, a uniform bound for that statistic, and a target, restricting to jointly nonoverflowing prefix lengths cannot increase squared risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.sqRisk_independentPrefixRaoBlackwellStatistic_le` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q NX NY) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic lambdaX lambdaY T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY) T theta + (b - a) ^ 2 * ((ProbabilityTheory.poissonMeasure lambdaX).real (Set.Ioi NX) + (ProbabilityTheory.poissonMeasure lambdaY).real (Set.Ioi NY))` | Given probability laws for the first and second observations, the first and second Poisson intensities, the first and second pool capacities, a measurable two-prefix statistic, an ordered outcome interval, a certificate that the statistic stays in that interval, a target in the interval, and a fallback in the interval, the fixed-pool Rao--Blackwell risk is bounded by the untruncated Poisson-prefix risk plus the squared interval diameter times the two upper tails. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.finiteAlphabet_independentUnequalPrefix_risk_le` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X × Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample Y), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q NX NY) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPrefixRaoBlackwellStatistic lambdaX lambdaY T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.independentPoissonPrefixLaw P Q lambdaX lambdaY) T theta + (b - a) ^ 2 * ((ProbabilityTheory.poissonMeasure lambdaX).real (Set.Ioi NX) + (ProbabilityTheory.poissonMeasure lambdaY).real (Set.Ioi NY))` | Given finite measurable alphabets, probability laws for the two observations, the first and second Poisson intensities, the first and second pool capacities, a measurable two-prefix statistic, an ordered outcome interval, a certificate that the statistic stays in it, a target in it, and a fallback in it, the unequal-prefix risk transfer bound holds without coordinatewise pairing. |
<!-- /GEN -->
...
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool` | `(X : Type uX) → (Y : Type uY) → [MeasurableSpace X] → [MeasurableSpace Y] → Type (max uX uY)` | Given the first observation space and the second observation space, the raw two-pool observation consists of two ordered finite records with unrelated realized counts. It is represented by their product. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment` | `ProbabilityTheory.Kernel Theta (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y) → (P : ProbabilityTheory.Kernel Theta X) → (Q : ProbabilityTheory.Kernel Theta Y) → [∀ (theta : Theta), MeasureTheory.IsProbabilityMeasure (P theta)] → [∀ (theta : Theta), MeasureTheory.IsProbabilityMeasure (Q theta)] → (Theta → NNReal) → NNReal → NNReal → Prop` | Given a raw experiment kernel, the first mark kernel, the second mark kernel, a shared nonnegative scale, and two intensity multipliers, the shared-scale two-pool experiment property says that each parameter selects two independent marked Poisson records with the stated scaled intensities. It is given by the fibrewise product-law identity. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsFixedTwoPoolExperiment` | `(n m : ℕ) → ProbabilityTheory.Kernel Theta (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m) → (P : ProbabilityTheory.Kernel Theta X) → (Q : ProbabilityTheory.Kernel Theta Y) → [∀ (theta : Theta), MeasureTheory.IsProbabilityMeasure (P theta)] → [∀ (theta : Theta), MeasureTheory.IsProbabilityMeasure (Q theta)] → Prop` | Given the first fixed sample size, the second fixed sample size, a fixed-pool experiment kernel, the first mark kernel, and the second mark kernel, the fixed unequal-pool experiment property says that each parameter selects the corresponding independent iid product law. It is given by the fibrewise product-law identity. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet` | `ℕ → ℕ → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y)` | Given the first requested prefix length and the second requested prefix length, the retention-success event is that both raw records contain their respective requested numbers of marks. It is given by the conjunction of the two count bounds. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet` | `ℕ → ℕ → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y)` | Given the first requested prefix length and the second requested prefix length, the retention-failure event is that at least one raw record is too short. It is given by the union of the two lower-count events. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurableSet_retentionSuccessSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet n m)` | Given the first requested prefix length and the second requested prefix length, the retention-success event is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurableSet_retentionFailureSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m)` | Given the first requested prefix length and the second requested prefix length, the retention-failure event is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet_eq_compl_failureSet` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet n m = (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m)ᶜ` | Given the first requested prefix length and the second requested prefix length, success is exactly the complement of retention failure. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.orderedPrefix` | `(Fin n → X) → Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample X → Fin n → X` | Given a fallback fixed array and a finite record, the ordered prefix retains its initial requested segment when available and otherwise returns the fallback. It is given by the count test. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurable_orderedPrefix` | `Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.orderedPrefix fallback)` | Given a fallback fixed array, ordered prefix retention is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.twoPoolOrderedRetention` | `(Fin n → X) → (Fin m → Y) → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m` | Given a fallback first array and a fallback second array, the two-pool ordered-retention map applies ordered prefix retention in each coordinate. It is given by the pair of retained prefixes. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurable_twoPoolOrderedRetention` | `Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.twoPoolOrderedRetention fallbackX fallbackY)` | Given a fallback first array and a fallback second array, the unequal two-pool ordered-retention map is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.orderedRetentionKernel` | `(Fin n → X) → (Fin m → Y) → ProbabilityTheory.Kernel (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m)` | Given a fallback first array and a fallback second array, the ordered-record retention kernel deterministically applies two-pool ordered retention. It is given by the Dirac kernel at that measurable map. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.map_orderedPrefix_restrict_count_ge` | `MeasureTheory.Measure.map (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.orderedPrefix fallback) ((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambda).restrict (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.FiniteSample.count ⁻¹' Set.Ici n)) = (ProbabilityTheory.poissonMeasure lambda) (Set.Ici n) • MeasureTheory.Measure.pi fun x => P` | Given a mark law, a Poisson intensity, and a fallback fixed array, the pushforward of the successful-count restriction has the unnormalized iid prefix law, with its scalar mass equal to the Poisson success probability. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.map_twoPoolOrderedRetention_restrict_success` | `MeasureTheory.Measure.map (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.twoPoolOrderedRetention fallbackX fallbackY) (((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambdaX).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw Q lambdaY)).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet n m)) = ((ProbabilityTheory.poissonMeasure lambdaX) (Set.Ici n) * (ProbabilityTheory.poissonMeasure lambdaY) (Set.Ici m)) • Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q n m` | Given the first mark law, the second mark law, the first Poisson intensity, the second Poisson intensity, the first requested length, the second requested length, a fallback first array, and a fallback second array, ordered retention on joint success has the unnormalized exact product law, including the complete auxiliary iid law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.normalized_map_twoPoolOrderedRetention_restrict_success` | `(ProbabilityTheory.poissonMeasure lambdaX) (Set.Ici n) * (ProbabilityTheory.poissonMeasure lambdaY) (Set.Ici m) ≠ 0 → ((ProbabilityTheory.poissonMeasure lambdaX) (Set.Ici n) * (ProbabilityTheory.poissonMeasure lambdaY) (Set.Ici m))⁻¹ • MeasureTheory.Measure.map (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.twoPoolOrderedRetention fallbackX fallbackY) (((Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw P lambdaX).prod (Causalean.Mathlib.Probability.FiniteMarkedPoissonPartition.finitePoissonSampleLaw Q lambdaY)).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionSuccessSet n m)) = Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.fixedPoolsLaw P Q n m` | Given the first mark law, the second mark law, the first Poisson intensity, the second Poisson intensity, the first requested length, the second requested length, a fallback first array, a fallback second array, and positive joint-success mass, normalizing retained successful records gives the exact fixed product law, preserving the full auxiliary distribution. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.IndependentPrefix.RandomScaleTransfer.Failure -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.conditional_retentionFailure_le_poissonLowerTails` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment K P Q S u v → ∀ (n m : ℕ) (theta : Theta), (K theta) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m) ≤ (ProbabilityTheory.poissonMeasure (u * S theta)) (Set.Iio n) + (ProbabilityTheory.poissonMeasure (v * S theta)) (Set.Iio m)` | Given a raw experiment kernel, the first mark kernel, the second mark kernel, a shared scale, the first intensity multiplier, the second intensity multiplier, the raw-experiment law, the first requested length, the second requested length, and a parameter value, conditional retention failure is bounded by the sum of the two Poisson lower tails. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.priorPredictive_retentionFailure_le_scaleFloor_add_integratedTails` | `Measurable S → ∀ (u v s0 : NNReal), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment K P Q S u v → ∀ (n m : ℕ), (Causalean.Stat.Minimax.MomentMatchedMixture.priorPredictive pi K) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m) ≤ pi {theta \| S theta < s0} + ∫⁻ (theta : Theta) in {theta \| s0 ≤ S theta}, (ProbabilityTheory.poissonMeasure (u * S theta)) (Set.Iio n) + (ProbabilityTheory.poissonMeasure (v * S theta)) (Set.Iio m) ∂pi` | Given a parameter prior, a raw experiment kernel, the first mark kernel, the second mark kernel, a measurable shared scale, the two intensity multipliers, a deterministic scale floor, the raw-experiment law, the first requested length, and the second requested length, prior-predictive retention failure is bounded by low-scale mass plus the integrated high-scale Poisson tails. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.priorPredictive_retentionFailure_le` | `Measurable S → ∀ (u v s0 : NNReal), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment K P Q S u v → ∀ (n m : ℕ) (epsScale epsX epsY : ENNReal), pi {theta \| S theta < s0} ≤ epsScale → (∀ (theta : Theta), s0 ≤ S theta → (ProbabilityTheory.poissonMeasure (u * S theta)) (Set.Iio n) ≤ epsX) → (∀ (theta : Theta), s0 ≤ S theta → (ProbabilityTheory.poissonMeasure (v * S theta)) (Set.Iio m) ≤ epsY) → (Causalean.Stat.Minimax.MomentMatchedMixture.priorPredictive pi K) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m) ≤ epsScale + epsX + epsY` | Given a parameter prior, a raw experiment kernel, the first mark kernel, the second mark kernel, a measurable shared scale, the two intensity multipliers, a deterministic scale floor, the raw-experiment law, the first requested length, the second requested length, a scale-error budget, a first-tail budget, a second-tail budget, the scale concentration bound, the first uniform tail bound, and the second uniform tail bound, prior-predictive retention failure is at most the sum of the three error budgets. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.IndependentPrefix.RandomScaleTransfer.Risk -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.decisionRisk` | `ProbabilityTheory.Kernel Theta Z → (Theta → A → ENNReal) → (Z → A) → Theta → ENNReal` | Given an experiment kernel, a nonnegative loss, a decision rule, and a parameter value, the decision risk is the expected loss under that parameter's experiment law. It is given by its lower integral. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk` | `MeasureTheory.Measure Theta → ProbabilityTheory.Kernel Theta Z → (Theta → A → ENNReal) → (Z → A) → ENNReal` | Given a parameter prior, an experiment kernel, a nonnegative loss, and a decision rule, the Bayes decision risk averages the rule's parameterwise risks. It is given by the prior lower integral. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.worstCaseDecisionRisk` | `ProbabilityTheory.Kernel Theta Z → (Theta → A → ENNReal) → (Z → A) → ENNReal` | Given an experiment kernel, a nonnegative loss, and a decision rule, the worst-case decision risk is the largest parameterwise risk. It is given by the supremum over parameters. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk` | `ProbabilityTheory.Kernel Theta Z → (Theta → A → ENNReal) → ENNReal` | Given an experiment kernel and a nonnegative loss, the minimax decision risk is the smallest worst-case risk attainable by a measurable rule. It is given by the infimum over measurable rules. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.transferredRawRule` | `(n m : ℕ) → (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m → A) → (Fin n → X) → (Fin m → Y) → A → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y → A` | Given the first requested length, the second requested length, a fixed sample rule, a fallback first array, a fallback second array, and a fallback action, the transferred raw rule uses retained prefixes on count success and the fallback action otherwise. It is given by the success test. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurable_transferredRawRule` | `Measurable rule → ∀ (fallbackX : Fin n → X) (fallbackY : Fin m → Y) (fallbackA : A), Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.transferredRawRule n m rule fallbackX fallbackY fallbackA)` | Given the first requested length, the second requested length, a measurable fixed-sample rule, a fallback first array, a fallback second array, and a fallback action, the transferred raw rule is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.measurable_decisionRisk` | `Measurable (Function.uncurry loss) → ∀ {rule : Z → A}, Measurable rule → Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.decisionRisk K loss rule)` | Given an s-finite experiment kernel, a jointly measurable loss, and a measurable rule, parameterwise decision risk is measurable as a function of the parameter. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.decisionRisk_transferredRawRule_le` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment Kraw P Q S u v → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsFixedTwoPoolExperiment n m Kfixed P Q → ∀ (loss : Theta → A → ENNReal) (B : ENNReal), B ≠ ⊤ → Measurable (Function.uncurry loss) → (∀ (theta : Theta) (a : A), loss theta a ≤ B) → ∀ (rule : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m → A), Measurable rule → ∀ (fallbackX : Fin n → X) (fallbackY : Fin m → Y) (fallbackA : A) (theta : Theta), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.decisionRisk Kraw loss (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.transferredRawRule n m rule fallbackX fallbackY fallbackA) theta ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.decisionRisk Kfixed loss rule theta + B * (Kraw theta) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m)` | Given a raw experiment, a fixed-pool experiment, the first mark kernel, the second mark kernel, a shared scale, two intensity multipliers, the raw experiment law, the fixed experiment law, a loss, a finite loss bound, joint measurability of the loss, a measurable fixed rule, a fallback first array, a fallback second array, a fallback action, and a parameter value, the transferred raw-rule risk is at most fixed risk plus bounded failure probability. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk_transferredRawRule_le` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment Kraw P Q S u v → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsFixedTwoPoolExperiment n m Kfixed P Q → ∀ (loss : Theta → A → ENNReal) (B : ENNReal), B ≠ ⊤ → Measurable (Function.uncurry loss) → (∀ (theta : Theta) (a : A), loss theta a ≤ B) → ∀ (rule : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m → A), Measurable rule → ∀ (fallbackX : Fin n → X) (fallbackY : Fin m → Y) (fallbackA : A), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi Kraw loss (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.transferredRawRule n m rule fallbackX fallbackY fallbackA) ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi Kfixed loss rule + B * (Causalean.Stat.Minimax.MomentMatchedMixture.priorPredictive pi Kraw) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m)` | Given a parameter prior, a raw experiment, a fixed-pool experiment, the first mark kernel, the second mark kernel, a shared scale, two intensity multipliers, the raw experiment law, the fixed experiment law, a loss, a finite loss bound, joint measurability of the loss, a measurable fixed rule, a fallback first array, a fallback second array, and a fallback action, the raw transferred-rule Bayes risk is bounded by fixed Bayes risk plus prior-predictive failure cost. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk_le_worstCase` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi K loss rule ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.worstCaseDecisionRisk K loss rule` | Given a probability prior, an experiment kernel, a nonnegative loss, and a decision rule, Bayes decision risk is no larger than worst-case decision risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk_transferredRawRule_le_worstCase_add` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment Kraw P Q S u v → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsFixedTwoPoolExperiment n m Kfixed P Q → ∀ (loss : Theta → A → ENNReal) (B eps : ENNReal), B ≠ ⊤ → Measurable (Function.uncurry loss) → (∀ (theta : Theta) (a : A), loss theta a ≤ B) → (Causalean.Stat.Minimax.MomentMatchedMixture.priorPredictive pi Kraw) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.retentionFailureSet n m) ≤ eps → ∀ (rule : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FixedPools n m → A), Measurable rule → ∀ (fallbackX : Fin n → X) (fallbackY : Fin m → Y) (fallbackA : A), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi Kraw loss (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.transferredRawRule n m rule fallbackX fallbackY fallbackA) ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.worstCaseDecisionRisk Kfixed loss rule + B * eps` | Given a parameter prior, a raw experiment, a fixed-pool experiment, the first mark kernel, the second mark kernel, a shared scale, two intensity multipliers, the raw experiment law, the fixed experiment law, a loss, a finite loss bound, a failure-probability budget, joint measurability of the loss, a measurable fixed rule, a fallback first array, a fallback second array, and a fallback action, the transferred raw Bayes risk is at most fixed worst-case risk plus bounded failure cost. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteDesign.RaoBlackwell.Poisson.IndependentPrefix.RandomScaleTransfer.Main -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk_lower_of_rawBayesTransfer` | `(∀ (rule : { f // Measurable f }), rawLower ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.worstCaseDecisionRisk Kfixed loss ↑rule + penalty) → smallFloor ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk Kfixed loss → max (rawLower - penalty) smallFloor ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk Kfixed loss` | Given a fixed experiment kernel, a loss, a raw Bayes lower bound, a transfer penalty, a small-signal fixed-sample floor, a rulewise raw-to-fixed transfer bound, and the small-signal floor bound, the fixed experiment's minimax risk is at least the larger of the penalized raw bound and the small-signal floor. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.randomScale_twoFuzzy_minimax_lower_transfer` | `Measurable S → ∀ (u v s0 : NNReal), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsRandomScaleTwoPoolExperiment Kraw P Q S u v → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.IsFixedTwoPoolExperiment n m Kfixed P Q → ∀ (loss : Theta → A → ENNReal) (B : ENNReal), B ≠ ⊤ → Measurable (Function.uncurry loss) → (∀ (theta : Theta) (a : A), loss theta a ≤ B) → ∀ (fallbackX : Fin n → X) (fallbackY : Fin m → Y) (fallbackA : A) (rawLower smallFloor epsScale epsX epsY : ENNReal), (∀ (rawRule : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.RawTwoPool X Y → A), Measurable rawRule → rawLower ≤ max (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi0 Kraw loss rawRule) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.bayesDecisionRisk pi1 Kraw loss rawRule)) → pi0 {theta \| S theta < s0} ≤ epsScale → pi1 {theta \| S theta < s0} ≤ epsScale → (∀ (theta : Theta), s0 ≤ S theta → (ProbabilityTheory.poissonMeasure (u * S theta)) (Set.Iio n) ≤ epsX) → (∀ (theta : Theta), s0 ≤ S theta → (ProbabilityTheory.poissonMeasure (v * S theta)) (Set.Iio m) ≤ epsY) → smallFloor ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk Kfixed loss → max (rawLower - B * (epsScale + epsX + epsY)) smallFloor ≤ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.RandomScaleMinimaxTransfer.minimaxDecisionRisk Kfixed loss` | Given two fuzzy parameter priors, a raw experiment, a fixed two-pool experiment, the first mark kernel, the second mark kernel, a measurable shared scale, two intensity multipliers, a scale floor, the raw experiment law, the fixed experiment law, a loss, a finite loss bound, joint measurability of the loss, fallback first and second arrays, a fallback action, a raw lower bound, a small-signal floor, a scale-error budget, two Poisson-tail budgets, the fixed unequal-pool minimax risk is at least the larger of the penalized fuzzy-prior lower bound and the small-signal floor. |
<!-- /GEN -->
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.RawMoment -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.MomentProblems.rawMoment` | `MeasureTheory.Measure ℝ → ℕ → ℝ` | Given a real measure and a nonnegative integer order, the raw moment is given by integrating the corresponding power. This is the real-valued Bochner integral, which is zero when the power is not integrable, so it is the moment only when the moment exists. |
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.Basic -->
| Decl | Signature | Description |
|---|---|---|
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.OrthogonalPerturbation -->
| Decl | Signature | Description |
|---|---|---|
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.DensityPerturbation -->
| Decl | Signature | Description |
|---|---|---|
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.Carleman -->
| Decl | Signature | Description |
|---|---|---|
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.CumulantTransfer -->
| Decl | Signature | Description |
|---|---|---|
...
<!-- GEN:Causalean.Stat.Nonparametric.MomentProblems.GaussianPerturbation.Main -->
| Decl | Signature | Description |
|---|---|---|
...
| Decl | Signature | Description |
|---|---|---|
| `Stat.finAverage` | `(Fin n → ℝ) → ℝ` | Given a real vector of length `n`, its finite average is `1/n` times the sum of its coordinates, using Lean's reciprocal convention at `n = 0`. |
| `Stat.integral_finAverage_bootstrapResample` | `n ≠ 0 → ∫ (y : Fin n → ℝ), Causalean.Stat.finAverage y ∂Causalean.Stat.bootstrapResample x = Causalean.Stat.finAverage x` | For real data with nonzero length, the bootstrap expectation of the resample mean equals the original data mean. |
...
| `Stat.centeredBootstrapSum_eq_sqrt_mul_finAverage_sub` | `n ≠ 0 → ∀ (ψ : X → ℝ) (x xstar : Fin n → X), Causalean.Stat.centeredBootstrapSum ψ x xstar = √↑n * ((Causalean.Stat.finAverage fun i => ψ (xstar i)) - Causalean.Stat.finAverage fun i => ψ (x i))` | With a nonzero sample size, an observation statistic, observed data, and a resample, the centered bootstrap sum equals the square-root-scaled difference between their averages. |
...
| `Stat.integral_sq_centeredEmpiricalLaw_eq_empiricalVar` | `n ≠ 0 → ∀ (S : Causalean.Stat.IIDSample Ω X μ P) (ψ : X → ℝ), Measurable ψ → ∀ (ω : Ω), ∫ (y : ℝ), y ^ 2 ∂Causalean.Stat.centeredEmpiricalLaw S ψ n ω = S.empiricalVar ψ n ω` | With a nonzero row size, an iid sample, an observation statistic that is measurable, and a realized outcome, the centered empirical row law's second moment equals the plug-in empirical variance. |
...
| `Stat.populationVariance` | `(X → ℝ) → MeasureTheory.Measure X → NNReal` | Given an observation statistic and a population probability measure, the nonnegative population variance is the variance of the statistic represented as a nonnegative real number for use as a Gaussian variance parameter. |
<!-- /GEN -->
...
### OLS bootstrap conditions from moments

For random-design OLS, the closed-form coefficient map turns the bootstrap weak law for the
resampled raw moments into conditional consistency and exact normal equations with probability
tending to one. The resulting contrast and percentile-interval theorems require no resampling-side
assumption from a user.

<!-- GEN:Causalean.Stat.Bootstrap.SmoothZEstimator.OLSConditions -->
<!-- /GEN -->
...
| `Stat.Minimax.FiniteSideInformation.worstCaseRisk_comp_surjective` | `Function.Surjective phi → Causalean.Stat.worstCaseRisk (fun e a => risk e (phi a)) e = Causalean.Stat.worstCaseRisk risk e` | Given a risk, a procedure, and a surjective parameter map, reindexing preserves the deprecated real worst-case risk. |
| `Stat.Minimax.FiniteSideInformation.minimaxValue_comp_surjective` | `Function.Surjective phi → (Causalean.Stat.minimaxValue fun e a => risk e (phi a)) = Causalean.Stat.minimaxValue risk` | Given a risk and a surjective parameter map, reindexing preserves the deprecated real minimax value. |
...
| `Stat.Minimax.FiniteSideInformation.minimaxValueOfReal_comp_surjective` | `Function.Surjective phi → (Causalean.Stat.minimaxValueOfReal fun e a => risk e (phi a)) = Causalean.Stat.minimaxValueOfReal risk` | Given a real-valued risk, a parameter map, and surjectivity, reindexing leaves its lifted minimax value unchanged. |
...
## `SCM/Examples/IV.lean` — IV DAG Example
...
## `SCM/Examples/BackDoor.lean` — Backdoor Adjustment Example
...
## `SCM/Examples/ContinuousBackdoor.lean` — Linear-Gaussian backdoor (continuous treatment)
