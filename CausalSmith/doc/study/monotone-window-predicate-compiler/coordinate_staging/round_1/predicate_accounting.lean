...
/-- [A fixed predicate-pass family](goal) assigns one predicate window schedule to each member of
[a finite pass count](hyp:passes), all over [the same raw keys](hyp:keys) and [the same ordered
steps](hyp:steps). -/
