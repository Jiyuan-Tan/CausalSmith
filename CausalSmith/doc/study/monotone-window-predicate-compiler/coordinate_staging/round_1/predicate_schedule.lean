...
/-- [A predicate window schedule](goal) on [a sparse raw-key list](hyp:keys) and [an ordered step
list](hyp:steps) records prefix-shaped entry predicates, suffix-shaped stay predicates, and the two
predicate-inclusion laws along the step order. -/
