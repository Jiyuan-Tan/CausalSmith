...
/-- [A raw push batch](goal) over [a raw-key type](hyp:ι) records the final key deque, the pushed
keys, and the back-popped keys. -/
structure RawPushBatch (ι : Type*) where
  state : List ι
  pushed : List ι
  backPopped : List ι

/-- [A raw score](hyp:score), [an initial raw deque](hyp:q), and [a key list](hyp:xs) determine
[the recorded batch of rightmost-stable pushes](goal). -/
...
/-- [A raw predicate-scan state](goal) over [a raw-key type](hyp:ι) consists of the unentered suffix
and the current raw-key deque. -/
structure RawState (ι : Type*) where
  pending : List ι
  deque : List ι

/-- [A raw deque trace](goal) over [a raw-key type](hyp:ι) records the lists corresponding exactly
to the list-valued fields of one generic position-level deque update. -/
...
/-- [A raw predicate-scan step trace](goal) over [a raw-key type](hyp:ι) and [a step type](hyp:κ)
records the step, pending suffix, and complete take-while entry segment before filtering keys
already expired at that step. -/
