...
### Predicate-defined sparse-key scans

Sparse key arrays often describe feasibility directly with entry and stay predicates rather than
with integer endpoints.  These modules compile prefix/suffix predicates into the verified position
schedule, identify the raw-key trace with the generic trace, and transfer argmax and linear-resource
bounds back to raw keys.
