...
[uses the independent compatible-ordering average when both totals are large enough](step:1),
[returns the fallback when the first total is too small](step:2), and
[returns the fallback when only the second total is too small](step:3). -/
