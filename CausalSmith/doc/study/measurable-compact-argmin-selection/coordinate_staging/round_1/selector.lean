...
/-- A [standard Borel parameter space](hyp:X) and [finite-dimensional Euclidean action space](hyp:ι)
with [a fixed nonempty compact action set](hyp:K,hK,hKne), a [jointly Borel objective](hyp:f,hf),
and [continuous action sections on that set](hyp:hfc) admit [a Borel measurable feasible rule that
minimizes the objective at every parameter](goal).
...
/-- A [finite-dimensional Euclidean ambient space](hyp:ι) with a [fixed nonempty compact
subset](hyp:K,hK,hKne) admits [a total Borel measurable nearest-point rule whose chosen point is
feasible and realizes the exact distance infimum](goal).
