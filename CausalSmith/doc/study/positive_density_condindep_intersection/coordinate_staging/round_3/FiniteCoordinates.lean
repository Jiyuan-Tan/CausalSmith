...
/-- For [four finite coordinate blocks](hyp:I,J,K,L) with [all pairwise overlaps excluded](hyp:hIJ,hIK,hIL,hJK,hJL,hKL),
...
positivity of that density almost everywhere](hyp:hpos), and [the two conditional independence
relations conditioning on the alternate coordinate blocks](hyp:hIJ_given_LK,hIK_given_LJ), [the
coordinates in the first block are conditionally independent of the second block given the fourth](goal). -/
