...
### Measurable exact-table bridge

This companion layer treats a bounded procedure defined on ambient real
side-probability coordinates as a Borel-measurable statistical rule.  It
extends the continuous conditional average from the probability simplex and
uses finite-product integral identities to compare its minimax value directly
with the existing exact-side and empirical-side experiments.
