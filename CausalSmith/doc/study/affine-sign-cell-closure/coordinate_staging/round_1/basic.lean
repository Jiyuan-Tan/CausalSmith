...
/-- Given [a finite number of coordinates](hyp:n), an [affine real-valued function](goal)
is specified by [one coefficient for each coordinate](step:1) and [one constant term](step:2).
...
/-- Given [a finite number of coordinates](hyp:n), a [marked affine constraint](goal)
contains [its normalized affine left-hand side](step:1) and [its weak-or-strict mark](step:2).
