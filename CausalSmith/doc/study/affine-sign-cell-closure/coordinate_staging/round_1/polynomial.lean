...
/-- Given [a finite number of coordinates](hyp:n), a [checked polynomial constraint](goal)
contains [its polynomial left-hand side](step:1), [its weak-or-strict mark](step:2), and [a
certificate that it is affine](step:3).
