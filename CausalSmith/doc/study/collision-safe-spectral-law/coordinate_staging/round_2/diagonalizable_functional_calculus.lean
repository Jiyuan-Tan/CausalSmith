...
import Causalean.Mathlib.Analysis.MatrixEuclidean
...
/-- [Separate real diagonalizations of two matrices](hyp:DA,DB), [a one-Lipschitz scalar function](hyp:f,hf), and [upper bounds on their diagonalizer condition numbers](hyp:hκA,hκB) give [a collision-safe operator bound for the difference of their functional-calculus matrices](goal).

The bound counts distinct eigenvalues and makes no eigenvalue matching or eigengap assumption. -/
