...
/-- [Two finite atomic laws](hyp:μ,ν) whose [weights are nonnegative and sum to one](hyp:hμ,hν)
have [a transport plan attaining their finite one-Wasserstein cost](goal). -/
