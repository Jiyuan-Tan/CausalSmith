...
/-- [Two real rectangular matrices](hyp:A,B) with [the same specified rank](hyp:hrankA,hrankB) and [a positive common lower singular-value margin](hyp:hs,hmarginA,hmarginB) have [Moore--Penrose inverses whose difference is bounded by three times the inverse squared margin times their operator difference](goal).

Neither row space nor column space is held fixed. -/
