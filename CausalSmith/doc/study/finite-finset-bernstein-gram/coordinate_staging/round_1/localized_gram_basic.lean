...
/-- The unnormalised local count is the sum of the observation weights. -/
def localCount {N : ℕ} {X : Type*} (q : X → ℝ) (omega : Fin N → X) : ℝ :=
  ∑ i, q (omega i)

/-- An entry of the unnormalised weighted empirical Gram matrix. -/
def localGramEntry {N : ℕ} {κ X : Type*} [Fintype κ]
    (q : X → ℝ) (phi : κ → X → ℝ) (omega : Fin N → X) (j k : κ) : ℝ :=
  ∑ i, q (omega i) * phi j (omega i) * phi k (omega i)

/-- The quadratic form of the unnormalised weighted empirical Gram matrix. -/
def localGramQuadratic {N : ℕ} {κ X : Type*} [Fintype κ]
    (q : X → ℝ) (phi : κ → X → ℝ) (omega : Fin N → X)
    (v : κ → ℝ) : ℝ :=
  ∑ j, ∑ k, v j * v k * localGramEntry q phi omega j k

/-- A localized empirical Gram matrix is good when its local count is positive and its
quadratic form dominates one half of `lambda` times that realized count. -/
