...
/-- The affine `2 × 2` determinant of three shift points in two selected coordinates. -/
def pairAffineDet {p : ℕ} {E : Type*} (s : E → Fin p → ℝ)
    (i j : Fin p) (e₀ e₁ e₂ : E) : ℝ :=
  (s e₁ i - s e₀ i) * (s e₂ j - s e₀ j) -
    (s e₂ i - s e₀ i) * (s e₁ j - s e₀ j)

/-- A shift family is uniformly pairwise affinely separated by `δ` when every distinct
coordinate pair has three, possibly pair-dependent, environments whose affine determinant
has absolute value at least `δ`. -/
def PairwiseAffineSeparated {p : ℕ} {E : Type*} [Fintype E]
    (s : E → Fin p → ℝ) (δ : ℝ) : Prop :=
  ∀ i j, i ≠ j → ∃ e₀ e₁ e₂ : E, δ ≤ |pairAffineDet s i j e₀ e₁ e₂|

/-- A candidate has off-diagonal simultaneous-congruence residual at most `ε` when every
off-diagonal entry of every transformed matrix has absolute value at most `ε`.  Its
diagonal output is deliberately left unrestricted. -/
...
/-- The transition error from an invertible reference to a candidate is
`B B₀⁻¹ - I`. -/
def transitionError {p : ℕ} (B₀ B : SqMatrix p) : SqMatrix p :=
  transition B₀ B - 1

/-- The entrywise Euclidean, or Frobenius, size of a real square matrix. -/
def entryL2 {p : ℕ} (R : SqMatrix p) : ℝ :=
  Real.sqrt (∑ i, ∑ j, |R i j| ^ 2)

/-- The conservative two-coordinate Cramer amplification.  The factor six leaves room
for centering and triangle inequalities while retaining the requested specialization. -/
def pairwiseSolveFactor (M δ : ℝ) : ℝ :=
  6 * M / δ

/-- The aggregate entrywise-L² amplification obtained from all ordered coordinate pairs
and unit-diagonal normalization. -/
def pairwiseAggregateFactor (p : ℕ) (M δ L : ℝ) : ℝ :=
  pairwiseSolveFactor M δ *
    Real.sqrt ((p : ℝ) * (p - 1 : ℕ) * (1 + L ^ 2))

/-- The displayed operator-norm stability constant used by the condition-number
specialization. -/
def pairwiseStabilityConstant (p : ℕ) (M δ L : ℝ) : ℝ :=
  16 * pairwiseAggregateFactor p M δ L * L ^ 3

/-- The residual threshold used in the local branch-selection argument.  Its use of
`max 1` also covers the one-dimensional case, where the ordered-pair aggregate vanishes. -/
def pairwiseResidualRadius (p : ℕ) (M δ L : ℝ) : ℝ :=
  1 / (32 * (max 1 (pairwiseAggregateFactor p M δ L)) ^ 2 * M)

/-- The entrywise-L² neighborhood radius around the reference from which the quadratic
pairwise estimate can be bootstrapped onto the identity branch.  The parameter `J` is an
upper bound for the reference inverse's Euclidean operator norm. -/
def pairwiseLocalRadius (p : ℕ) (M δ L J : ℝ) : ℝ :=
  min 1 (3 / (8 * max 1 (pairwiseAggregateFactor p M δ L) * M)) /
    ((p : ℝ) * J)

/-- The determinant/condition root
`((p!) κ^(p-1))^(1/p)` used to bound a unit-diagonal matrix's operator norm. -/
def conditionRoot (p : ℕ) (κ : ℝ) : ℝ :=
  ((Nat.factorial p : ℝ) * κ ^ (p - 1)) ^ (1 / (p : ℝ))

/-- A reference/candidate pair has Euclidean operator norm at most `L`. -/
def PairMatrixNormBound {p : ℕ} (L : ℝ) (B₀ B : SqMatrix p) : Prop :=
  ‖B₀‖ ≤ L ∧ ‖B‖ ≤ L

/-- A candidate lies in the declared local reference neighborhood when its entrywise-L²
distance from the reference is at most `ρ`. -/
def InReferenceNeighborhood {p : ℕ} (ρ : ℝ) (B₀ B : SqMatrix p) : Prop :=
  entryL2 (B - B₀) ≤ ρ

/-- A matrix has the determinant and condition envelope used to derive `conditionRoot`:
it is invertible, its determinant is at most `p!` in absolute value, and its Euclidean
operator condition number is at most `κ`. -/
def DetConditionEnvelope {p : ℕ} (κ : ℝ) (B : SqMatrix p) : Prop :=
  IsUnit B.det ∧ |B.det| ≤ (Nat.factorial p : ℝ) ∧
    operatorConditionNumber B ≤ κ

/-- Both the reference and candidate obey the same determinant/condition envelope. -/
def PairDetConditionEnvelope {p : ℕ} (κ : ℝ) (B₀ B : SqMatrix p) : Prop :=
  DetConditionEnvelope κ B₀ ∧ DetConditionEnvelope κ B

/-- The explicit local identity-branch conditions used to absorb the quadratic transition
remainder: the transition error has entrywise L² size at most one and its quadratic
coefficient is at most one half. -/
