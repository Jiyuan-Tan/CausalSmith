...
/-- With [a nonzero row size](hyp:hn), [an iid sample](hyp:S), [an observation statistic](hyp:ψ)
that [is measurable](hyp:hψ), and [a realized outcome](hyp:ω), [the centered empirical row
law's second moment equals the plug-in empirical variance](goal). -/
...
/-- Given [an iid sample](hyp:S), [a measurable observation statistic](hyp:ψ,hψ), [a row size](hyp:n),
and [a realized outcome](hyp:ω), the [conditional bootstrap law of the centered normalized mean](goal)
is Efron's resampling law at positive sizes and a point mass at zero at size zero. -/
...
/-- Given [an observation statistic](hyp:ψ) and [a population probability measure](hyp:P), the
