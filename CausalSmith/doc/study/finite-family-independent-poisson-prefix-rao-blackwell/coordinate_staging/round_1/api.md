...
### Finite-family independent Poisson-prefix transfer

This extension retains separate iid pools and independent Poisson prefix lengths for any finite
heterogeneous family. It supplies the exact joint prefix law, a capped fixed-pool implementation,
a summed-overflow risk bound, and a directly usable three-pool specialization in which two pools
may share an alphabet and law without sharing their randomization.

<!-- GEN:Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.Basic -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.FixedPools` | `(X : I → Type uX) → [(i : I) → MeasurableSpace (X i)] → (I → ℕ) → Type (max uI uX)` | Given a family of observation alphabets and pool capacities, fixed pools give each coordinate an ordered iid observation pool. They are represented by the dependent family of coordinate pools. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools` | `(X : I → Type uX) → [(i : I) → MeasurableSpace (X i)] → (I → ℕ) → Type (max (max uI 0) uX)` | Given a family of observation alphabets and pool capacities, the randomized pool family appends one requested prefix length to each coordinate pool. It is represented by the dependent family of pool-length pairs. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily` | `(X : I → Type uX) → [(i : I) → MeasurableSpace (X i)] → Type (max uI uX)` | Given a family of observation alphabets, a prefix family contains one variable-length finite sample for each coordinate. It is represented by the dependent family of finite samples. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PoissonStreamFamily` | `(X : I → Type uX) → [(i : I) → MeasurableSpace (X i)] → Type (max (max uI uX) 0)` | Given a family of observation alphabets, a Poisson stream family gives each coordinate a requested count and an infinite observation stream. It is represented by the dependent family of count-stream pairs. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.fixedPoolsLaw` | `(P : (i : I) → MeasureTheory.Measure (X i)) → [∀ (i : I), MeasureTheory.IsProbabilityMeasure (P i)] → (N : I → ℕ) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.FixedPools X N)` | Given coordinatewise probability laws and pool capacities, the fixed-pool law makes the observations iid within and independent across pools. It is given by the finite product of coordinatewise iid laws. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw` | `(P : (i : I) → MeasureTheory.Measure (X i)) → [∀ (i : I), MeasureTheory.IsProbabilityMeasure (P i)] → (I → NNReal) → (N : I → ℕ) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N)` | Given coordinatewise probability laws, Poisson intensities, and pool capacities, the randomized-pool law makes pools, counts, and coordinates mutually independent. It is given by the finite product of pool-count laws. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw` | `(P : (i : I) → MeasureTheory.Measure (X i)) → [∀ (i : I), MeasureTheory.IsProbabilityMeasure (P i)] → (I → NNReal) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X)` | Given coordinatewise probability laws and Poisson intensities, the independent Poisson-prefix law is the joint law of all untruncated iid prefixes. It is given by the finite product of coordinatewise Poisson sample laws. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonStreamLaw` | `(P : (i : I) → MeasureTheory.Measure (X i)) → [∀ (i : I), MeasureTheory.IsProbabilityMeasure (P i)] → (I → NNReal) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PoissonStreamFamily X)` | Given coordinatewise probability laws and Poisson intensities, the independent count-and-stream law independently supplies a count and iid stream in every coordinate. It is given by the finite product of coordinatewise stream laws. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet` | `(N : I → ℕ) → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N)` | Given pool capacities, the joint nonoverflow event is the event that every requested prefix fits its coordinate pool. It is given by the coordinatewise capacity bounds. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet` | `(N : I → ℕ) → Set (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N)` | Given pool capacities, the overflow event is the event that at least one requested prefix exceeds its coordinate pool. It is given by the union of marginal overflow events. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurableSet_prefixNonoverflowSet` | `MeasurableSet {s \| ∀ (i : I), (s i).count ≤ N i}` | Given pool capacities, the event that every untruncated prefix fits its coordinate capacity is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurableSet_nonoverflowSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet N)` | Given pool capacities, the joint nonoverflow event is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurableSet_overflowSet` | `MeasurableSet (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet N)` | Given pool capacities, the union of marginal overflow events is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet_eq_compl_overflowSet` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet N = (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet N)ᶜ` | Given pool capacities, joint nonoverflow is the complement of the overflow event. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.streamFamilyToPrefixFamily` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X` | Given a count-and-stream family, its prefix family retains the requested initial segment of every stream. It is given by coordinatewise stream truncation. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurable_streamFamilyToPrefixFamily` | `Measurable Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.streamFamilyToPrefixFamily` | Conversion of count-and-stream families to prefix families is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.map_independentPoissonStreamLaw_eq_independentPoissonPrefixLaw` | `MeasureTheory.Measure.map Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.streamFamilyToPrefixFamily (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonStreamLaw P lambda) = Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda` | Given coordinatewise probability laws and Poisson intensities, mapping independent count-and-stream coordinates to prefixes gives exactly the independent Poisson-prefix law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.totalizedPrefixFamily` | `Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X` | Given fallback prefixes and a randomized pool family, the totalized prefix family uses genuine prefixes when available and the corresponding fallback otherwise. It is given by coordinatewise one-pool totalization. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurable_totalizedPrefixFamily` | `Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.totalizedPrefixFamily overflow)` | Given fallback prefixes, coordinatewise totalization is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic` | `(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X → ℝ) → ℝ → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N → ℝ` | Given a prefix statistic, an overflow value, and a randomized pool family, the capped-prefix statistic uses genuine prefixes precisely when every request fits. It is given by the joint capacity test. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurable_cappedPrefixStatistic` | `Measurable T → ∀ (zOver : ℝ), Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic T zOver)` | Given a measurable prefix statistic and an overflow value, the capped-prefix statistic is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic_eq_off_overflow` | `∀ z ∉ Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet N, Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic T zOver z = T (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.totalizedPrefixFamily overflow z)` | Given a prefix statistic, an overflow value, fallback prefixes, a randomized pool family, and evidence of no overflow, the capped statistic equals the statistic of the totalized prefixes. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.map_totalizedPrefixFamily_restrict_nonoverflow` | `MeasureTheory.Measure.map (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.totalizedPrefixFamily overflow) ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw P lambda N).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet N)) = (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda).restrict {s \| ∀ (i : I), (s i).count ≤ N i}` | Given coordinatewise probability laws, Poisson intensities, pool capacities, and fallback prefixes, mapping jointly nonoverflowing randomized pools to totalized prefixes gives the restricted untruncated product law. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.Risk -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonCountKernel` | `(I → NNReal) → ProbabilityTheory.Kernel (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.FixedPools X N) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.RandomizedPools X N)` | Given Poisson intensities, the independent Poisson-count kernel keeps every fixed pool and independently attaches one requested length in every coordinate. It is given by the product Poisson mechanism. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonCountKernel.isMarkovKernel` | `ProbabilityTheory.IsMarkovKernel (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonCountKernel lambda)` | Given Poisson intensities, the independent Poisson-count kernel is a Markov kernel. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonCountKernel_comp_fixedPoolsLaw` | `(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.fixedPoolsLaw P N).bind ⇑(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonCountKernel lambda) = Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw P lambda N` | Given coordinatewise probability laws, Poisson intensities, and pool capacities, mixing the count kernel over fixed pools gives the randomized-pool law. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic` | `(I → NNReal) → (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X → ℝ) → ℝ → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.FixedPools X N → ℝ` | Given Poisson intensities, a prefix statistic, and an overflow value, the prefix Rao--Blackwell statistic averages the capped statistic over all requested lengths conditional on fixed pools. It is given by the count-kernel mean. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.measurable_prefixRaoBlackwellStatistic` | `Measurable T → ∀ (zOver : ℝ), Measurable (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic lambda T zOver)` | Given Poisson intensities, a measurable prefix statistic, and an overflow value, the prefix Rao--Blackwell statistic is measurable. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic_eq_integral` | `Measurable T → ∀ (zOver : ℝ) (pools : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.FixedPools X N), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic lambda T zOver pools = ∫ (counts : I → ℕ), Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic T zOver fun i => (pools i, counts i) ∂MeasureTheory.Measure.pi fun i => ProbabilityTheory.poissonMeasure (lambda i)` | Given Poisson intensities, a prefix statistic, evidence that it is measurable, an overflow value, and fixed pools, the Rao--Blackwell statistic equals its explicit count integral. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.sqRisk_prefixRaoBlackwellStatistic_le_capped` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.fixedPoolsLaw P N) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic lambda T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw P lambda N) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic T zOver) theta` | Given coordinatewise probability laws, Poisson intensities, pool capacities, a measurable prefix statistic, an ordered outcome interval, a statistic confined to that interval, a target in it, and an overflow value in it, conditional averaging cannot increase squared risk relative to the capped randomized experiment. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.sqRisk_cappedPrefixStatistic_restrict_nonoverflow_eq` | `Measurable T → ∀ (zOver theta : ℝ), Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw P lambda N).restrict (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.nonoverflowSet N)) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.cappedPrefixStatistic T zOver) theta = Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda).restrict {s \| ∀ (i : I), (s i).count ≤ N i}) T theta` | Given coordinatewise probability laws, Poisson intensities, pool capacities, a measurable prefix statistic, an overflow value, and a target, capped risk on joint nonoverflow equals restricted untruncated-prefix risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.sqRisk_independentPoissonPrefixLaw_restrict_nonoverflow_le` | `Measurable T → Causalean.Stat.UniformlyBounded T → ∀ (theta : ℝ), Causalean.Stat.sqRisk ((Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda).restrict {s \| ∀ (i : I), (s i).count ≤ N i}) T theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda) T theta` | Given coordinatewise probability laws, Poisson intensities, pool capacities, a measurable prefix statistic, a uniform bound, and a target, restricting to coordinatewise nonoverflow cannot increase untruncated Poissonized squared risk. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet_measureReal_le_sum_poisson_tails` | `(Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.randomizedPoolsLaw P lambda N).real (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.overflowSet N) ≤ ∑ i, (ProbabilityTheory.poissonMeasure (lambda i)).real (Set.Ioi (N i))` | Given coordinatewise probability laws, Poisson intensities, and pool capacities, the probability of any overflow is bounded by the sum of marginal Poisson upper-tail probabilities. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.sqRisk_prefixRaoBlackwellStatistic_le` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily X), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.fixedPoolsLaw P N) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic lambda T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P lambda) T theta + (b - a) ^ 2 * ∑ i, (ProbabilityTheory.poissonMeasure (lambda i)).real (Set.Ioi (N i))` | Given coordinatewise probability laws, Poisson intensities, pool capacities, a measurable prefix statistic, an ordered outcome interval, a statistic confined to that interval, a target in it, and an overflow value in it, fixed-pool Rao--Blackwell risk is bounded by untruncated Poissonized risk plus squared interval diameter times the summed upper tails. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePool -->
| Decl | Signature | Description |
|---|---|---|
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex` | `Type` | The three-pool index distinguishes one complete pool from two independently randomized shared-alphabet pools. Its complete coordinate is available, its left shared coordinate is available, and its right shared coordinate is available. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.instDecidableEqThreePoolIndex` | `DecidableEq Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex` | Decidable equality for the three-pool index distinguishes exactly its equal and unequal coordinate labels. It is given by the nine constructor comparisons. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.instFintypeThreePoolIndex` | `Fintype Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex` | The three-pool index has a finite enumeration. The enumeration is given and its completeness certificate is given. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet` | `Type uX → Type uX → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex → Type uX` | Given the complete-pool alphabet and the shared-pool alphabet, the three-pool alphabet family assigns the latter independently to both shared coordinates. It is given by the coordinate case split. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolAlphabetMeasurableSpace` | `(i : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex) → MeasurableSpace (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B i)` | Given a complete-pool alphabet and a shared-pool alphabet, the coordinatewise measurable-space structure is inherited from the corresponding alphabet. It is given by the three coordinate cases. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolAlphabetFintype` | `(i : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex) → Fintype (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B i)` | Given a complete-pool alphabet and a shared-pool alphabet, the coordinatewise finite-type structure is inherited from the corresponding alphabet. It is given by the three coordinate cases. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolAlphabetDecidableEq` | `(i : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex) → DecidableEq (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B i)` | Given a complete-pool alphabet and a shared-pool alphabet, the coordinatewise decidable-equality structure is inherited from the corresponding alphabet. It is given by the three coordinate cases. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolAlphabetMeasurableSingletonClass` | `MeasurableSingletonClass (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B i)` | Given a complete-pool alphabet and a shared-pool alphabet, the coordinatewise measurable-singleton structure is inherited from the corresponding alphabet. It is given by the three coordinate cases. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolObservationLaw` | `MeasureTheory.Measure A → MeasureTheory.Measure B → (i : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex) → MeasureTheory.Measure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B i)` | Given the complete-pool law and the shared-pool law, the three-pool observation-law family assigns the shared law separately to both shared coordinates. It is given by the coordinate case split. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolObservationLawIsProbabilityMeasure` | `MeasureTheory.IsProbabilityMeasure (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolObservationLaw P Q i)` | Given the complete-pool law, the shared-pool law, and a coordinate, the assigned observation law is a probability measure. It is given by the coordinate case split. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolCapacity` | `ℕ → ℕ → ℕ → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex → ℕ` | Given the complete-pool capacity, the left shared-pool capacity, and the right shared-pool capacity, the three-pool capacity function records all three without identifying the shared pools. It is given by the coordinate case split. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolIntensity` | `NNReal → NNReal → NNReal → Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolIndex → NNReal` | Given the complete-pool intensity, the left shared-pool intensity, and the right shared-pool intensity, the three-pool intensity function records all three independently. It is given by the coordinate case split. |
| `Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.finiteAlphabet_threePoolSharedLaw_risk_le` | `Measurable T → ∀ {a b theta zOver : ℝ}, a ≤ b → (∀ (s : Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.PrefixFamily (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.ThreePoolAlphabet A B)), T s ∈ Set.Icc a b) → theta ∈ Set.Icc a b → zOver ∈ Set.Icc a b → let P₃ := Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolObservationLaw P Q; have lambda₃ := Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolIntensity lambdaComplete lambdaLeft lambdaRight; have N₃ := Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.threePoolCapacity NComplete NLeft NRight; Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.fixedPoolsLaw P₃ N₃) (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.prefixRaoBlackwellStatistic lambda₃ T zOver) theta ≤ Causalean.Stat.sqRisk (Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily.independentPoissonPrefixLaw P₃ lambda₃) T theta + (b - a) ^ 2 * ((ProbabilityTheory.poissonMeasure lambdaComplete).real (Set.Ioi NComplete) + (ProbabilityTheory.poissonMeasure lambdaLeft).real (Set.Ioi NLeft) + (ProbabilityTheory.poissonMeasure lambdaRight).real (Set.Ioi NRight))` | Given the complete-pool law, the shared-pool law, three Poisson intensities, three pool capacities, a measurable prefix statistic, an ordered outcome interval, a statistic confined to it, a target in it, and an overflow value in it, the three-pool Rao--Blackwell risk is bounded by untruncated risk plus three upper-tail terms. The finite-family bound supplies this conclusion. |
<!-- /GEN -->

<!-- GEN:Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily -->
_(no documented declarations in Causalean.Stat.FiniteRaoBlackwell.IndependentPoissonPrefix.FiniteFamily)_
<!-- /GEN -->
