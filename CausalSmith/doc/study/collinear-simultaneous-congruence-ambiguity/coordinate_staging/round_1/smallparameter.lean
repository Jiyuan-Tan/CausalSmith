...
/-- For a [matrix dimension and a reference diagonalizer and invariant](hyp:d,B,Ω), [two
distinct selected coordinates](hyp:i,j,hij), a [positive-definite invariant](hyp:hΩ), and
[line coefficients](hyp:u,v,c), [there is a positive radius on which every transformed
invariant is positive definite](goal). -/
...
/-- For a [matrix dimension, reference diagonalizer, selected coordinates, and line
coefficients](hyp:d,B,i,j,u,v), [there is a positive radius on which both normalization
denominators and the shear determinant are nonzero](goal). -/
...
[reference diagonalizer and invariant](hyp:B,Ω), [two selected coordinates](hyp:i,j), and
[a distinct-coordinate condition](hyp:hij), if [the diagonalizer is unit-diagonal](hyp:hBdiag),
[invertible](hyp:hBunit), and [cycle-admissible](hyp:hcycle), [the invariant is positive
definite](hyp:hΩ), [the shifts](hyp:s) are [certified affine-collinear](hyp:cert) and
[nonnegative](hyp:hs), and [a positive upper bound is given](hyp:r,hr), [a nonzero smaller
parameter simultaneously preserves every stated algebraic, positivity, and nonnegativity
condition](goal). -/
