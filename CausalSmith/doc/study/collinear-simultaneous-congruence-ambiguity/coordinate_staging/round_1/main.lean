...
import Causalean.Discovery.LinearDisentanglement.CollinearAmbiguity.Congruence
...
/-- Given a [finite-dimensional, finite nonempty covariance family](hyp:d,E), a
[reference diagonalizer and invariant matrix](hyp:B₀,Ω₀), [its diagonal shift vectors](hyp:s),
and [two selected coordinates](hyp:i,j), if [the coordinates are distinct](hyp:hij), [the
reference has unit diagonal](hyp:hdiag), [is invertible](hyp:hunit), [satisfies the selected
cycle-product restriction](hyp:hcycle), [the invariant is positive definite](hyp:hΩ), [the
shifts are nonnegative](hyp:hs), [their selected pairs lie on a certified affine line](hyp:cert),
and [a positive deformation bound is specified](hyp:r,hr), [there is a strictly nonzero smaller
deformation with a distinct normalized invertible diagonalizer, positive-definite symmetric
invariant, nonnegative shifts, and exactly the same positive-definite covariance family](goal). -/
...
/-- Given a [finite-dimensional, finite nonempty nonnegative shift family](hyp:d,E,s,hs) and
[two distinct selected coordinates whose shift pairs lie on a certified affine line](hyp:i,j,hij,cert),
[there is a positive-definite covariance family with the prescribed shifts and two distinct
normalized invertible diagonalizers that represent it](goal). -/
