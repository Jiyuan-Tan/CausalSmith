...
/-!
# Lindeberg--Feller central limit theorem for i.i.d. rows

This module proves weak convergence for triangular arrays whose `n`-th row consists of `n`
independent draws from a row-dependent real law.  It provides both the variance-scaled form
and the normalized-sum form used by bootstrap arguments.
-/

@[expose] public section

namespace Causalean.Stat.CLT.LindebergFeller

open Complex Filter MeasureTheory ProbabilityTheory Topology

open Filter MeasureTheory ProbabilityTheory Topology

/-- Given [a row length](hyp:n), [the i.i.d. row-sum map](goal) is [the finite sum of its
coordinates](step:1). -/
def iidRowSum (n : ℕ) : (Fin n → ℝ) → ℝ := fun y => ∑ i, y i

/-- For [a row length](hyp:n), [the corresponding finite-coordinate sum is Borel
measurable](goal). -/
lemma measurable_iidRowSum (n : ℕ) : Measurable (iidRowSum n) := by
  unfold iidRowSum
  fun_prop

/-- Given [a sequence of row laws](hyp:Q) and [a row index](hyp:n), [the i.i.d. row-sum
law](goal) is [the product-law pushforward by the row sum](step:1). -/
...
/-- Given [row laws](hyp:Q), [a row index](hyp:n), and [a frequency](hyp:t), [the
characteristic function of the i.i.d. row sum is the corresponding power of the one-draw
...
/-- Given [a sequence of row laws](hyp:R) and [a row index](hyp:n), [the normalized i.i.d.
row-sum law](goal) is [the product-law pushforward by the sum divided by the square root of the
row length](step:1). -/
...
/-- Given [a sequence of row laws](hyp:R) and [a row index](hyp:n), [the scaled row law](goal)
is [the image measure under division by the square root of the row length](step:1). At row zero,
the inverse convention makes the scale zero. -/
noncomputable def scaledRowMeasure (R : ℕ → Measure ℝ) (n : ℕ) : Measure ℝ :=
  (R n).map (fun x => (Real.sqrt (n : ℝ))⁻¹ * x)

/-- Given [row laws](hyp:R) and [a row index](hyp:n), [scaling every draw before summing gives
the same probability law as scaling the row sum](goal). This is the measure-theoretic bridge from
the scaled theorem to the bootstrap-facing normalized theorem. -/
...
/-- Given [row laws](hyp:R), [a row index](hyp:n), and [square integrability of that row](hyp:hR2),
[the scaled row law remains square-integrable](goal). -/
...
/-- Given [row laws](hyp:R), [a nonzero row index](hyp:n,hn), [square integrability](hyp:hR2),
and [zero row mean](hyp:hcenter), [the scaled row still has zero mean](goal). -/
lemma integral_id_scaledRowMeasure_eq_zero
  ...
  have _hn : (n : ℝ) ≠ 0 := Nat.cast_ne_zero.mpr hn
...
/-- Given [row laws](hyp:R), [a nonzero row index](hyp:n,hn), and [square integrability](hyp:hR2),
[the scaled-row second moment equals the original second moment divided by the row length](goal). -/
...
/-- Given [row laws](hyp:R), [a nonzero row index](hyp:n,hn), [square integrability](hyp:hR2),
[a positive tail threshold](hyp:ε,hε), [the scaled-row tail second moment equals the
original tail second moment above the correspondingly rescaled threshold, divided by the row
length](goal). -/
...
/-- Given [row laws](hyp:Q), [a nonnegative limiting variance](hyp:σ2), [rowwise square
integrability](hyp:hQ2), [zero row means](hyp:hcenter), [convergence of total row second
moments](hyp:hsecond), [vanishing Lindeberg tails](hyp:hlindeberg), and [a frequency](hyp:t),
[the row characteristic functions raised to their row lengths converge to the Gaussian
characteristic function](goal). -/
...
/-- Given [row laws](hyp:Q), [a nonnegative limiting variance](hyp:σ2), [rowwise square
integrability](hyp:hQ2), [zero row means](hyp:hcenter), [convergence of total row second
moments](hyp:hsecond), and [vanishing Lindeberg tails](hyp:hlindeberg), [the laws of the sums of
the rowwise independent draws converge weakly to the centered Gaussian law with that
variance](goal).
The conclusion includes the degenerate zero-variance Gaussian. -/
...
/-- Given [row laws](hyp:R), [a nonnegative limiting variance](hyp:σ2), [rowwise square
integrability](hyp:hR2), [zero row means](hyp:hcenter), [convergence of row second
moments](hyp:hsecond), and [vanishing square-root-scale Lindeberg tails](hyp:hlindeberg), [the
laws of normalized sums of the rowwise independent draws converge weakly to the centered Gaussian
law with that variance](goal). The conclusion includes the degenerate zero-variance Gaussian. -/
...
end Causalean.Stat.CLT.LindebergFeller
