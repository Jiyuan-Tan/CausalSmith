...
/-- The admissible tensor perturbation radius for the quantitative tensor-pencil inverse. With [its explicit inputs](hyp:n,q,sigma,kappa,Lambda), [the defined object](goal) is given by [the contracted margin](step:1), [the lifted condition envelope](step:2), [the pencil perturbation coefficient](step:3), and [the smaller admissible radius](step:4). -/
...
/-- The coordinatewise trace-recovery Lipschitz factor for the quantitative tensor-pencil
inverse. With [its explicit inputs](hyp:n,q,sigma,kappa,Lambda), [the defined object](goal) is given by [the contracted margin](step:1), [the lifted condition envelope](step:2), [the pencil perturbation coefficient](step:3), and [the displayed trace-recovery factor](step:4). -/
