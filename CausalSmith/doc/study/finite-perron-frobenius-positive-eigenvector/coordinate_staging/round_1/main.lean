...
This module assembles a finite-dimensional Perron--Frobenius theorem for real
symmetric irreducible nonnegative matrices.  It identifies the positive
eigenvalue with each standard top-Rayleigh presentation.
-/
...
/-- For [a real symmetric irreducible finite matrix](hyp:A,hA_symm,hA_irred),
[there is a strictly positive unit eigenvector whose eigenvalue is simultaneously
the coordinate-sphere, Euclidean-sphere, and nonzero-vector top Rayleigh value](goal). -/
theorem finite_positive_perron_eigenvector
    (A : Matrix ι ι ℝ) (hA_symm : A.IsSymm) (hA_irred : A.IsIrreducible) :
    ∃ (v : EVec ι) (ρ : ℝ),
      ‖v‖ = 1 ∧ (∀ i, 0 < v i) ∧ A.mulVec v = ρ • v ∧
      rayleighForm A v = ρ ∧ ρ = sphereRayleighValue A ∧
      ρ = coordinateSphereRayleighValue A ∧ ρ = iSupRayleighValue A := by
  ...
  obtain ⟨habs_norm, habs_nonneg, habs_top, habs_eigen⟩ :=
    absVec_preserves_top_eigenvector A hA_symm hA_irred.nonneg hx_norm hx_eigen hx_top
  refine ⟨absVec x, sphereRayleighValue A, habs_norm, ?_, habs_eigen, habs_top, rfl, ?_, ?_⟩
  · exact IsIrreducible.unit_eigenvector_pos hA_irred habs_nonneg habs_norm habs_eigen
  · exact (coordinateSphereRayleighValue_eq_sphereRayleighValue A).symm
  · exact sphereRayleighValue_eq_iSupRayleighValue A

/-- For [a symmetric finite matrix](hyp:A,hA_symm), [a nonempty finite coordinate
set](hyp:s), [an irreducible restricted block](hyp:hA_irred), and [block closure
from outside rows into that set](hyp:hclosed), [the block's strictly positive unit
Perron vector zero-extends to a global eigenvector](goal). -/
theorem finite_positive_perron_eigenvector_on_restriction
  ...
  obtain ⟨v, ρ, hv_norm, hv_pos, hv_eigen, _, hρ_sphere, hρ_coordinate, hρ_iSup⟩ :=
    finite_positive_perron_eigenvector (restrictMatrix A s) (restrictMatrix_isSymm hA_symm s) hA_irred
