...
This module isolates the graph-theoretic Perron--Frobenius step: a nonzero
nonnegative eigenvector of an irreducible nonnegative finite matrix is strictly
positive in every coordinate.  It does not require symmetry.
-/
...
/-- For [an entrywise nonnegative matrix](hyp:A,hA_nonneg), [a nonnegative
eigenvector](hyp:x,hx_nonneg,hx_eigen), [a row coordinate where it vanishes](hyp:i,hxi),
and [a strictly positive matrix entry from that row](hyp:j,hAij), [the eigenvector
...
/-- For [an irreducible finite matrix](hyp:hA), [a nonnegative nonzero
eigenvector](hyp:x,hx_nonneg,hx_ne,hx_eigen), [every coordinate is strictly
positive](goal). -/
...
/-- For [an irreducible finite matrix](hyp:hA) and [a normalized nonnegative
eigenvector](hyp:x,hx_nonneg,hx_norm,hx_eigen), [every coordinate is strictly
positive](goal). -/
theorem IsIrreducible.unit_eigenvector_pos
    ...
    (hx_eigen : A.mulVec x = ρ • x) :
    ∀ i, 0 < x i := by
  apply IsIrreducible.eigenvector_pos hA hx_nonneg _ hx_eigen
  intro hx
  simp [hx] at hx_norm
