...
This module proves that coordinatewise absolute value preserves Euclidean norm
and top-Rayleigh maximality for entrywise nonnegative matrices.  For symmetric
matrices it therefore produces a nonnegative top eigenvector.
-/
...
/-- For [a Euclidean coordinate vector](hyp:x), [coordinatewise absolute value
preserves its Euclidean norm](goal). -/
@[simp] theorem norm_absVec (x : EVec ι) : ‖absVec x‖ = ‖x‖ := by
  apply (sq_eq_sq₀ (norm_nonneg _) (norm_nonneg _)).mp
  simp [EuclideanSpace.real_norm_sq_eq, absVec]

/-- For [a Euclidean coordinate vector](hyp:x) and [a coordinate](hyp:i), [the
corresponding coordinatewise absolute value is nonnegative](goal). -/
theorem absVec_nonneg (x : EVec ι) (i : ι) : 0 ≤ absVec x i := by
  simp [absVec]

/-- For [an entrywise nonnegative finite real matrix](hyp:A,hA) and [a Euclidean
coordinate vector](hyp:x), [taking coordinatewise absolute values cannot lower
the Rayleigh form](goal). -/
...
/-- For [a radius](hyp:r) and [a Euclidean coordinate vector on the sphere of
that radius](hyp:x,hx), [coordinatewise absolute value remains on that
same sphere](goal). -/
...
/-- For [an entrywise nonnegative finite real matrix](hyp:A,hA), [a unit-sphere
vector](hyp:x,hx), and [its Rayleigh-form maximality](hyp:hmax), [taking
coordinatewise absolute values preserves the Rayleigh-form value](goal). -/
...
/-- For [an entrywise nonnegative finite real matrix](hyp:A,hA), [a unit-sphere
vector](hyp:x,hx), and [its Rayleigh-form maximality](hyp:hmax), [coordinatewise
absolute value is another unit-sphere maximizer](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst), an [entrywise nonnegative
symmetric real matrix](hyp:A,hA_symm,hA_nonneg), [a unit-sphere vector](hyp:x,hx),
and [its Rayleigh-form maximality](hyp:hmax), [the coordinatewise absolute vector
is an eigenvector at the top Rayleigh value](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst), an [entrywise nonnegative
symmetric real matrix](hyp:A,hA_symm,hA_nonneg), and [a normalized top
eigenvector](hyp:x,hx_norm,hx_eigen,hx_top), [coordinatewise absolute value is a
normalized nonnegative top eigenvector with the same top value](goal). -/
