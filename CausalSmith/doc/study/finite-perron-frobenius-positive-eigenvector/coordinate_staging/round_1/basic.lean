...
This module gives finite real matrices a Euclidean Rayleigh-form interface and
connects its coordinate-sphere, Euclidean-sphere, and Rayleigh-quotient
presentations.  It also proves attainment of the top value for symmetric
matrices.
-/
...
/-- A Euclidean real vector [indexed by a finite coordinate type](hyp:ι), called
[a finite real coordinate vector](goal). -/
abbrev EVec (ι : Type*) [Fintype ι] := EuclideanSpace ℝ ι

/-- The Euclidean vector obtained from [a finite real vector](hyp:x) by [taking the
absolute value of every coordinate](goal). -/
def absVec (x : EVec ι) : EVec ι :=
  WithLp.toLp 2 fun i => |x i|

/-- The quadratic form associated with [a finite real matrix](hyp:A) and [a
Euclidean coordinate vector](hyp:x), called [its Rayleigh form](goal). -/
def rayleighForm (A : Matrix ι ι ℝ) (x : EVec ι) : ℝ :=
  ∑ i, x i * (A.mulVec x) i

/-- The greatest quadratic Rayleigh-form value among [the Euclidean unit
vectors](hyp:A), called [the Euclidean-sphere top Rayleigh value](goal). -/
def sphereRayleighValue (A : Matrix ι ι ℝ) : ℝ :=
  sSup {r : ℝ | ∃ x : EVec ι, ‖x‖ = 1 ∧ r = rayleighForm A x}

/-- The greatest quadratic Rayleigh-form value among [the coordinate vectors
whose squared coordinates sum to one](hyp:A), called [the coordinate-sphere top
Rayleigh value](goal). -/
def coordinateSphereRayleighValue (A : Matrix ι ι ℝ) : ℝ :=
  sSup {r : ℝ | ∃ x : ι → ℝ, (∑ i, x i ^ 2) = 1 ∧
    r = ∑ i, x i * (A.mulVec x) i}

/-- Mathlib's supremum Rayleigh quotient for [a finite real matrix](hyp:A), called
[the nonzero-vector top Rayleigh value](goal). -/
...
/-- For [a finite real matrix](hyp:A), [the coordinate and Euclidean unit-sphere
top Rayleigh values agree](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst) and [a real matrix](hyp:A),
[the Euclidean unit-sphere top value equals Mathlib's supremum Rayleigh
quotient](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst) and [a real matrix](hyp:A),
[the coordinate unit-sphere top value equals Mathlib's supremum Rayleigh
quotient](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst), a [real symmetric
matrix](hyp:A,hA) has [a unit vector attaining its Euclidean-sphere top
Rayleigh value](goal). -/
...
/-- For [a nonempty finite coordinate space](hyp:inst), a [real symmetric
matrix](hyp:A,hA) has [a unit eigenvector at its Euclidean-sphere top Rayleigh
value](goal). -/
