...
/-- For [an i.i.d. sample](hyp:S), [a real statistic](hyp:ψ) that is [measurable](hyp:hψ), [a
sample size](hyp:n) that is [nonzero](hyp:hn), and [an outcome](hyp:ω), [the variance of the
statistic under the empirical measure of the first `n` observations equals Causalean's plug-in
