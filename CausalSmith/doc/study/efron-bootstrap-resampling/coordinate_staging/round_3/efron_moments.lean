...
/-- For [a real data vector](hyp:x) with [nonzero length](hyp:hn), [the bootstrap expectation of
the resample mean equals the original data mean](goal). -/
...
/-- For [a real data vector](hyp:x) with [nonzero length](hyp:hn), [the bootstrap second moment
of the square-root-`n` centered resample mean equals the empirical centered second moment](goal). -/
