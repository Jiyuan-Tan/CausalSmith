...
/-- For [a data vector](hyp:x) whose type-level length is positive, [the empirical measure has a
probability-measure instance](goal). -/
...
/-- For [a data vector](hyp:x) whose type-level length is positive, [the Efron resampling law has
a probability-measure instance](goal). -/
...
/-- For [a real-valued sample statistic](hyp:T), [observed data](hyp:x), a statistic that is
[measurable](hyp:hT), and [nonzero data size](hyp:hn), [the bootstrap law has total mass one](goal). -/
