...
/-- For [a nonempty data vector](hyp:hn), [its empirical measure has total mass one](goal). -/
...
/-- The empirical measure of data with a type-level positive sample size is a probability
measure. -/
...
/-- For [a nonempty data vector](hyp:hn), [the Efron resampling law has total mass one](goal). -/
...
/-- The Efron resampling law at a type-level positive sample size is a probability measure. -/
...
/-- If [the statistic is measurable](hyp:hT) and [the data size is nonzero](hyp:hn), [its
bootstrap law has total mass one](goal). -/
