...
/-- If [the statistic is measurable](hyp:hT) and [the sample size is nonzero](hyp:hn), [the
bootstrap CDF is the average of the threshold indicators over all index resamples](goal). -/
...
/-- If [the statistic is measurable](hyp:hT) and [the sample size is nonzero](hyp:hn), then for
[each fixed threshold](hyp:t), [the bootstrap CDF is measurable as a function of the data](goal). -/
...
/-- If [the statistic is measurable](hyp:hT) and [the sample size is nonzero](hyp:hn), [the
bootstrap CDF is right-continuous at every threshold](goal). -/
...
/-- At [a positive level no greater than one](hyp:hβ0,hβ1), for [a measurable statistic](hyp:hT)
and [nonzero sample size](hyp:hn), [the lower bootstrap quantile can be computed by taking the
...
/-- At [a positive level no greater than one](hyp:hβ0,hβ1), for [a measurable statistic](hyp:hT)
and [nonzero sample size](hyp:hn), [a lower bootstrap quantile is at most a threshold exactly when
the bootstrap CDF has reached the level there](goal). -/
...
/-- For [a measurable statistic](hyp:hT), [a nonzero sample size](hyp:hn), and [any real quantile
level](hyp:β), [the lower bootstrap quantile is measurable as a function of the data](goal).
