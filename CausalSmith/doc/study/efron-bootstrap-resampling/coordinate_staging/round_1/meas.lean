...
/-- For [a statistic](hyp:T), [observed data](hyp:x), and [a threshold](hyp:t), if [the statistic
is measurable](hyp:hT) and [the sample size is nonzero](hyp:hn), [the bootstrap CDF is
...
/-- For [a statistic](hyp:T), [observed data](hyp:x), and [a positive quantile level no greater
than one](hyp:β,hβ0,hβ1), if [the statistic is measurable](hyp:hT) and [the sample size is
nonzero](hyp:hn), [the lower bootstrap quantile can be computed by taking the infimum only over
rational thresholds](goal). -/
...
/-- For [a statistic](hyp:T), [observed data](hyp:x), [a positive quantile level no greater than
one](hyp:β,hβ0,hβ1), and [a threshold](hyp:t), if [the statistic is measurable](hyp:hT) and [the
sample size is nonzero](hyp:hn), [a lower bootstrap quantile is at most that threshold exactly
when the bootstrap CDF has reached the level there](goal). -/
...
/-- For [a statistic](hyp:T) that [is measurable](hyp:hT), [a nonzero sample size](hyp:hn), and
[any real quantile level](hyp:β), [the lower bootstrap quantile is measurable as a function of
the data](goal).
