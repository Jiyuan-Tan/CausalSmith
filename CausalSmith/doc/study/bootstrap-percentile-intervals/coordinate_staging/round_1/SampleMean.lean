...
/-- For [a sample size](hyp:n) and [a real data vector](hyp:x), [the sample-mean statistic](goal)
is [the vector's finite average](step:1). -/
def sampleMeanStatistic (n : ℕ) (x : Fin n -> ℝ) : ℝ :=
  Causalean.Stat.finAverage x

/-- Given [an iid real sample](hyp:S), [an integrable population square](hyp:hX2), and
[strictly positive population variance](hyp:hvar), [the sample mean has bootstrap asymptotic
linearity at its population mean with centered-observation influence function](goal). -/
