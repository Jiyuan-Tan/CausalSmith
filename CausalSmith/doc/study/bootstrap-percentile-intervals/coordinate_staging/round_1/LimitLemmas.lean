...
public import Causalean.Stat.Bootstrap.AsymptoticLinearity.Basic
...
/-- For [three real probability laws](hyp:mu,nu,xi), [their Kolmogorov CDF distance satisfies
...
/-- Given [a sampling probability space](hyp:mu), [conditional laws](hyp:nu), [measurable
linear and remainder terms](hyp:linear,remainder,hlinearMeas,hremainderMeas), and [a continuous
target law](hyp:G,hGcont), if [the linear laws converge in Kolmogorov distance](hyp:hlinear) and
[the remainders are conditionally negligible](hyp:hremainder), [the summed conditional laws
converge in Kolmogorov distance to the target law in sampling probability](goal). -/
...
/-- Given [conditional real laws](hyp:nu), [a target law](hyp:G), [an interior quantile
level](hyp:hbeta0,hbeta1), [continuity and strict increase of the target CDF at that
quantile](hyp:hGcont,hGstrict), [measurable conditional quantiles](hyp:hqMeas), and [Kolmogorov
convergence in sampling probability](hyp:hdist), [the conditional lower quantiles converge in
...
/-- Given [measurable statistics](hyp:hSnMeas) converging in distribution to [a limiting
law](hyp:hSn), [measurable random interval endpoints](hyp:lower,upper,hlowerMeas,hupperMeas)
converging in probability to [ordered deterministic endpoints](hyp:a,b,hlower,hupper,hab), and
[a boundary-null limit](hyp:hboundary), [the random interval-coverage probabilities converge to
the limiting interval probability](goal). -/
...
[a centered Gaussian assigns probability one minus that level to its equal-tail quantile
interval](goal). -/
...
/-- Given [a positive Gaussian variance](hyp:hv) and [an interior error level](hyp:halpha0,halpha1),
[a centered Gaussian assigns probability one minus that level to the reflected equal-tail
quantile interval](goal). -/
