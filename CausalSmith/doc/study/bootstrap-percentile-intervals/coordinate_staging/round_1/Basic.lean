...
/-- For [an estimator](hyp:est), [a sample size](hyp:n), [observed data](hyp:x), and [a
same-size resample](hyp:xstar), [the centered estimator bootstrap statistic](goal) is [the
square-root-scaled difference between the resampled and observed estimates](step:1). -/
def centeredEstimatorBootstrapStatistic
    (est : (n : ℕ) -> (Fin n -> X) -> ℝ) (n : ℕ)
    (x xstar : Fin n -> X) : ℝ :=
  Real.sqrt (n : ℝ) * (est n xstar - est n x)

/-- For [an estimator](hyp:est), [a sample size](hyp:n), [a probability level](hyp:beta), and
[observed data](hyp:x), [the bootstrap quantile](goal) is [the lower quantile of the Efron law of
the centered estimator statistic](step:1). -/
...
/-- For [an estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha), and
[observed data](hyp:x), [the percentile confidence interval](goal) is [the interval obtained by
adding the two equal-tail centered-bootstrap quantiles, divided by the square root of sample size,
...
/-- For [an estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha), and
[observed data](hyp:x), [the basic confidence interval](goal) is [the reflection of the two
equal-tail centered-bootstrap quantiles around the estimate](step:1). -/
...
/-- Given [an estimator](hyp:est) whose [sample statistics are measurable](hyp:hest), [a sample
...
/-- Given [an estimator](hyp:est) with [measurable sample statistics](hyp:hest), a [positive
sample size](hyp:hn), an [interior quantile level](hyp:hbeta0,hbeta1), and [observed data](hyp:x),
[the centered bootstrap quantile is the corresponding affine transform of the ordinary bootstrap
quantile](goal). -/
...
/-- Given [an estimator](hyp:est) with [measurable sample statistics](hyp:hest), [a sample
size](hyp:n), and [an interior quantile level](hyp:hbeta0,hbeta1), [its centered bootstrap
quantile is measurable as a function of observed data](goal). -/
...
/-- Given [an iid sample](hyp:S), [an estimator](hyp:est), its [target parameter](hyp:theta0),
[measurable sample statistics](hyp:hest), and [a sample size](hyp:n), [the sampling estimator
law](goal) is [the sampling-law pushforward of the centered square-root-scaled estimator](step:1). -/
...
/-- Given [an estimator](hyp:est) with [measurable sample statistics](hyp:hest), [a sample
size](hyp:n), and [observed data](hyp:x), [the conditional bootstrap estimator law](goal) is [the
Efron law of the centered estimator statistic, totalized by a point mass at zero at size zero](step:1). -/
...
/-- Given [an estimator](hyp:est) with [measurable sample statistics](hyp:hest), a [positive
sample size](hyp:hn), an [interior quantile level](hyp:hbeta0,hbeta1), and [observed data](hyp:x),
[the bootstrap quantile is the lower quantile of the bundled conditional law](goal). -/
...
/-- An [iid sample](hyp:S), an [estimator evaluated on finite data vectors](hyp:est), its
[target parameter](hyp:theta0), and an [influence function](hyp:psi) satisfy [bootstrap
asymptotic linearity](goal) when [the statistics are measurable](hyp:meas), [the influence
function is centered](hyp:mean_zero), [its variance is positive and finite](hyp:var_pos_finite),
[the sampling remainder vanishes in probability](hyp:linear), and [the conditional bootstrap
remainder vanishes in sampling probability](hyp:boot_linear). -/
...
/-- For [a bootstrap-asymptotically-linear estimator](hyp:h), [its asymptotic variance](goal) is
[the influence function's population second moment](step:1). -/
def asymptoticVariance (h : BootstrapAsymLinear S est theta0 psi) : ℝ :=
  ∫ x, (psi x) ^ 2 ∂P

/-- For [a bootstrap-asymptotically-linear estimator](hyp:h), [its asymptotic Gaussian law](goal)
is [the centered Gaussian law with the influence function's population second moment as variance](step:1). -/
...
/-- When [an estimator has bootstrap asymptotic linearity](hyp:h), [its sample-vector
evaluation is asymptotically linear in the library's existing sense](goal). -/
...
/-- Given [bootstrap asymptotic linearity](hyp:h), [an interior nominal error level](hyp:halpha0,halpha1),
and [a sample size](hyp:n), [the percentile-interval coverage event is measurable](goal). -/
...
/-- Given [bootstrap asymptotic linearity](hyp:h), [an interior nominal error level](hyp:halpha0,halpha1),
and [a sample size](hyp:n), [the basic-interval coverage event is measurable](goal). -/
