...
/-- Given [three real probability laws](hyp:mu,nu,xi), [their Kolmogorov CDF distance satisfies
...
/-- Given [conditional laws](hyp:nu), [a linear term and its remainder](hyp:linear,remainder),
[measurability of both terms](hyp:hlinearMeas,hremainderMeas), [a limiting law](hyp:G) with [a
continuous CDF](hyp:hGcont), [Kolmogorov convergence of the linear laws](hyp:hlinear), and [a
conditionally negligible remainder](hyp:hremainder), [the conditional laws of their sums converge
to the limiting law in Kolmogorov distance in sampling probability](goal). -/
...
/-- Given [random probability laws](hyp:nu), [a limiting law](hyp:G), [an interior quantile
level](hyp:hbeta0,hbeta1), [continuity](hyp:hGcont) and [strict increase](hyp:hGstrict) of its CDF
at that quantile, [measurable random quantiles](hyp:hqMeas), and [Kolmogorov convergence in
sampling probability](hyp:hdist), [the random lower quantiles converge in sampling probability
to the limiting lower quantile](goal). -/
...
/-- Given [measurable statistics](hyp:hSnMeas) with [a distributional limit](hyp:hSn), [random
lower and upper endpoints](hyp:lower,upper), [fixed endpoints](hyp:a,b), [measurability of the
endpoints](hyp:hlowerMeas,hupperMeas), [their convergence in probability](hyp:hlower,hupper),
[the limiting order](hyp:hab), and [zero limiting boundary mass](hyp:hboundary), [the random
interval coverage probabilities converge to the fixed-interval probability](goal). -/
...
/-- Given [a positive Gaussian variance](hyp:hv) and [an interior error level](hyp:halpha0,halpha1),
[a centered Gaussian assigns probability one minus that error level to the interval between its
equal-tail quantiles](goal). -/
...
/-- Given [a positive Gaussian variance](hyp:hv) and [an interior error level](hyp:halpha0,halpha1),
[a centered Gaussian assigns probability one minus that error level to the interval obtained by
negating and reversing its equal-tail quantiles](goal). -/
