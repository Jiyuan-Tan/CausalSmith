...
/-- Given [a sample size](hyp:n) and [a finite real data vector](hyp:x), [the sample-mean
statistic](goal) is [that vector's finite average](step:1). -/
def sampleMeanStatistic (n : ℕ) (x : Fin n -> ℝ) : ℝ :=
  Causalean.Stat.finAverage x

/-- Given [an iid real sample](hyp:S) with [an integrable square](hyp:hX2) and [strictly positive
variance](hyp:hvar), [its sample mean is bootstrap asymptotically linear at the population mean
with centered-observation influence function](goal). -/
