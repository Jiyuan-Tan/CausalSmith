...
/-- Given [an estimator](hyp:est), [a sample size](hyp:n), and [observed data with a same-size
resample](hyp:x,xstar), [the centered estimator bootstrap statistic](goal) is [the
square-root-sample-size-scaled difference between the resampled and observed estimates](step:1). -/
def centeredEstimatorBootstrapStatistic
    (est : (n : ℕ) -> (Fin n -> X) -> ℝ) (n : ℕ)
    (x xstar : Fin n -> X) : ℝ :=
  Real.sqrt (n : ℝ) * (est n xstar - est n x)

/-- Given [an estimator](hyp:est), [a sample size](hyp:n), [a quantile level](hyp:beta), and
[observed data](hyp:x), [the bootstrap quantile](goal) is [the lower quantile of the
Efron-bootstrap law of the centered estimator statistic](step:1). -/
...
/-- Given [an estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha),
and [observed data](hyp:x), [the percentile bootstrap confidence interval](goal) is [the interval
obtained by adding its equal-tail centered-bootstrap quantiles, divided by the square root of the
sample size, to the estimate](step:1). -/
...
/-- Given [an estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha),
and [observed data](hyp:x), [the basic bootstrap confidence interval](goal) is [the interval
formed by reflecting the two centered-bootstrap quantiles around the estimate](step:1). -/
...
/-- Given [an estimator](hyp:est) whose statistics are [measurable](hyp:hest), [a sample
size](hyp:n), and [observed data](hyp:x), [the centered bootstrap statistic is measurable as a
function of the resample](goal). -/
...
/-- Given [an estimator](hyp:est) with [measurable statistics](hyp:hest), [a nonzero sample
size](hyp:hn), [an interior quantile level](hyp:hbeta0,hbeta1), and [observed data](hyp:x), [the
centered bootstrap quantile equals the corresponding affine transform of the ordinary bootstrap
quantile](goal). -/
...
/-- Given [an estimator](hyp:est) with [measurable statistics](hyp:hest), [a sample size](hyp:n),
and [an interior quantile level](hyp:hbeta0,hbeta1), [the centered bootstrap quantile is
...
/-- Given [an iid sample](hyp:S), [an estimator](hyp:est), [its target](hyp:theta0), [measurable
statistics](hyp:hest), and [a sample size](hyp:n), [the sampling estimator law](goal) is [the
pushforward of the sampling measure by the centered, square-root-scaled estimator](step:1). -/
...
/-- Given [an estimator](hyp:est) with [measurable statistics](hyp:hest), [a sample
size](hyp:n), and [observed data](hyp:x), [the bootstrap estimator law](goal) is [the
Efron-bootstrap law of the centered estimator statistic at positive sizes and a point mass at
zero at size zero](step:1). -/
...
/-- Given [an estimator](hyp:est) with [measurable statistics](hyp:hest), [a nonzero sample
size](hyp:hn), [an interior quantile level](hyp:hbeta0,hbeta1), and [observed data](hyp:x), [the
bootstrap quantile is the lower quantile of the bundled conditional estimator-bootstrap law](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:S,est,theta0,psi) records an iid sample, estimator,
target, and influence function whose [measurability](hyp:meas), [zero population mean](hyp:mean_zero),
[positive finite second moment](hyp:var_pos_finite), [sampling linearization](hyp:linear), and
[conditional bootstrap linearization](hyp:boot_linear). -/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h), [its asymptotic variance](goal)
is [the influence function's population second moment](step:1). -/
def asymptoticVariance (h : BootstrapAsymLinear S est theta0 psi) : ℝ :=
  ∫ x, (psi x) ^ 2 ∂P

/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h), [its asymptotic Gaussian law](goal)
is [the centered Gaussian distribution with the influence function's population second moment as
variance](step:1). -/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h), [the estimator evaluated on its
iid sample vector is asymptotically linear in the library's existing sense](goal). -/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h), [an interior nominal error
level](hyp:halpha0,halpha1), and [a sample size](hyp:n), [the event that its percentile interval
contains the target is measurable](goal). -/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h), [an interior nominal error
level](hyp:halpha0,halpha1), and [a sample size](hyp:n), [the event that its basic bootstrap
interval contains the target is measurable](goal). -/
