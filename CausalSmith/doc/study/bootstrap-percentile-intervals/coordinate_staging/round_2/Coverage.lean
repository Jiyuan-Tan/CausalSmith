...
/-!
# Bootstrap percentile and basic coverage

This module derives the asymptotic coverage of percentile and basic Efron-bootstrap confidence
intervals from bootstrap asymptotic linearity.
-/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h) and [an interior nominal error
level](hyp:halpha0,halpha1), [its percentile bootstrap confidence interval covers the target with
probability tending to one minus the error level](goal). -/
...
/-- Given [a bootstrap-asymptotically-linear estimator](hyp:h) and [an interior nominal error
level](hyp:halpha0,halpha1), [its basic bootstrap confidence interval covers the target with
probability tending to one minus the error level](goal). -/
