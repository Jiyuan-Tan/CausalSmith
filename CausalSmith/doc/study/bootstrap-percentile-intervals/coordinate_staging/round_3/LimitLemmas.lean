...
/-- [Three real probability laws](hyp:mu,nu,xi) satisfy [the triangle inequality for their
Kolmogorov CDF distance](goal). -/
theorem cdfKolmogorov_triangle
    (mu nu xi : ProbabilityMeasure ℝ) :
    Causalean.Stat.cdfKolmogorov mu xi <=
    Causalean.Stat.cdfKolmogorov mu nu +
        Causalean.Stat.cdfKolmogorov nu xi := by
...
/-- [A sequence of conditional laws](hyp:nu), [a linear term and remainder](hyp:linear,remainder),
[their conditional measurability](hyp:hlinearMeas,hremainderMeas), and [a real limiting law with
a continuous CDF](hyp:G,hGcont), together with [Kolmogorov convergence of the linear term](hyp:hlinear)
and [conditional negligibility of the remainder](hyp:hremainder), ensure that [the conditional
law of their sum converges to the same law in Kolmogorov distance in sampling probability](goal). -/
theorem conditionalSlutsky_cdfKolmogorov_inProb
    ...
    Tendsto_inProb
      ...
      (fun _ => 0) mu := by
  rw [Causalean.Stat.Tendsto_inProb_iff,
  ...
  have hshift : TendstoUniformly
    ...
    refine Causalean.Stat.tendstoUniformly_of_monotone_of_tendsto_at
  ...
  have hcdfError (rho sigma : ProbabilityMeasure ℝ) (t : ℝ) :
      |cdf (rho : Measure ℝ) t - cdf (sigma : Measure ℝ) t| ≤
        Causalean.Stat.cdfKolmogorov rho sigma := by
    unfold Causalean.Stat.cdfKolmogorov
    apply le_ciSup (c := t)
    refine ⟨1, ?_⟩
    rintro _ ⟨x, rfl⟩
  ...
  have hsumBound (n : ℕ) (omega : Omega) :
    ...
    have heMinus := hcdfError
      ((nu n omega).map (hlinearMeas n omega).aemeasurable) G (t - delta)
    unfold Causalean.Stat.cdfKolmogorov at hePlus heMinus
  ...
  have hsubset (n : ℕ) :
    ...
    have hlinNonneg := Causalean.Stat.cdfKolmogorov_nonneg
      ((nu n omega).map (hlinearMeas n omega).aemeasurable) G
    have hsumNonneg := Causalean.Stat.cdfKolmogorov_nonneg
      ((nu n omega).map
        ((hlinearMeas n omega).add (hremainderMeas n omega)).aemeasurable) G
    have hlinlt : Causalean.Stat.cdfKolmogorov
...
/-- [Random probability laws](hyp:nu), [a limiting law](hyp:G), and an [interior quantile
level](hyp:beta,hbeta0,hbeta1) whose [limiting CDF is continuous](hyp:hGcont) and [strictly
increasing there](hyp:hGstrict), with [measurable random quantiles](hyp:hqMeas) and [Kolmogorov
convergence in sampling probability](hyp:hdist), ensure that [the random lower quantiles
converge in sampling probability](goal). -/
theorem quantile_tendsto_inProb_of_cdfKolmogorov
    ...
    (hGcont : ContinuousAt (cdf (G : Measure ℝ))
      (Causalean.Stat.quantile (G : Measure ℝ) beta))
    (hGstrict : Causalean.Stat.StrictlyIncreasingAt (cdf (G : Measure ℝ))
      (Causalean.Stat.quantile (G : Measure ℝ) beta))
    (hqMeas : forall n, Measurable (fun omega =>
      Causalean.Stat.quantile (nu n omega : Measure ℝ) beta))
    (hdist : Tendsto_inProb
      (fun n omega => Causalean.Stat.cdfKolmogorov (nu n omega) G)
      (fun _ => 0) mu) :
    Tendsto_inProb
      (fun n omega => Causalean.Stat.quantile (nu n omega : Measure ℝ) beta)
  ...
  intro epsilon hepsilon
  let q := Causalean.Stat.quantile (G : Measure ℝ) beta
  have hcdfq : cdf (G : Measure ℝ) q = beta := by
    exact Causalean.Stat.cdf_quantile_eq (G : Measure ℝ) hbeta0 hbeta1 hGcont
  ...
  have hsubset (n : ℕ) :
      {omega | epsilon ≤ dist
        (Causalean.Stat.quantile (nu n omega : Measure ℝ) beta) q} ⊆
      {omega | delta ≤ dist
        (Causalean.Stat.cdfKolmogorov (nu n omega) G) 0} := by
    intro omega hbad
    have hkolNonneg := Causalean.Stat.cdfKolmogorov_nonneg (nu n omega) G
    by_contra hnot
    have hkoldist : dist (Causalean.Stat.cdfKolmogorov (nu n omega) G) 0 < delta :=
      lt_of_not_ge hnot
    have hkol : Causalean.Stat.cdfKolmogorov (nu n omega) G < delta := by
    ...
    have hqLower : q - epsilon / 2 <
        Causalean.Stat.quantile (nu n omega : Measure ℝ) beta := by
      by_contra h
      have := (Causalean.Stat.quantile_le_iff hbeta0 hbeta1).1 (le_of_not_gt h)
      linarith
    have hqUpper : Causalean.Stat.quantile (nu n omega : Measure ℝ) beta ≤
        q + epsilon / 2 :=
      (Causalean.Stat.quantile_le_iff hbeta0 hbeta1).2 hnuRight.le
    change epsilon ≤ dist
      (Causalean.Stat.quantile (nu n omega : Measure ℝ) beta) q at hbad
    rw [Real.dist_eq] at hbad
    have hgood :
        |Causalean.Stat.quantile (nu n omega : Measure ℝ) beta - q| < epsilon := by
      rw [abs_lt]
      constructor <;> linarith
    linarith
...
/-- [A measurable statistic sequence](hyp:hSnMeas) that [converges in distribution](hyp:hSn),
[measurable random endpoints and fixed endpoints](hyp:lower,upper,a,b,hlowerMeas,hupperMeas),
[endpoint convergence in probability](hyp:hlower,hupper), and [ordered endpoints with a
boundary-null limiting interval](hyp:hab,hboundary) ensure that [random interval coverage
probabilities converge to the limiting interval probability](goal). -/
...
/-- [A strictly positive Gaussian variance](hyp:v,hv) and an [interior error level](hyp:alpha,halpha0,halpha1)
ensure that [the centered Gaussian assigns probability one minus that error level to its
equal-tail quantile interval](goal). -/
...
/-- [A strictly positive Gaussian variance](hyp:v,hv) and an [interior error level](hyp:alpha,halpha0,halpha1)
ensure that [the centered Gaussian assigns probability one minus that error level to the
reflected equal-tail quantile interval](goal). -/
