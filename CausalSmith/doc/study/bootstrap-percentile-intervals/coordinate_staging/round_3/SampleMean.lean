...
open Causalean.Stat MeasureTheory ProbabilityTheory
...
/-- [A sample size](hyp:n) and [a finite real data vector](hyp:x) determine [the sample-mean
statistic](goal), [given by that vector's finite average](step:1). -/
def sampleMeanStatistic (n : ℕ) (x : Fin n -> ℝ) : ℝ :=
  Causalean.Stat.finAverage x

/-- [An iid real sample](hyp:S) whose [population square is integrable](hyp:hX2) and has
[strictly positive variance](hyp:hvar) ensures that [its sample mean is bootstrap
asymptotically linear at the population mean with centered-observation influence function](goal). -/
theorem BootstrapAsymLinear.sampleMean
  ...
  · constructor
    · intro n
      unfold sampleMeanStatistic Causalean.Stat.finAverage
      fun_prop
    · fun_prop
  ...
  · unfold Tendsto_inProb
    ...
    · exact Filter.eventually_atTop.2 ⟨1, fun n hn =>
        Filter.Eventually.of_forall fun omega => by
          ...
          simp only
          unfold sampleMeanStatistic Causalean.Stat.finAverage IIDSample.sampleVector
    ...
    have hev :
          ...
          epsilon < (Causalean.Stat.bootstrapResample (S.sampleVector n omega)).real
            {xstar | epsilon < abs
              (centeredEstimatorBootstrapStatistic sampleMeanStatistic n
                ...
                Causalean.Stat.centeredBootstrapSum
                  (fun x => x - ∫ y, y ∂P)
                  (S.sampleVector n omega) xstar)}})
      ...
      exact Filter.eventually_atTop.2 ⟨1, fun n hn => by
        ...
        have hrem (omega : Omega) (xstar : Fin n → ℝ) :
          ...
          rw [Causalean.Stat.centeredBootstrapSum_eq_sqrt_mul_finAverage_sub
