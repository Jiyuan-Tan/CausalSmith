...
from the bundled sampling linearization to `Causalean.Stat.IsAsymLinear`.
-/

@[expose] public section

namespace Causalean.Stat.Bootstrap.AsymptoticLinear
...
/-- [An estimator](hyp:est), [a sample size](hyp:n), [observed data](hyp:x), and [a same-size
resample](hyp:xstar) determine [the centered estimator bootstrap statistic](goal), [given by the
square-root-scaled difference between its resampled and observed estimates](step:1). -/
def centeredEstimatorBootstrapStatistic
    (est : (n : ℕ) -> (Fin n -> X) -> ℝ) (n : ℕ)
    (x xstar : Fin n -> X) : ℝ :=
  Real.sqrt (n : ℝ) * (est n xstar - est n x)

/-- [An estimator](hyp:est), [a sample size](hyp:n), [a quantile level](hyp:beta), and [observed
data](hyp:x) determine [the bootstrap quantile](goal), [given by the lower Efron-bootstrap
quantile of the centered estimator statistic](step:1). -/
def bootstrapQuantile (est : (n : ℕ) -> (Fin n -> X) -> ℝ)
    (n : ℕ) (beta : ℝ) (x : Fin n -> X) : ℝ :=
  Causalean.Stat.bootstrapLowerQuantile
    (centeredEstimatorBootstrapStatistic est n x) beta x

/-- [An estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha), and
[observed data](hyp:x) determine [the percentile bootstrap confidence interval](goal), [given by
adding its equal-tail centered-bootstrap quantiles, divided by the square root of the sample size,
to the estimate](step:1). -/
...
/-- [An estimator](hyp:est), [a sample size](hyp:n), [a nominal error level](hyp:alpha), and
[observed data](hyp:x) determine [the basic bootstrap confidence interval](goal), [given by
reflecting the equal-tail centered-bootstrap quantiles around the estimate](step:1). -/
...
/-- [An estimator](hyp:est) whose [each sample-size statistic is measurable](hyp:hest), [a sample
size](hyp:n), and [observed data](hyp:x) ensure that [the centered bootstrap statistic is
measurable in the resample](goal). -/
...
/-- [An estimator](hyp:est) with [measurable sample-size statistics](hyp:hest), a [positive
sample size](hyp:hn), an [interior quantile level](hyp:beta,hbeta0,hbeta1), and [observed
data](hyp:x) ensure that [the centered bootstrap quantile is the corresponding affine transform
of the uncentered bootstrap quantile](goal). -/
theorem bootstrapQuantile_eq_affine
    ...
    bootstrapQuantile est n beta x =
      Real.sqrt (n : ℝ) *
        (Causalean.Stat.bootstrapLowerQuantile (est n) beta x - est n x) := by
  ...
  have hcdf (t : ℝ) :
      Causalean.Stat.bootstrapCDF (centeredEstimatorBootstrapStatistic est n x) x
          (Real.sqrt (n : ℝ) * (t - est n x)) =
        Causalean.Stat.bootstrapCDF (est n) x t := by
    rw [Causalean.Stat.bootstrapCDF_eq_average_indicators _ hcenter x _ hn0,
      Causalean.Stat.bootstrapCDF_eq_average_indicators _ (hest n) x t hn0]
    congr 1
    ...
    simp only [hiff]
  apply le_antisymm
  · change Causalean.Stat.bootstrapLowerQuantile
        (centeredEstimatorBootstrapStatistic est n x) beta x ≤ _
    rw [Causalean.Stat.bootstrapLowerQuantile_le_iff _ hcenter x hn0 beta _
      hbeta0 hbeta1', hcdf]
    exact (Causalean.Stat.bootstrapLowerQuantile_le_iff (est n) (hest n) x hn0 beta _
      hbeta0 hbeta1').mp le_rfl
  · let qstar := bootstrapQuantile est n beta x
    have hreach : beta ≤ Causalean.Stat.bootstrapCDF
        (centeredEstimatorBootstrapStatistic est n x) x qstar :=
      (Causalean.Stat.bootstrapLowerQuantile_le_iff _ hcenter x hn0 beta qstar
        hbeta0 hbeta1').mp le_rfl
    have hcdf' : Causalean.Stat.bootstrapCDF
        (centeredEstimatorBootstrapStatistic est n x) x qstar =
        Causalean.Stat.bootstrapCDF (est n) x
    ...
    have hq : Causalean.Stat.bootstrapLowerQuantile (est n) beta x ≤
        qstar / Real.sqrt (n : ℝ) + est n x :=
      (Causalean.Stat.bootstrapLowerQuantile_le_iff (est n) (hest n) x hn0 beta _
        hbeta0 hbeta1').mpr (by rwa [← hcdf'])
    change Real.sqrt (n : ℝ) *
        (Causalean.Stat.bootstrapLowerQuantile (est n) beta x - est n x) ≤ qstar
    calc
      Real.sqrt (n : ℝ) *
          (Causalean.Stat.bootstrapLowerQuantile (est n) beta x - est n x) ≤
          Real.sqrt (n : ℝ) *
...
/-- [An estimator](hyp:est) with [measurable sample-size statistics](hyp:hest), [a sample
size](hyp:n), and an [interior quantile level](hyp:beta,hbeta0,hbeta1) ensure that [the centered
bootstrap quantile is measurable in the observed data](goal). -/
theorem measurable_bootstrapQuantile
  ...
  · rw [show bootstrapQuantile est n beta = fun x ↦ Real.sqrt (n : ℝ) *
    ...
    exact measurable_const.mul
      ((Causalean.Stat.measurable_bootstrapLowerQuantile (est n) (hest n) hn beta).sub
        (hest n))

/-- [An iid sample](hyp:S), [an estimator](hyp:est), [its target](hyp:theta0), [measurable
sample-size statistics](hyp:hest), and [a sample size](hyp:n) determine [the sampling estimator
law](goal), [given by the original sampling measure pushed forward through the centered,
square-root-scaled estimator](step:1). -/
...
/-- [An estimator](hyp:est) with [measurable sample-size statistics](hyp:hest), [a sample
size](hyp:n), and [observed data](hyp:x) determine [the conditional estimator-bootstrap
law](goal), [given by the Efron law of the centered statistic at positive sizes and a point mass
at zero at size zero](step:1). -/
def bootstrapEstimatorLaw
  ...
  if hn : n = 0 then
    ...
    ⟨Causalean.Stat.bootstrapLaw
      ...
      Causalean.Stat.bootstrapLaw_isProbabilityMeasure
        _ _ hn (measurable_centeredEstimatorBootstrapStatistic est hest n x)⟩

/-- [An estimator](hyp:est) with [measurable sample-size statistics](hyp:hest), a [positive
sample size](hyp:hn), an [interior quantile level](hyp:beta,hbeta0,hbeta1), and [observed
data](hyp:x) ensure that [the bootstrap quantile equals the lower quantile of the bundled
conditional estimator-bootstrap law](goal). -/
theorem bootstrapQuantile_eq_quantile_bootstrapEstimatorLaw
    (est : (n : ℕ) -> (Fin n -> X) -> ℝ)
    (hest : forall n, Measurable (est n)) {n : ℕ} (hn : n != 0)
    {beta : ℝ} (hbeta0 : 0 < beta) (hbeta1 : beta < 1) (x : Fin n -> X) :
    bootstrapQuantile est n beta x =
      Causalean.Stat.quantile (bootstrapEstimatorLaw est hest n x : Measure ℝ) beta := by
  ...
  let _ : IsProbabilityMeasure
      (Causalean.Stat.bootstrapLaw (centeredEstimatorBootstrapStatistic est n x) x) :=
    Causalean.Stat.bootstrapLaw_isProbabilityMeasure _ _ hn0
      (measurable_centeredEstimatorBootstrapStatistic est hest n x)
  have hlaw : (bootstrapEstimatorLaw est hest n x : Measure ℝ) =
      Causalean.Stat.bootstrapLaw (centeredEstimatorBootstrapStatistic est n x) x := by
    simp [bootstrapEstimatorLaw, hn0]
  rw [hlaw]
  unfold bootstrapQuantile Causalean.Stat.bootstrapLowerQuantile
    Causalean.Stat.bootstrapCDF Causalean.Stat.quantile Causalean.Stat.quantileSet
  congr 1
  ext t
  simp only [Set.mem_ofPred_eq]
  rw [ProbabilityTheory.cdf_eq_real, Measure.real_def]

/-- [An iid sample](hyp:S), [an estimator](hyp:est), [its target](hyp:theta0), and [an influence
function](hyp:psi) satisfy bootstrap asymptotic linearity when [the statistics and
influence function are measurable](hyp:meas), [the influence function has mean zero](hyp:mean_zero),
[its squared value has finite positive mean](hyp:var_pos_finite), [the sampling linearization is
negligible in probability](hyp:linear), and [the conditional bootstrap linearization is
negligible in sampling probability](hyp:boot_linear). -/
structure BootstrapAsymLinear (S : IIDSample Omega X mu P)
  ...
  boot_linear : forall epsilon : ℝ, 0 < epsilon ->
    Tendsto
      (fun n => mu.real {omega |
        epsilon < (Causalean.Stat.bootstrapResample (S.sampleVector n omega)).real
          {xstar | epsilon < abs
...
/-- [Bootstrap asymptotic linearity](hyp:h) determines [the asymptotic variance](goal), [given by
the influence function's population second moment](step:1). -/
def asymptoticVariance (h : BootstrapAsymLinear S est theta0 psi) : ℝ :=
  ∫ x, (psi x) ^ 2 ∂P

/-- [Bootstrap asymptotic linearity](hyp:h) determines [the limiting Gaussian law](goal), [given
by the centered Gaussian probability measure with the influence function's population second
moment as variance](step:1). -/
def asymptoticGaussian (h : BootstrapAsymLinear S est theta0 psi) :
    ProbabilityMeasure ℝ :=
  ⟨Causalean.Stat.gaussianMeasure 0 h.asymptoticVariance, inferInstance⟩

/-- [Bootstrap asymptotic linearity](hyp:h) ensures that [the estimator evaluated on the iid
sample vector is asymptotically linear in the library's existing sense](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h), an [interior nominal error level](hyp:alpha,halpha0,halpha1),
and [a sample size](hyp:n) ensure that [the percentile-interval coverage event is measurable
under the sampling sigma-algebra](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h), an [interior nominal error level](hyp:alpha,halpha0,halpha1),
and [a sample size](hyp:n) ensure that [the basic-interval coverage event is measurable under
the sampling sigma-algebra](goal). -/
...
end Causalean.Stat.Bootstrap.AsymptoticLinear
