...
/-!
# Validity of percentile and basic bootstrap intervals

This module proves the user-facing consequences of bootstrap asymptotic linearity.  The sampling
and conditional bootstrap laws approach the same centered Gaussian law, hence approach one
another in Kolmogorov distance; conditional quantiles converge in sampling probability; and the
percentile and basic intervals attain their nominal asymptotic coverage.
-/
...
/-- [Bootstrap asymptotic linearity](hyp:h) ensures that [the sampling distribution of the
square-root-scaled estimator converges weakly to the centered Gaussian law with the influence
function's second-moment variance](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h) ensures that [the conditional bootstrap distribution
of the centered estimator statistic converges in sampling probability, in Kolmogorov distance,
to its centered Gaussian influence-function law](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h) ensures that [the conditional law of the centered
estimator bootstrap statistic and the finite-sample sampling law approach one another in
...
/-- [Bootstrap asymptotic linearity](hyp:h) and an [interior probability level](hyp:beta,hbeta0,hbeta1)
ensure that [the lower conditional bootstrap quantile converges in sampling probability to the
corresponding centered-Gaussian quantile](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h) and an [interior nominal error level](hyp:alpha,halpha0,halpha1)
ensure that [the percentile bootstrap confidence interval covers the target with probability
converging to one minus that error level](goal). -/
...
/-- [Bootstrap asymptotic linearity](hyp:h) and an [interior nominal error level](hyp:alpha,halpha0,halpha1)
ensure that [the basic bootstrap confidence interval covers the target with probability
converging to one minus that error level](goal). -/
