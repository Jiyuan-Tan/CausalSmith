...
/-!
# Parameterized unit-interval quantiles for real kernels

This module realizes a real-valued Markov kernel supported on the unit interval by one uniform
variable, jointly measurably in the kernel parameter and the uniform input.  It defines the
unit-interval generalized inverse, proves its joint and sectionwise measurability, proves its
exact sectionwise pushforward law, and records its pointwise range guarantee.

The construction is the generalized inverse
`sup {x ∈ [0,1] | (κ s)([0,x]) < u}`.  Its measurability is proved directly from rational
thresholds, without a measurable-selection assumption.
-/

open MeasureTheory ProbabilityTheory Set Function Filter Topology ENNReal unitInterval
open scoped unitInterval

namespace Causalean.Mathlib.Probability.Kernel.QuantileRealization

variable {S : Type*} [MeasurableSpace S]

/-- A [real-valued kernel](hyp:κ) is [supported on the closed unit interval](goal) precisely
when [each of its conditional laws assigns that interval probability one](step:1). -/
def SupportedOnUnitInterval (κ : ProbabilityTheory.Kernel S ℝ) : Prop :=
  ∀ s, κ s (Icc (0 : ℝ) 1) = 1

/-- For [a real-valued kernel](hyp:κ), [a parameter value](hyp:s), and [a uniform level in the
unit interval](hyp:u), [the unit-interval-valued kernel quantile](goal) is [the supremum of
interval values whose conditional cumulative mass is strictly below that level](step:1). -/
...
/-- For [a real-valued kernel](hyp:κ) and [a parameter--input pair](hyp:p), [the real-valued
quantile realization](goal) is [the unit-interval quantile evaluated after projecting the real
input onto the closed unit interval](step:1). -/
...
/-- Given [a real-valued kernel](hyp:κ) and [a parameter--input pair](hyp:p), [the realization
value lies in the closed unit interval](goal). -/
...
/-- Given [a real-valued Markov kernel](hyp:κ), [its unit-interval generalized inverse is
jointly measurable in the kernel parameter and uniform level](goal). -/
...
/-- Given [a real-valued Markov kernel](hyp:κ), [the real-valued quantile realization is
jointly measurable in its parameter and real randomization input](goal). -/
...
/-- Given [a real-valued Markov kernel](hyp:κ) and [a fixed parameter value](hyp:s), [the
corresponding real-valued quantile section is measurable in its randomization input](goal). -/
@[fun_prop]
...
/-- Given [a real-valued Markov kernel](hyp:κ), [fiberwise support on the closed unit
interval](hyp:hκ), and [a parameter value](hyp:s), [coercing the unit-interval quantile pushes
the canonical uniform law exactly to that kernel fiber](goal). -/
...
/-- Given [a real-valued Markov kernel](hyp:κ), [fiberwise support on the closed unit
interval](hyp:hκ), and [a parameter value](hyp:s), [the real quantile section pushes Lebesgue
measure restricted to the unit interval exactly to that kernel fiber](goal). -/
...
/-- Given [a real-valued kernel](hyp:κ) and [a parameter value](hyp:s), [the real quantile
...
end Causalean.Mathlib.Probability.Kernel.QuantileRealization
