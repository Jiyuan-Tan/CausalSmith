...
real argument `u` is `exp(iu) - 1 - iu + u²/2`. [The remaining declared inputs](hyp:u) determine [the defined object](goal). -/
...
of the quadratic Taylor remainder of `exp(iu)` is at most `|u|³`](goal). [The remaining declared inputs](hyp:u) determine [the stated conclusion](goal). -/
...
a quadratic contribution supported on increments larger than `η`](goal). [The remaining declared inputs](hyp:η) determine [the stated conclusion](goal). -/
