...
Euler factor `exp(x) * (1 - x)` is at most `exp(x) * x² / 2`](goal). [The remaining declared inputs](hyp:x) determine [the stated conclusion](goal). -/
...
weight times the Taylor error plus a quadratic Euler error](goal). [The remaining declared inputs](hyp:c,v) determine [the stated conclusion](goal). -/
...
with a constant depending only on the budget](goal). [The remaining declared inputs](hyp:c,V,K) determine [the stated conclusion](goal). -/
