...
Lindeberg term](goal). [The remaining declared inputs](hyp:A,n,k,η) determine [the stated conclusion](goal). -/
...
at most `(η² + δ)K`](goal). [The remaining declared inputs](hyp:A,n,η,K,δ) determine [the stated conclusion](goal). -/
