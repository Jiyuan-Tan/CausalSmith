import Causalean.Mathlib.Probability.GaussianMeanEmbedding.Taylor
...
/-- [Two probability measures `μ` and `ν`](hyp:μ,ν), [a nonnegative support radius `R`](hyp:R,hR),
[a nonnegative approximation error `ε`](hyp:ε,hε), [a truncation index `N`](hyp:N), [the
condition that each measure is concentrated on the interval from zero to `R`](hyp:hμ,hν),
and [a uniform Gaussian-weighted Taylor approximation of the square on that interval](hyp:happrox)
give [a second-moment gap bounded by the recovery coefficient times Gaussian mean-embedding
distance plus twice the approximation error](goal). -/
...
/-- [Two probability measures `μ` and `ν`](hyp:μ,ν) [concentrated on the interval from
zero to five](hyp:hμ,hν) have [a raw second-moment gap no greater than 5151 times their
explicit Gaussian mean-embedding distance plus one divided by ten to the fifteenth
power](goal). -/
...
/-- [Two probability measures `μ` and `ν`](hyp:μ,ν) [concentrated on the interval from
zero to five](hyp:hμ,hν) whose [certified raw second-moment gap exceeds one divided by ten
to the fifteenth power](hyp:hgap) have [a strictly positive explicit lower bound on their
Gaussian mean-embedding distance](goal). -/
