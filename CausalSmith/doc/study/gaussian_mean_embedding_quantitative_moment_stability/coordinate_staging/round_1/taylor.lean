import Causalean.Mathlib.Probability.GaussianMeanEmbedding.Coefficient
...
/-- [A truncation index `N`](hyp:N) determines [the polynomial](goal) formed from the
first `N + 1` Taylor terms of `r² exp(r²)`. -/
def secondMomentTaylorPolynomial (N : ℕ) : ℝ[X] :=
  ∑ k ∈ Finset.range (N + 1),
    Polynomial.monomial (2 * k + 2) (1 / (k.factorial : ℝ))

/-- [A truncation index `N`](hyp:N) and [a real input `r`](hyp:r) make [evaluation of the
recovery polynomial equal its finite even-power Taylor sum](goal). -/
...
/-- [The Taylor truncation through index one hundred has degree exactly 202](goal). -/
...
/-- [A truncation index `N`](hyp:N), [a nonnegative real argument `x`](hyp:x,hx), and
[the condition that the first omitted denominator exceeds `x`](hyp:hden) give [a geometric
upper bound for the exponential-series tail after index `N`](goal). -/
...
/-- [A real input `r` in the interval from zero to five](hyp:r,hr) has [its `r² exp(r²)`
value approximated by the degree-202 Taylor polynomial within the stated factorial-tail
certificate](goal). -/
...
/-- [A real input `r` in the interval from zero to five](hyp:r,hr) has [its raw square
approximated by the Gaussian-weighted degree-202 Taylor polynomial within the same
certified error](goal). -/
...
/-- [Twice the certified degree-202 Taylor remainder is at most the rational error
one divided by ten to the fifteenth power](goal). -/
