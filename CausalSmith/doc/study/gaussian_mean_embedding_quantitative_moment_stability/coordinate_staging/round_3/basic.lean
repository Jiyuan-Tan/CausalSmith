...
/-- A [real Hilbert space `H`](hyp:H) determines [a unit-norm feature realization of the
unit-scale Gaussian kernel](goal).  It consists of a feature map, a certificate that every
feature vector has unit norm, and a certificate that feature-vector inner products equal the
kernel. -/
