...
## Mathlib/Probability/GaussianMeanEmbedding — explicit Gaussian mean-embedding moment recovery

This module family provides the explicit weighted-monomial Gaussian feature realization in real ℓ² and a certified finite Taylor argument turning Gaussian mean-embedding distance into a quantitative second-moment bound for laws supported on [0,5].

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Basic -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianKernel` | `ℝ → ℝ → ℝ` | Two real inputs `a` and `b` determine the unit-scale Gaussian kernel value `exp(−(a − b)²)`. |
| `Mathlib.Probability.GaussianMeanEmbedding.UnitNormGaussianFeatureMap` | `(H : Type u_1) → [inst : NormedAddCommGroup H] → [InnerProductSpace ℝ H] → Type u_1` | A real Hilbert space determines a unit-norm feature realization of the unit-scale Gaussian kernel. It consists of a feature map, a certificate that every feature vector has unit norm, and a certificate that feature-vector inner products equal the kernel. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization` | `ℕ → ℝ` | A coordinate index `m` determines the positive normalization that multiplies coordinate `m` of the explicit Gaussian feature vector. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization_pos` | `0 < Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization m` | A coordinate index `m` has strictly positive Gaussian-feature normalization. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureCoefficient` | `ℝ → ℕ → ℝ` | A real input `r` and a coordinate index `m` determine the explicit Gaussian feature coefficient at that coordinate. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature_memℓp` | `Memℓp (Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureCoefficient r) 2` | A real input `r` has a square-summable explicit Gaussian coefficient sequence. |
| `Mathlib.Probability.GaussianMeanEmbedding.GaussianHilbert` | `AddSubgroup (PreLp fun x => ℝ)` | The complete real Hilbert space used for the explicit Gaussian feature map is the square-summable sequence space. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature` | `ℝ → ↥Causalean.Mathlib.Probability.GaussianMeanEmbedding.GaussianHilbert` | A real input `r` determines its explicit Gaussian feature vector in the real square-summable sequence space. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature_inner` | `inner ℝ (Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature a) (Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature b) = Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianKernel a b` | Two real inputs `a` and `b` have an explicit feature-vector inner product equal to their Gaussian kernel value. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature_norm` | `‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature r‖ = 1` | A real input `r` has an explicit Gaussian feature vector of norm one. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap` | `Causalean.Mathlib.Probability.GaussianMeanEmbedding.UnitNormGaussianFeatureMap ↥Causalean.Mathlib.Probability.GaussianMeanEmbedding.GaussianHilbert` | The explicit Gaussian feature map is given by the square-summable feature vector, its unit-norm certificate, and its kernel-inner-product certificate. |
| `Mathlib.Probability.GaussianMeanEmbedding.UnitNormGaussianFeatureMap.continuous` | `Continuous U.feature` | A unit-norm Gaussian feature realization `U` has a continuous feature map. |
| `Mathlib.Probability.GaussianMeanEmbedding.UnitNormGaussianFeatureMap.integrable` | `MeasureTheory.Integrable U.feature μ` | A unit-norm Gaussian feature realization `U` and a finite real measure `μ` make the feature map Bochner integrable. |
| `Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding` | `MeasureTheory.Measure ℝ → H` | A unit-norm Gaussian feature realization `U` and a real measure `μ` determine their Bochner mean embedding. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianCoordinate` | `ℕ → ↥Causalean.Mathlib.Probability.GaussianMeanEmbedding.GaussianHilbert →L[ℝ] ℝ` | A coordinate index `m` determines the continuous linear evaluation map on the real square-summable sequence space. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianCoordinate_feature` | `(Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianCoordinate m) (Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeature r) = Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureCoefficient r m` | A coordinate index `m` and a real input `r` make coordinate evaluation of the explicit feature equal its defining coefficient. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianCoordinate_meanEmbedding` | `(Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianCoordinate m) (Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ) = ∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureCoefficient r m ∂μ` | A finite real measure `μ` and a coordinate index `m` make coordinate evaluation commute with the Bochner Gaussian mean integral. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianWeight` | `ℝ → ℝ` | The input real value `r` is assigned its Gaussian weight, the factor `exp (−r²)` used by the explicit feature coordinates. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial` | `ℕ → ℝ → ℝ` | A nonnegative integer degree `m` and a real value `r` determine the Gaussian-weighted monomial `exp (−r²) rᵐ`. |
| `Mathlib.Probability.GaussianMeanEmbedding.integrable_gaussianWeightedMonomial` | `MeasureTheory.Integrable (Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial m) μ` | A finite real measure `μ` and a monomial degree `m` make the corresponding Gaussian-weighted monomial integrable. |

<!-- /GEN -->

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Coordinates -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.GaussianMeanEmbedding.abs_gaussianWeightedMoment_sub_le` | `\|∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial m r ∂μ - ∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial m r ∂ν\| ≤ ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖ / Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization m` | Two finite real measures `μ` and `ν` and a coordinate order `m` have a Gaussian-weighted moment gap no larger than their explicit Gaussian mean-embedding distance divided by that coordinate's positive normalization. |
| `Mathlib.Probability.GaussianMeanEmbedding.abs_sum_gaussianWeightedMoment_sub_le` | `\|∑ i ∈ s, coefficient i * (∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial (degree i) r ∂μ - ∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial (degree i) r ∂ν)\| ≤ (∑ i ∈ s, \|coefficient i\| / Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization (degree i)) * ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖` | Two finite real measures `μ` and `ν`, a finite index set `s`, a degree assigned to each index, and a real coefficient assigned to each index have their finite Gaussian-weighted-moment combination bounded by mean-embedding distance times the sum of absolute normalized coefficients. |

<!-- /GEN -->

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Coefficient -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient` | `ℕ → ℝ` | A truncation index `N` determines the normalization-adjusted coefficient sum used to recover a raw second moment from its Taylor terms. |
| `Mathlib.Probability.GaussianMeanEmbedding.abs_secondMomentWeightedSum_sub_le` | `\|∑ k ∈ Finset.range (N + 1), 1 / ↑k.factorial * (∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial (2 * k + 2) r ∂μ - ∫ (r : ℝ), Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeightedMonomial (2 * k + 2) r ∂ν)\| ≤ Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient N * ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖` | Two finite real measures `μ` and `ν` and a truncation index `N` have their first `N + 1` even Gaussian-weighted Taylor moments bounded by the recovery coefficient times Gaussian mean-embedding distance. |
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient_term_le` | `1 / ↑k.factorial / Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureNormalization (2 * k + 2) ≤ ↑(k + 1)` | A Taylor index `k` has a normalization-adjusted coefficient no greater than its one-based index, the termwise central-binomial estimate behind 5151. |
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient_oneHundred_le` | `Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient 100 ≤ 5151` | The 101-term normalization-adjusted recovery coefficient is at most 5151. |

<!-- /GEN -->

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Taylor -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial` | `ℕ → Polynomial ℝ` | A truncation index `N` determines the polynomial formed from the first `N + 1` Taylor terms of `r² exp(r²)`. |
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial_eval` | `Polynomial.eval r (Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial N) = ∑ k ∈ Finset.range (N + 1), r ^ (2 * k + 2) / ↑k.factorial` | A truncation index `N` and a real input `r` make evaluation of the recovery polynomial equal its finite even-power Taylor sum. |
| `Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial_natDegree_oneHundred` | `(Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial 100).natDegree = 202` | The Taylor truncation through index one hundred has degree exactly 202. |
| `Mathlib.Probability.GaussianMeanEmbedding.exp_series_tail_le_geometric` | `0 ≤ x → x < ↑N + 2 → Real.exp x - ∑ k ∈ Finset.range (N + 1), x ^ k / ↑k.factorial ≤ x ^ (N + 1) / ↑(N + 1).factorial * ((↑N + 2) / (↑N + 2 - x))` | A truncation index `N`, a nonnegative real argument `x`, and the condition that the first omitted denominator exceeds `x` give a geometric upper bound for the exponential-series tail after index `N`. |
| `Mathlib.Probability.GaussianMeanEmbedding.abs_secondMoment_exp_sub_taylor_oneHundred_le` | `r ∈ Set.Icc 0 5 → \|r ^ 2 * Real.exp (r ^ 2) - Polynomial.eval r (Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial 100)\| ≤ 25 * (25 ^ 101 / ↑(Nat.factorial 101)) * (102 / 77)` | A real input `r` in the interval from zero to five has its `r² exp(r²)` value approximated by the degree-202 Taylor polynomial within the stated factorial-tail certificate. |
| `Mathlib.Probability.GaussianMeanEmbedding.abs_sq_sub_gaussianWeightedTaylor_oneHundred_le` | `r ∈ Set.Icc 0 5 → \|r ^ 2 - Real.exp (-r ^ 2) * Polynomial.eval r (Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial 100)\| ≤ 25 * (25 ^ 101 / ↑(Nat.factorial 101)) * (102 / 77)` | A real input `r` in the interval from zero to five has its raw square approximated by the Gaussian-weighted degree-202 Taylor polynomial within the same certified error. |
| `Mathlib.Probability.GaussianMeanEmbedding.two_taylorRemainder_oneHundred_le` | `2 * (25 * (25 ^ 101 / ↑(Nat.factorial 101)) * (102 / 77)) ≤ 1 / 10 ^ 15` | Twice the certified degree-202 Taylor remainder is at most the rational error one divided by ten to the fifteenth power. |

<!-- /GEN -->

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Stability -->
| Decl | Signature | Description |
|---|---|---|
| `Mathlib.Probability.GaussianMeanEmbedding.secondMoment_sub_le_of_taylor_error` | `0 ≤ R → 0 ≤ ε → μ (Set.Icc 0 R)ᶜ = 0 → ν (Set.Icc 0 R)ᶜ = 0 → (∀ r ∈ Set.Icc 0 R, \|r ^ 2 - Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianWeight r * Polynomial.eval r (Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentTaylorPolynomial N)\| ≤ ε) → \|∫ (r : ℝ), r ^ 2 ∂μ - ∫ (r : ℝ), r ^ 2 ∂ν\| ≤ Causalean.Mathlib.Probability.GaussianMeanEmbedding.secondMomentRecoveryCoefficient N * ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖ + 2 * ε` | Two probability measures `μ` and `ν`, a nonnegative support radius `R`, a nonnegative approximation error `ε`, a truncation index `N`, the condition that each measure is concentrated on the interval from zero to `R`, and a uniform Gaussian-weighted Taylor approximation of the square on that interval give a second-moment gap bounded by the recovery coefficient times Gaussian mean-embedding distance plus twice the approximation error. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussian_meanEmbedding_secondMoment_stability_Icc_zero_five` | `μ (Set.Icc 0 5)ᶜ = 0 → ν (Set.Icc 0 5)ᶜ = 0 → \|∫ (r : ℝ), r ^ 2 ∂μ - ∫ (r : ℝ), r ^ 2 ∂ν\| ≤ 5151 * ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖ + 1 / 10 ^ 15` | Two probability measures `μ` and `ν` concentrated on the interval from zero to five have a raw second-moment gap no greater than 5151 times their explicit Gaussian mean-embedding distance plus one divided by ten to the fifteenth power. |
| `Mathlib.Probability.GaussianMeanEmbedding.gaussian_meanEmbedding_norm_lowerBound_of_secondMoment_gap` | `μ (Set.Icc 0 5)ᶜ = 0 → ν (Set.Icc 0 5)ᶜ = 0 → 1 / 10 ^ 15 < \|∫ (r : ℝ), r ^ 2 ∂μ - ∫ (r : ℝ), r ^ 2 ∂ν\| → 0 < (\|∫ (r : ℝ), r ^ 2 ∂μ - ∫ (r : ℝ), r ^ 2 ∂ν\| - 1 / 10 ^ 15) / 5151 ∧ (\|∫ (r : ℝ), r ^ 2 ∂μ - ∫ (r : ℝ), r ^ 2 ∂ν\| - 1 / 10 ^ 15) / 5151 ≤ ‖Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap μ - Causalean.Mathlib.Probability.GaussianMeanEmbedding.meanEmbedding Causalean.Mathlib.Probability.GaussianMeanEmbedding.gaussianFeatureMap ν‖` | Two probability measures `μ` and `ν` concentrated on the interval from zero to five whose certified raw second-moment gap exceeds one divided by ten to the fifteenth power have a strictly positive explicit lower bound on their Gaussian mean-embedding distance. |

<!-- /GEN -->

<!-- GEN:Causalean.Mathlib.Probability.GaussianMeanEmbedding.Main -->
_(no documented declarations in Causalean.Mathlib.Probability.GaussianMeanEmbedding.Main)_
