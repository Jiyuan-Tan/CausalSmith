...
[an iid sample, measurable sample-function estimator, and continuous linear contrast](hyp:S,est,hEstMeas,c), [data and
bootstrap consistency](hyp:hConsistent,hBootConsistent), [data and bootstrap solution of the
estimating equation](hyp:hSolve,hBootSolve), and [a positive contrast influence
...
/-- Under [the estimating function, target, population law, and smooth-Z bootstrap hypotheses](hyp:psi,theta0,P,reg,S,est,hEstMeas,c,
hConsistent,hSolve,hBootConsistent,hBootSolve,hVar) and [an interior error
level](hyp:halpha0,halpha1), [the percentile bootstrap interval for the contrast covers the true
contrast with probability tending to one minus that level](goal). -/
...
/-- Under [the estimating function, target, population law, and smooth-Z bootstrap hypotheses](hyp:psi,theta0,P,reg,S,est,hEstMeas,c,
hConsistent,hSolve,hBootConsistent,hBootSolve,hVar) and [an interior error
level](hyp:halpha0,halpha1), [the basic bootstrap interval for the contrast covers the true
contrast with probability tending to one minus that level](goal). -/
