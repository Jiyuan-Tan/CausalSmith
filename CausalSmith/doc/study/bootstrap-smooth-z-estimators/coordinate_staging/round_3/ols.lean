...
/-- Given [regressors and an outcome](hyp:x,y), [a sample size and finite data](hyp:n,data), [the
OLS sample-function estimator](goal) applies the closed-form OLS coefficient map to the finite
average of the raw moment vector. -/
...
/-- For [measurable regressors and outcome](hyp:hx,hy) and [a sample size](hyp:n), [the OLS
sample-function estimator is measurable](goal). -/
@[fun_prop]
...
/-- For [an iid sample](hyp:S), [regressors and an outcome](hyp:x,y), and [a row size and
realized sample point](hyp:n,omega), [the sample-function OLS estimator evaluated on the sample
vector equals the library OLS estimator](goal). -/
...
/-- Under [iid sampling, integrable OLS raw moments, and a positive-definite population Gram
matrix](hyp:S,hx,hy,hraw,hQ), [the OLS sample-function estimator solves its score equation with
probability tending to one](goal). -/
...
/-- Under [an iid sample](hyp:S), [the standard OLS moment and rank conditions](hyp:hx,hy,hraw,hQ),
[a continuous contrast](hyp:c), [a resampled OLS coefficient that is consistent and solves its
normal equations in bootstrap probability, in outer sampling probability](hyp:hBootConsistent,hBootSolve),
and [positive contrast influence variance](hyp:hVar), [that OLS contrast is bootstrap
asymptotically linear](goal). -/
theorem olsContrast
    [IsProbabilityMeasure mu]
    (S : IIDSample Omega X mu P)
    {x : X → EuclideanSpace ℝ K} {y : X → ℝ}
