...
/-!
# Bootstrap validity for smooth feasible GMM

This module gives sample-function forms of the smooth GMM Jacobian, normalized moment, and
first-order-condition residual.  It proves the conditional bootstrap linearization under
bootstrap consistency of the parameter and weight and a negligible bootstrap FOC, then combines
that result with the existing data-side feasible-GMM asymptotic-linearity theorem.
-/
...
private theorem feasibleGMM_populationRemainder_le
    (prob : GMMProblem (E := E) (F := F) P)
...
omit [MeasurableSpace E] [BorelSpace E] [BorelSpace F] in
private theorem gmmNormalizedMomentFn_expansion_le
    {prob : GMMProblem (E := E) (F := F) P}
    (reg : SmoothGMomentRegularity prob)
  ...
  have hboot_eq : ∀ n (x xstar : Fin n → X),
    ...
    · rw [centeredBootstrapSum_eq_sqrt_mul_finAverage_sub hn]
      simp [centeredEstimatorBootstrapStatistic, zEstimatorBootstrapRemainder,
        ...
        Causalean.Stat.finAverage, influence, map_sub, map_smul, map_sum]
