...
private theorem exactRoot_populationRemainder_le
    ...
    (hroot : zEstimatorSampleScore psi theta x = 0)
  ...
  have hR : ∀ i : Fin n, ‖R i‖ ≤ L (x i) * ‖Delta‖ ^ 2 := by
    intro i
    exact reg.score_taylor_remainder_le hLnonneg hLbound (x i)
  ...
  have hscaledR :
    ...
    calc
      ...
      _ = (finMean (fun i ↦ L (x i)) * ‖Delta‖) * ‖v‖ := by
            ...
            ring
  have herr :
      ‖U + reg.jacobian v‖ ≤ C * ‖v‖ := by
    have hzero : Real.sqrt (n : ℝ) • finMean (fun i ↦ psi theta (x i)) = 0 := by
      simpa [zEstimatorSampleScore] using
        congrArg (fun z : E ↦ Real.sqrt (n : ℝ) • z) hroot
    have heq :
        U + reg.jacobian v =
          -(Jn - reg.jacobian) v - Real.sqrt (n : ℝ) • finMean R := by
      rw [hscoreExpansion] at hzero
      simp only [smul_add] at hzero
      have hzero' : U + Jn v + Real.sqrt (n : ℝ) • finMean R = 0 := by
        simpa [U, v, map_smul] using hzero
      apply eq_of_sub_eq_zero
      calc
        U + reg.jacobian v -
          (-(Jn - reg.jacobian) v - Real.sqrt (n : ℝ) • finMean R) =
            U + Jn v + Real.sqrt (n : ℝ) • finMean R := by
            simp only [sub_apply]
            abel
        _ = 0 := hzero'
    rw [heq]
    calc
      ...
      _ = ‖(Jn - reg.jacobian) v‖ +
              ‖Real.sqrt (n : ℝ) • finMean R‖ := by rw [norm_neg]
      _ ≤ ‖Jn - reg.jacobian‖ * ‖v‖ +
          (finMean (fun i ↦ L (x i)) * ‖Delta‖) * ‖v‖ :=
            add_le_add ((Jn - reg.jacobian).le_opNorm v) hscaledR
      _ = C * ‖v‖ := by simp only [C, Jn]; ring
  have hv_bound : ‖v‖ ≤ 2 * c * ‖U‖ := by
    have hvid : v = -reg.jacobianInv U +
        reg.jacobianInv (U + reg.jacobian v) := by
      rw [map_add, hJleft]
      abel
    have hpre : ‖v‖ ≤ c * ‖U‖ + (1 / 2) * ‖v‖ := calc
      ...
      _ ≤ c * ‖U‖ + (1 / 2) * ‖v‖ := by
        ...
        nlinarith
    nlinarith [hpre, mul_nonneg hc.le (norm_nonneg U)]
  have hinfluence :
      (Real.sqrt (n : ℝ))⁻¹ • ∑ i : Fin n, reg.influence (x i) =
...
sample-function estimator](hyp:est), [data consistency](hyp:hConsistent), [data
estimating-equation validity](hyp:hSolve), [bootstrap consistency](hyp:hBootConsistent), and
[bootstrap estimating-equation validity](hyp:hBootSolve), [the conditional bootstrap vector
linearization remainder vanishes in outer probability](goal). -/
theorem zEstimator_bootstrapLinearization
  ...
  have hJdata : Tendsto_inProb
    ...
    simpa [SmoothZEstimatorRegularity.jacobian] using
  ...
  have hroot := (hBootSolve inner hinner).eventually (Metric.ball_mem_nhds 0 halpha)
  ...
  filter_upwards [hUdata, hScoreBoot, hqJ, hqL, hroot, hcons, hJd, hLd, hRd,
      eventually_ne_atTop 0] with n hUn hScore_n hqJn hqLn hrootn hconsn hJdn hLdn hRdn hn
  simp only [Real.dist_eq, sub_zero, abs_of_nonneg measureReal_nonneg]
    at hqJn hqLn hrootn hconsn hJdn hLdn hRdn ⊢
  have hJdn' : mu.real {x |
  ...
  let badRoot : Set Omega := {omega |
    inner < (bootstrapResample (S.sampleVector n omega)).real
      {xstar | zEstimatorSampleScore psi (est n xstar) xstar ≠ 0}}
  let badCons : Set Omega := {omega |
    inner < (bootstrapResample (S.sampleVector n omega)).real
      {xstar | a / ML < ‖est n xstar - theta0‖}}
  ...
  have houterSubset :
    ...
    have hbadSubset : bad ⊆ rootFail ∪ consFail ∪ scoreFail ∪ jFail ∪ lFail := by
      intro xstar hxstar
      by_contra hxgood
      simp only [Set.mem_union, not_or] at hxgood
      rcases hxgood with ⟨⟨⟨⟨hrootx, hconsx⟩, hscorex⟩, hJx⟩, hLx⟩
      ...
      have hpop := exactRoot_populationRemainder_le psi theta0 P reg L hLnonneg
        hLbound hJleft hn xstar (est n xstar) (not_ne_iff.mp hrootx) (by simpa [c] using hsmall)
      have hpopSmall :
    ...
    have hmeasureBad : (bootstrapResample data).real bad ≤ 5 * inner := by
      calc
        _ ≤ (bootstrapResample data).real
            (rootFail ∪ consFail ∪ scoreFail ∪ jFail ∪ lFail) :=
          measureReal_mono hbadSubset
        _ ≤ (bootstrapResample data).real rootFail +
        ...
        _ ≤ 5 * inner := by
          have hr : (bootstrapResample data).real rootFail ≤ inner :=
            le_of_not_gt (by simpa [badRoot, rootFail, data] using hRootGood)
  ...
  calc
    mu.real {omega | delta < (bootstrapResample (S.sampleVector n omega)).real
        {xstar | epsilon < ‖zEstimatorBootstrapRemainder est reg.influence n
          (S.sampleVector n omega) xstar‖}}
        ≤ mu.real (badU ∪ badScore ∪ badJboot ∪ badLboot ∪ badRoot ∪ badCons ∪
    ...
    _ ≤ mu.real badU + mu.real badScore + mu.real badJboot + mu.real badLboot +
        mu.real badRoot + mu.real badCons + mu.real badJdata + mu.real badLdata +
        mu.real badRdata := measureReal_union_nine_le mu _ _ _ _ _ _ _ _ _
    _ < eta := by
      dsimp [badU, badScore, badJboot, badLboot, badRoot, badCons, badJdata,
