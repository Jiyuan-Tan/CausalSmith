...
public import Causalean.Stat.Bootstrap.SmoothZEstimator.FeasibleGMM.Linearization

/-!
# Bootstrap asymptotic linearity for smooth feasible GMM contrasts

This module constructs bootstrap asymptotic linearity for a scalar contrast of a smooth feasible
GMM estimator by combining the data-side expansion with its conditional bootstrap analogue.
-/
...
[a sample-function parameter estimator](hyp:est), [measurability at every sample size](hyp:hEstMeas),
[a sample-function weight estimator](hyp:weightEst), [a continuous linear contrast](hyp:c), [data
parameter consistency](hyp:hConsistent), [data weight consistency](hyp:hWeight), [a negligible data
first-order-condition residual](hyp:hApproxFOC), [bootstrap parameter consistency](hyp:hBootConsistent),
[bootstrap weight consistency](hyp:hBootWeight), [a negligible bootstrap first-order-condition
residual](hyp:hBootFOC), and [positive contrast influence variance](hyp:hVar), [the feasible-GMM
contrast is bootstrap asymptotically linear](goal). -/
theorem feasibleGMM
