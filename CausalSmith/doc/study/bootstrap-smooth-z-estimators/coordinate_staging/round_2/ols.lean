...
open Causalean.Stat Filter Matrix MeasureTheory ProbabilityTheory Topology
...
/-- For [regressors](hyp:x) and [an outcome](hyp:y), [the OLS sample-function estimator](goal) is
given by [applying the closed-form OLS coefficient map to the finite average of raw moments](step:1). -/
def olsSampleEstimator (x : X → EuclideanSpace ℝ K) (y : X → ℝ)
    (n : ℕ) (data : Fin n → X) : EuclideanSpace ℝ K :=
  olsBetaFromMoments (Causalean.Stat.finMean (fun i ↦ olsRawMoment x y (data i)))

/-- For [measurable regressors](hyp:hx), [a measurable outcome](hyp:hy), and [a sample
size](hyp:n), [the OLS sample-function estimator is measurable](goal). -/
@[fun_prop]
theorem measurable_olsSampleEstimator
  ...
  have hM : Measurable (fun data : Fin n → X =>
      Causalean.Stat.finMean (fun i ↦ olsRawMoment x y (data i))) := by
  ...
  have hQ : Measurable (fun data : Fin n → X =>
      olsQFromMoments (Causalean.Stat.finMean
  ...
  have hR : Measurable (fun data : Fin n → X =>
      olsRFromMoments (Causalean.Stat.finMean
        (fun i ↦ olsRawMoment x y (data i)))) := by
  ...
  have hInv : Measurable (fun data : Fin n → X =>
    ...
    rw [show (fun data : Fin n → X =>
      ...
      (fun data : Fin n → X => Ring.inverse (Matrix.det (olsQFromMoments
          (Causalean.Stat.finMean (fun i ↦ olsRawMoment x y (data i))))) •
  ...
  rw [show (fun data : Fin n → X =>
      (olsQFromMoments (Causalean.Stat.finMean
        ...
        (WithLp.toLp 2 (olsRFromMoments (Causalean.Stat.finMean
          (fun i ↦ olsRawMoment x y (data i)))))) =
...
/-- For [an iid sample](hyp:S), [regressors](hyp:x), [an outcome](hyp:y), [a sample size](hyp:n),
and [an outcome of the sampling experiment](hyp:omega), [the sample-function OLS estimator
equals the library OLS estimator on the sample vector](goal). -/
theorem olsSampleEstimator_sampleVector
  ...
  unfold olsSampleEstimator olsBetaHat olsEmpiricalMoments IIDSample.sampleMeanVec
    IIDSample.sampleVector Causalean.Stat.finMean
  exact congrArg (fun M : OLSMoment K =>
    olsBetaFromMoments ((n : ℝ)⁻¹ • M))
      (Fin.sum_univ_eq_sum_range
        (fun i => olsRawMoment x y (S.Z i omega)) n)

/-- For [an iid sample](hyp:S), [measurable regressors](hyp:hx), [a measurable outcome](hyp:hy),
[integrable OLS raw moments](hyp:hraw), and [a positive-definite population Gram matrix](hyp:hQ),
[the OLS sample-function estimator solves its score equation with probability tending to one](goal). -/
theorem olsSampleEstimator_solvesInProbability
  ...
  unfold zEstimatorSampleScore Causalean.Stat.finMean IIDSample.sampleVector
...
/-- For [an iid sample](hyp:S), [measurable regressors](hyp:hx), [a measurable outcome](hyp:hy),
[integrable OLS raw moments](hyp:hraw), [a positive-definite population Gram matrix](hyp:hQ), [a
continuous linear contrast](hyp:c), [bootstrap consistency of the OLS coefficient](hyp:hBootConsistent),
[bootstrap normal-equation validity](hyp:hBootSolve), and [positive contrast influence variance](hyp:hVar),
[the OLS contrast is bootstrap asymptotically linear](goal). -/
theorem olsContrast
    [IsProbabilityMeasure mu]
    (S : IIDSample Omega X mu P)
    {x : X → EuclideanSpace ℝ K} {y : X → ℝ}
