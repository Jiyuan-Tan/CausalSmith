...
/-- The `k`-fold i.i.d. product of a one-coordinate marked-Poisson predictive
law. -/
...
/-- The i.i.d. product marked-Poisson predictive law is a probability measure
...
/-- A one-coordinate TV estimate tensorizes to `k` independent coordinates
by `tvDist_pi_iid_le`. -/
...
/-- Given [a normalized certificate](hyp:C), [a finite product dimension](hyp:k), [experiment and support parameters](hyp:ε,a,u,v,κ,B,C₀,ρ), [positive shift and support-ratio conditions](hyp:ha,hκ), [the stated support condition](hyp:hsupp), and [a one-coordinate geometric total-variation estimate](hyp:hone), [the product predictive laws satisfy the corresponding dimension-scaled total-variation bound](goal). -/
