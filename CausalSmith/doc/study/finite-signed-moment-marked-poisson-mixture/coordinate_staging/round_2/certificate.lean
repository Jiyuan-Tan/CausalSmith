...
/-- A normalized finite signed moment certificate consists of distinct real
...
/-- The atomic signed measure represented by a finite signed certificate. -/
...
/-- The atomic absolute-weight measure associated with a finite signed
certificate. -/
...
/-- The signed certificate's vector-measure variation is exactly its atomic
...
/-- The total signed mass of the certificate is zero, as the degree-zero
...
/-- Integration against the atomic signed measure recovers every vanishing
...
/-- The variation of a normalized finite signed certificate is a probability
measure. -/
...
/-- The measurable atomic polar sign: it equals the sign of the unique weight
at a certificate node and is zero away from all nodes. -/
noncomputable def polarSign
    (C : NormalizedFiniteSignedMomentCertificate ι L) (x : ℝ) : ℝ :=
  ∑ i, if x = C.node i then Real.sign (C.weight i) else 0

/-- The atomic polar sign is Borel measurable. -/
theorem measurable_polarSign
    (C : NormalizedFiniteSignedMomentCertificate ι L) :
    Measurable C.polarSign := by
  exact measurable_atomicPolarSign C

/-- The atomic polar sign has absolute value one almost everywhere for the
variation measure. -/
...
/-- The atomic polar sign is integrable against the variation measure. -/
...
/-- Polar decomposition of the atomic signed measure: the signed certificate
...
/-- The normalized positive Jordan prior, obtained by doubling the positive
...
/-- The normalized negative Jordan prior, obtained by doubling the negative
atomic mass of the signed certificate. -/
...
/-- The positive Jordan prior is a probability measure. -/
...
/-- The negative Jordan prior is a probability measure. -/
...
/-- The positive part of the signed measure's Jordan decomposition is one
...
/-- The negative part of the signed measure's Jordan decomposition is one
...
/-- The two normalized Jordan priors have equal moments through degree `L`. -/
...
/-- The positive and negative priors are concentrated on the finite set of
certificate nodes. -/
...
/-- The null prior oriented for a target `f`; the naming is chosen so that the
...
/-- The alternative prior oriented for a target `f`; it swaps the Jordan
...
/-- The target-oriented null prior is a probability measure. -/
...
/-- The target-oriented alternative prior is a probability measure. -/
...
/-- Given [a normalized finite signed certificate](hyp:C) and [a target function](hyp:f), [the target-oriented prior difference equals twice the absolute signed target evaluation](goal). -/
...
/-- The target-oriented Jordan priors retain equal moments through degree
`L`. -/
...
/-- An existing finite alternation moment dual canonically supplies a
