...
/-- The two-count observation consisting of aggregate treated and aggregate
control counts. -/
abbrev AggregatePoissonObservation := ℕ × ℕ

/-- Aggregate treated intensity at latent mass `p`. -/
def aggregateTreatedRate (ε a t p : ℝ) : ℝ := t * ε * (p + a)

/-- Aggregate control intensity at latent mass `p`. -/
def aggregateControlRate (ε a t p : ℝ) : ℝ :=
  t * ((1 - ε) * p - ε * a)

/-- The explicit pair of independent Poisson laws with aggregate treated and
control rates. -/
...
/-- Every aggregate two-count law is a probability measure. -/
...
/-- The aggregate affine-Poisson experiment as a kernel from latent mass to
...
/-- The aggregate kernel fibre is the explicit product-Poisson law. -/
...
/-- Mixing the aggregate affine-Poisson kernel against a latent prior gives
...
/-- A square root of an exponential-series tail is uniformly geometric in
...
/-- For fixed overlap geometry, the aggregate affine-Poisson mixtures of the
...
/-- Given [admissible overlap geometry](hyp:ε,κ,hε,hεhalf,hκ), a finite signed certificate, and positive shift, support-bound, and exposure parameters, [there are positive bandwidth and prefactor constants and a geometric factor that bound the aggregate Jordan-prior predictive total variation under the stated support and bandwidth conditions](goal). -/
