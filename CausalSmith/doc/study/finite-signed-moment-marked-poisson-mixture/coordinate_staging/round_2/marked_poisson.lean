...
/-- One coordinate of the marked-Poisson experiment: labeled treated counts
with marks one and zero, followed by auxiliary-treated and aggregate-control
counts. -/
abbrev MarkedPoissonObservation := (ℕ × ℕ) × (ℕ × ℕ)

/-- The treated mass attached to latent mass `p`, overlap level `ε`, and shift
`a`. -/
def treatedMass (ε a p : ℝ) : ℝ := ε * (p + a)

/-- The aggregate control mass left after subtracting the treated mass from
the latent cell mass. -/
def controlMass (ε a p : ℝ) : ℝ := p - treatedMass ε a p

/-- The signed outcome mark for a branch: the alternative uses `h`, while the
null uses `-h`. -/
def branchMark (branch : Bool) (h : ℝ) : ℝ := if branch then h else -h

/-- The explicit law of the four independent Poisson counts at latent `p`.
...
/-- The marked-Poisson law is a probability measure for every parameter. -/
...
/-- The four-count experiment as a probability kernel from the latent real
...
/-- The marked-Poisson kernel's fibre is the explicit four-count law. -/
...
/-- The one-coordinate prior-predictive marked-Poisson law obtained by mixing
...
/-- The one-coordinate marked-Poisson predictive law is a probability measure
...
/-- The event that no labeled treated observation appears in either outcome
mark. -/
def noLabeledTreated : Set MarkedPoissonObservation :=
  {z | z.1.1 + z.1.2 = 0}

/-- The no-labeled-treated event is measurable. -/
...
/-- On the zero-labeled-count event, the two prior-predictive laws agree
...
/-- The marked-law discrepancy is at most the labeled Palm intensity
...
/-- Given [admissible overlap geometry](hyp:ε,κ,hε,hεhalf,hκ), a finite signed certificate, and shift, support, and nonnegative treated and control intensities, [there are positive bandwidth and prefactor constants and a geometric factor that bound the one-coordinate marked-Poisson predictive total variation under the stated support, positivity, and bandwidth conditions](goal). -/
