...
P(e)=\sum_xP(e,x),
\]
which is the checked ancestor marginalization identity. For \(\mathsf{SPLIT}\), the compiler enumerates all assignments and the districts of \(G[H]\); semi-Markovian district factorization identifies the ancestor-closed probability with the checked finite sum of products. Hence the first three transitions preserve both root denotations.
...
\]
The admissibility proof and district-kernel invariance replace \(q_D^s\) by \(q_D^\star\). For every retained derived row, the district-cell-linearity lemma gives
\[
