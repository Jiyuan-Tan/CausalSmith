...
\tag{1}
\]
The minimizer in \({\rm def:exact-optimal-bandwidth}\) exists.  Across all possible degrees, the finite-lattice scores and certificates in \({\rm def:pc-handle}\) have finitely many bandwidth breakpoints.  On every interval beginning at one of its included breakpoints they are constant, apart from the Taylor term, which is continuous and nondecreasing.  Hence the supremum over \(P\in\mathcal M_{n,d,L}\) is nondecreasing on each such interval and attains that interval's minimum at its included left endpoint.  There are finitely many endpoints in the nonempty compact interval \(\mathcal H_{n,d}\).  A global minimum is therefore attained, its smallest minimizer \(h^\dagger\) is well defined, and it is fixed by the model class rather than the realized sample.
