...
\]
Raising this bound to power \(q-2\), multiplying by the square, and taking expectations proves (4), because \(q!/2\geq1\).  For \(r=1\), the centered eigenfunction is zero.  Equations (1)--(4), independent uniform latent types, \({\rm ass:graphon-draw}\), and \({\rm ass:bernoulli-design}\) verify every graph, rank, regularity, and design condition in \({\rm def:model-class}\).
...
\tag{5}
\]
For deterministic \(g\in C^3[0,1]\), define
\[
...
The square root of (5), extended by zero, is absolutely continuous and has squared derivative norm \(\pi^2/(4s_0^2)\).  The fundamental theorem of calculus and Minkowski's inequality therefore give
\[
...
Put \(\bar c=c_0/r\).  Conditional on a unit's block, it connects independently to each other unit with marginal probability \(p_n=\bar c d/n\), hence
\[
 N_i\sim{\rm Binomial}(n-1,p_n),
 \qquad \mu_n=(n-1)p_n\asymp d.
\tag{11}
\]
The identity \(\mathbb E(N_i+1)^{-1}=\{1-(1-p_n)^n\}/(np_n)\), the inequality \(N^{-1}\mathbf1(N>0)\leq2(N+1)^{-1}\), and \(\mathbb E_Z(V_i^2\mid A,N_i=N)=1/(4N)\) imply
\[
...
For completeness, put \(a_q={2q\choose q}4^{-q}\).  The identities
\[
 \frac{a_{q+1}}{a_q}=\frac{2q+1}{2q+2},
 \qquad
 \left(\frac{2q+1}{2q+2}\right)^2\leq\frac{q+1}{q+2}
\]
give by induction \(a_q\leq(q+1)^{-1/2}\); an odd binomial central atom is bounded by an adjacent even one.  Thus the largest atom of \({\rm Binomial}(N,1/2)\) is at most \(CN^{-1/2}\).  Since \(|M/N-1/2|\leq h\) contains at most \(2Nh+2\) lattice points,
\[
 \Pr\{|M/N-1/2|\leq h\mid N\}\leq C(\sqrt N\,h+N^{-1/2}).
\tag{16}
\]
For a binomial variable \(N\) of mean \(\mu\), exponential Markov's inequality applied to \(e^{-(\log2)N}\) gives
\[
 \Pr(N<\mu/2)
 \leq e^{(\log2)\mu/2}\mathbb E e^{-(\log2)N}
 \leq\exp\{-\mu(1-\log2)/2\}.
\]
Splitting at \(N_i=\mu_n/2\), using \(N_i^{-1/2}\leq1\) on \(N_i>0\), and applying Jensen's inequality give \(\mathbb E\sqrt{N_i}\leq C\sqrt d\) and \(\mathbb E\{N_i^{-1/2}\mathbf1(N_i>0)\}\leq Cd^{-1/2}\).  Since \(h\geq d^{-1}\), (16) implies
\[
...
For the lattice pair, fix \(K>e\bar c\) so large that \(a_K:=K\log\{K/(e\bar c)\}\) satisfies \(a_KC_{\log}>2\), and put \(D_n=\lceil Kd\rceil\).  For \(t>0\), exponential Markov and \(1+y\leq e^y\) give
\[
 \Pr(N_i>D_n)\leq
 \exp\{\mu_n(e^t-1)-tD_n\}.
\]
Choosing \(e^t=D_n/\mu_n>1\), and using \(D_n\geq Kd\) and \(\mu_n\leq\bar c d\), yields \(\Pr(N_i>D_n)\leq e^{-a_Kd}\).  A union bound and \({\rm ass:log-degree}\) now give
\[
...
\tag{21}
\]
Derivative scaling verifies (7).  If \(1\leq N_i\leq D_n\), every noncentral exposure-lattice point has absolute centered value at least \((2D_n)^{-1}>w_n\); at a central point the odd bump is zero, and at an isolate it is also zero.  Thus the observed laws coincide on \(\{\max_iN_i\leq D_n\}\), and
\[
...
For any of these pairs, an estimator \(T\) satisfies
\[
