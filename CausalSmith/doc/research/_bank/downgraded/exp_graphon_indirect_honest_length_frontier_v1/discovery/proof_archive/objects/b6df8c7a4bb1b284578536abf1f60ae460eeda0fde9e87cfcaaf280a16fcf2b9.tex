...
\]
Fix \(i\) with \(N_i=m>0\).  By \(\texttt{ass:bernoulli-design}\), conditional on \(A\) and all preassignment variables, \(M_i\sim {\rm Binomial}(m,1/2)\), and \(Z_i\) is independent of \(M_i\).  Because this conditional law of \(Z\) is constant in the latent types, the same assertions hold after conditioning only on \(A\) and \((f_j)_{j\in\mathcal V_n}\), by iterated conditional expectation.  Therefore \(\texttt{ass:anonymous-interference}\) and \(\texttt{def:li-wager-target}\) give the unit-level contribution
\[
\begin{aligned}
\beta_i
...
\]
Taylor's formula with integral remainder along the segment from \(1/2\) to \(1/2+v\), which lies in \([0,1]\) for every realized \(v=V_i\), gives
\[
g_i(1/2+v)
...
It remains to establish the asserted asymptotic comparison.  Consider a triangular-array sequence satisfying the hypotheses of LiWager2022RandomGraph Theorem~6 and the assumptions declared in this proposition.  By \(\texttt{lem:li-wager-theorem-six-regime}\),
\[
...
\]
By the independent uniform latent types in \(\texttt{ass:unit-sampling}\) and the conditional edge independence in \(\texttt{ass:graphon-draw}\), after conditioning on \(U_i=u\) the edge indicators \((A_{ij})_{j\ne i}\) are independent and identically distributed Bernoulli variables with success probability
\[
...
\]
The parameter \(p_n(u)\) belongs to \([0,1]\) by \(\texttt{ass:edge-probability}\) and \(\texttt{ass:kernel-bound}\).  By \(\texttt{ass:degree-regularity}\), for Lebesgue-almost every \(u\),
\[
\mu_i(u):=\mathbb E[N_i\mid U_i=u]
=(n-1)p_n(u)
\ge\frac{(n-1)\rho_n}{\kappa}.
\]
Because every \(U_i\) is uniform by \(\texttt{ass:unit-sampling}\), all realized latent types lie in this full-measure set almost surely.

We next derive the required degree lower-tail estimate.  If \(S\sim{\rm Binomial}(m,p)\), \(\mu=mp\), and \(t=\log2\), exponential Markov's inequality and \(1-y\le e^{-y}\) give
\[
\begin{aligned}
\mathbb P(S\le\mu/2)
...
\end{aligned}
\]
where the last inequality uses \((1-\log2)/2\ge1/8\).  Applying this estimate conditionally on each \(U_i\), using the preceding lower bound for \(\mu_i(U_i)\), and taking a union bound yields, for
\[
\mathcal E_n
...
\]
Here \(L\) and \(\kappa\) are fixed class constants.  Combining the last three displays proves
\[
\bar\tau_{\mathrm{IND},n}-\theta_n=o_P(\sqrt{\rho_n}).
\]
Finally, \(\texttt{lem:li-wager-theorem-six-regime}\) records that the published principal-component-balancing Gaussian limit is centered at \(\bar\tau_{\mathrm{IND},n}\) on the same \(\sqrt{\rho_n}\) scale.  The preceding display therefore makes that theorem an asymptotic benchmark for \(\theta_n\).  The two finite-sample targets remain distinct by \(\texttt{def:target}\) and \(\texttt{def:li-wager-target}\), exactly as the degree-exact inequality records.