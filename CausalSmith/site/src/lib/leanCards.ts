/**
 * The structured (hypotheses / conclusion-cards) view of a proposition-bearing
 * declaration, shared by the paper drawer (`paperLean.ts`) and the library
 * page (`leanDeclView.ts`). Lifts a goal's leading `∀`/`premise →` telescope
 * into hypothesis rows, hoists `∃` runs, and splits conjunctions into nested
 * cards — recursively, so a theorem whose conclusion is itself an implication
 * or a bundle of clauses expands level by level. Every view is checked, token
 * by token, against its source: a view that would drop content is refused.
 *
 * PORTED to `tools/src/presentation/lean_structure.ts` for P4 (row identity
 * lives there); keep the two in sync.
 */
...
export function matchedCloseIndex(text: string): number {
...
export function keywordAt(text: string, i: number, kw: string): boolean {
 ...
 */
export function leadingImplicationIndex(text: string): number {
  const depths = normAwareDepths(text);
  for (let i = 0; i < text.length; i++) {
...
export function conjunctsOf(text: string): string[] | null {
...
export function stripLetRun(text: string): LetRun | null {
 ...
 * caller checks.
 */
export function liftTelescope(raw: string): Lift {
  let t = raw.trim();
  const prefix: string[] = [];
  const suffix: string[] = [];
  const hyps: HypRow[] = [];
 ...
 */
export function buildCard(raw: string, depth: number): BuiltCard | null {
 ...
 */
export function markCitedHyps(rows: readonly HypRow[], citedNames: ReadonlySet<string>): void {
...
export function allHypRows(view: StructuredView): HypRow[] {
