...
module
public import CausalSmith.Substrate.NestedPairedPoissonProductMoments.Flattening
...
/-- The total number of points in all two-count cells of a finite nested array. -/
...
/-- Every natural power of a real-cast Poisson count is integrable. -/
...
/-- Every natural power of the total count is integrable under a finite nested paired law. -/
...
/-- A measurable real statistic with polynomial growth in the total count belongs to `L²`. -/
...
/-- Polynomial growth of a statistic implies `L²` closure of every first-count add-one increment. -/
...
/-- Polynomial growth of a statistic implies `L²` closure of every second-count
add-one increment. -/
...
/-- One polynomial-growth hypothesis supplies all `L²` premises of the nested Poincare theorem. -/
