...
/-- The flattened coordinate type for two counts at every cell of a finite nested array. -/
abbrev NestedPairedPoissonIndex (iota kappa : Type*) :=
  Sigma fun _ : iota => Sigma fun _ : kappa => Fin 2

/-- The flattened rate selects the first or second cell rate according to its `Fin 2` tag. -/
...
/-- Currying a finite dependent product preserves the corresponding product probability law. -/
...
/-- Applying coordinatewise measure-preserving equivalences preserves a finite product law. -/
...
/-- The canonical measurable equivalence from a flat count vector to a nested array of pairs. -/
...
/-- The canonical measurable equivalence flattening a nested array of pairs to one count vector. -/
...
/-- Unflattening evaluates the two tagged flat coordinates as the two entries in a nested cell. -/
...
/-- Flattening at the first tag returns the first count in the selected nested cell. -/
...
/-- Flattening at the second tag returns the second count in the selected nested cell. -/
...
/-- The flat Poisson product is transported by unflattening to the nested paired law. -/
...
/-- The canonical flattening transports the nested paired law to the flat Poisson product. -/
...
/-- Incrementing a first tagged flat coordinate becomes first-count add-one after unflattening. -/
...
/-- Incrementing a second tagged flat coordinate becomes second-count add-one after unflattening. -/
