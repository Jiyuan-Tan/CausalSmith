...
This module isolates the ordered sign-block argument in the necessity proof
of Chebyshev's alternation theorem.  If a continuous residual has too few
alternating extrema, a low-degree polynomial can match its sign at every
point where the residual has maximum absolute value.
-/
...
most `L` has the residual's strict sign at every norm-attaining point. -/
