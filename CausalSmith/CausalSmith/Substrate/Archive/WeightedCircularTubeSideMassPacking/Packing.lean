module
public import CausalSmith.Substrate.Archive.WeightedCircularTubeSideMassPacking.Core
...
This file uses the non-strict convention that distinct sites are at distance at least `r`.  With
that convention, inclusion-maximality implies coverage by *open* radius-`r` balls.  Compactness
produces a finite maximal set.  Comparing chord distance with angular distance then gives matching
upper and lower cardinality bounds of order `h⁻¹` for radius `3h`.
-/
...
/-- A set is separated at scale `r` when every pair of distinct points has distance at least `r`. -/
def IsSeparatedAtLeast (r : ℝ) (s : Set Plane) : Prop :=
  s.Pairwise fun x y => r ≤ dist x y

/-- A finite site set is maximal `r`-separated on the unit circle when its underlying set is an
inclusion-maximal separated subset of the circle. -/
...
/-- A site set covers the unit circle at radius `r` when every circle point lies in an open ball of
radius `r` centered at one of the sites. -/
...
/-- Every finite inclusion-maximal non-strictly `r`-separated subset covers the unit circle by open
balls of radius `r`. -/
...
/-- At every positive scale there exists a finite inclusion-maximal separated subset of the unit
circle. -/
...
/-- There are universal positive constants such that every finite maximal `3h`-separated subset of
the unit circle has cardinality between those constants times `h⁻¹`, for all sufficiently small
positive `h`; it also covers the circle by open `3h`-balls. -/
...
/-- For every sufficiently small positive `h`, a finite maximal `3h`-separated covering set exists
and its cardinality satisfies the same universal two-sided `h⁻¹` bounds. -/
