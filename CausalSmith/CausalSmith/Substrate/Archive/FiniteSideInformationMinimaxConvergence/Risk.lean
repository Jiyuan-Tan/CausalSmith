...
/-- If the [label probabilities sum to one](hyp:hsum), [actions and targets lie in the same
ordered interval](hyp:hlu,htau), then the [finite squared risk is bounded](goal) between zero
and the squared interval width. -/
...
/-- When every [label coordinate](hyp:hp) and the [target](hyp:htau) are continuous, the
[finite squared risk of a fixed decision is continuous](goal) in the parameter. -/
...
/-- When every [label coordinate](hyp:hp) and the [target](hyp:htau) are continuous, the
[finite squared risk is jointly continuous](goal) in the bounded decision and parameter. -/
...
/-- The space of [bounded decisions on a finite alphabet](hyp:X) is [compact](goal) when the
action interval is ordered. -/
