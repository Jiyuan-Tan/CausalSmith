module
public import CausalSmith.Substrate.Archive.FiniteSideInformationMinimaxConvergence.Experiments
...
/-- For [label probabilities and a target](hyp:p,tau), [side coordinates](hyp:q,hq), and a
[center parameter](hyp:theta0), the [exact-fiber local minimax value](goal) minimizes finite
squared risk over decisions and maximizes over parameters with the same side law as the center. -/
noncomputable def fiberMinimaxValue (p : Theta → X → ℝ) (tau : Theta → ℝ)
  ...
  minimaxValueReal (fun (d : BoundedDecision X l u)
    (theta : {theta : Theta // sidePmf q hq theta = sidePmf q hq theta0}) ↦
      finiteSquaredRisk p tau d theta.1)

/-- For [label probabilities and a target](hyp:p,tau), [side coordinates](hyp:q,hq), a
[radius](hyp:r), and a [center parameter](hyp:theta0), the [local minimax value](goal)
minimizes finite squared risk over decisions and maximizes over parameters whose side law is
within that radius of the center law. -/
...
theorem localMinimaxValue_tendsto_fiber (p : Theta → X → ℝ)
  ...
  · intro b hb
    ...
    have hvalue : (⨅ d, worstCaseRiskReal fiberRisk d) < c := by
      simpa [fiberMinimaxValue, minimaxValueReal, fiberRisk] using hvalue_lt_c
    ...
    change minimaxValueReal localRisk < b
