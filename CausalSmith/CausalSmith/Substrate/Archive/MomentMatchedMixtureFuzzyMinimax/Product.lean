module
public import CausalSmith.Substrate.Archive.MomentMatchedMixtureFuzzyMinimax.Analytic
...
/-- The independent `d`-coordinate parameter prior is the finite product of the same scalar
prior in every coordinate. -/
noncomputable def productPrior (d : ℕ) (π : Measure ℝ) : Measure (Fin d → ℝ) :=
  Measure.pi fun _ : Fin d => π

/-- The `d`-coordinate predictive law for independent, identically mixed coordinates is the
finite product of the one-coordinate prior-predictive law. -/
noncomputable def productPriorPredictive (d : ℕ) (π : Measure ℝ) (K : Kernel ℝ X) :
    Measure (Fin d → X) :=
  Measure.pi fun _ : Fin d => priorPredictive π K

/-- A finite product of probability prior-predictive coordinate laws is again a probability
measure. -/
...
/-- On every measurable coordinate rectangle, mixing the coordinatewise product experiment
against the product prior factors into the product of the one-coordinate predictive masses. -/
...
/-- Mixing the coordinatewise product experiment against the product prior gives exactly the
product of the one-coordinate prior-predictive mixtures.  The supplied `productKernel` exposes
the measurability of the parameter-indexed product experiment while `hfiber` identifies each
of its fibres with the corresponding finite product law. -/
...
/-- Replacing both factors of a binary product probability law changes total variation by at
most the sum of the total-variation changes in the two coordinates. -/
...
/-- Total variation between two finite i.i.d. product laws is at most the number of coordinates
times their one-coordinate total variation. -/
...
/-- If the one-coordinate moment-matched predictive mixtures are bounded by the explicit
series-tail estimate, then `d` independent coordinates are within `d * sqrt(tail)` in total
variation. -/
