module
public import CausalSmith.Substrate.Archive.MomentMatchedMixtureFuzzyMinimax.Basic
public import CausalSmith.Substrate.Archive.MomentMatchedMixtureFuzzyMinimax.ExponentialEnergy
...
/-- The likelihood of a prior-predictive mixture is the prior average of the component
likelihoods. -/
noncomputable def mixtureLikelihood (π : Measure ℝ) (likelihood : ℝ → X → ℝ) (x : X) : ℝ :=
  ∫ θ, likelihood θ x ∂π

/-- A dominated probability kernel whose fibre likelihood is `likelihood θ` has
prior-predictive density equal to the prior average `mixtureLikelihood π likelihood`. -/
...
/-- The squared distance between two mixture likelihoods equals the alternating sum of the four
exponential prior energies when the component likelihoods have the exponential Gram kernel. -/
...
/-- Under an exponential likelihood Gram identity, two probability priors supported on
`[-a,a]` and matching moments through `degree` have squared mixture-likelihood distance at most
four times the unmatched exponential-series tail. -/
...
/-- The two predictive mixtures are absolutely continuous with respect to the common dominating
measure whenever every experiment fibre is represented by a nonnegative likelihood. -/
...
/-- A common probability dominating measure converts a square-integrable difference of two
probability densities into a total-variation bound by one half of its square root. -/
...
/-- Exponential likelihood inner products, bounded support, and moment agreement through
`degree` imply that the two one-coordinate prior-predictive mixtures are within the square root
of the explicit unmatched exponential-series tail in total variation. -/
