...
This module isolates the analytic change-of-variables statement used to transport conditional
independence: conditional expectation commutes, almost everywhere, with composition by a
measure-preserving measurable equivalence when the conditioning σ-algebra is pulled back too.
-/
...
/-- Under a measure-preserving measurable equivalence, the conditional expectation of an
integrable target-space function, after composition with the equivalence and conditioning on the
pulled-back σ-algebra, is the target conditional expectation composed with the equivalence. -/
...
/-- Under a measure-preserving measurable equivalence, the conditional probability of a target
event given a target σ-algebra pulls back to the conditional probability of its preimage given the
pulled-back σ-algebra. -/
